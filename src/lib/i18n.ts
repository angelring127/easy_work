// 커스텀 다국어 시스템

// 지원하는 언어
export const locales = ["en", "ko", "ja"] as const;
export type Locale = (typeof locales)[number];

// 기본 언어
export const defaultLocale: Locale = "en";

// 언어명 매핑
export const localeNames: Record<Locale, string> = {
  en: "English",
  ko: "한국어",
  ja: "日本語",
};

// 번역 데이터
const translations = {
  ko: {
    "schedule.autoAssign": "자동 배정",
    "schedule.autoAssignError": "자동 배정 중 오류가 발생했습니다.",
    "schedule.autoAssignResultTitle": "자동 배정 결과",
    "schedule.autoAssignResultDescription":
      "새 스케줄 {created}개를 등록하고 저장했습니다.",
    "schedule.autoAssignUnmetTitle": "운영 커버리지 경고 {count}개",
    "schedule.autoAssignUnmetGuidance":
      "아래 구간은 스케줄에서 수동으로 보정해주세요.",
    "schedule.autoAssignUnmetHeadcount":
      "필요 인원 {required}명 중 {assigned}명 배정",
    "schedule.autoAssignMissingRoles": "미충족 역할: {roles}",
    "schedule.autoAssignComplete":
      "모든 운영 패턴 구간의 최소 인원과 역할을 충족했습니다.",
    "schedule.deleteWeek": "주 삭제",
    "schedule.deleteWeekConfirm": "삭제",
    "schedule.deleteWeekConfirmTitle": "현재 주 스케줄을 삭제할까요?",
    "schedule.deleteWeekConfirmDescription":
      "{week}에 등록된 스케줄 {count}개가 모두 삭제됩니다. 이 작업은 되돌릴 수 없습니다.",
    "schedule.deleteWeekSuccess": "스케줄 {count}개가 삭제되었습니다.",
    "schedule.deleteWeekError": "현재 주 스케줄 삭제 중 오류가 발생했습니다.",
    "settings.store.autoSchedule": "자동 스케줄",
    "autoSchedule.title": "자동 스케줄 설정",
    "autoSchedule.description":
      "자동 배정 조건 우선순위, 운영 패턴, 직원 배정 순서를 매장별로 관리합니다.",
    "autoSchedule.loadError": "자동 스케줄 설정을 불러오지 못했습니다.",
    "autoSchedule.saveSuccess": "자동 스케줄 설정이 저장되었습니다.",
    "autoSchedule.saveError": "자동 스케줄 설정 저장에 실패했습니다.",
    "autoSchedule.conditionPriorities": "조건 우선순위",
    "autoSchedule.conditionPrioritiesDescription":
      "필수 조건을 통과한 후보에게 적용할 선호 조건의 우선순위를 정합니다.",
    "autoSchedule.condition.desiredWeeklyHours": "희망 주 근무 시간",
    "autoSchedule.condition.dayOffPreference": "휴일 희망일",
    "autoSchedule.condition.preferredWeekday": "출근 선호일",
    "autoSchedule.points": "점",
    "autoSchedule.requiredConditions": "잠긴 필수 조건",
    "autoSchedule.required.weeklyMax": "주간 최대 근무 시간",
    "autoSchedule.required.monthlyMax": "월간 최대 근무 시간",
    "autoSchedule.required.unavailable": "출근 불가",
    "autoSchedule.required.branchConflict": "다른 브랜치 출근 충돌",
    "autoSchedule.required.missingRole": "직무 미보유",
    "autoSchedule.required.openingAnchor": "오프닝 앵커 근무",
    "autoSchedule.required.staffLimit": "오전/오후 최대 인원",
    "autoSchedule.required.coverageGap": "영업시간 빈틈 없음",
    "autoSchedule.userPriorities": "직원 배정 우선순위",
    "autoSchedule.userPrioritiesDescription":
      "직원 목록을 드래그하거나 버튼으로 움직여 자동 배정 보조 점수 순서를 정합니다.",
    "autoSchedule.noMembers": "정렬할 활성 직원이 없습니다.",
    "autoSchedule.moveUp": "위로 이동",
    "autoSchedule.moveDown": "아래로 이동",
    "autoSchedule.openingPolicy": "오프닝 정책",
    "autoSchedule.openingPolicyDescription":
      "오프닝 앵커 사용 여부와 시간, 필요 인원, 실패 처리 방식을 설정합니다.",
    "autoSchedule.openingEnabled": "오프닝 앵커 사용",
    "autoSchedule.openingEnabledDescription":
      "영업 시작 구간에 오프닝 가능 직원을 먼저 배정합니다.",
    "autoSchedule.openingStartSource": "오프닝 시작 기준",
    "autoSchedule.startSourceBusinessOpen": "영업 시작 시간 사용",
    "autoSchedule.startSourceCustom": "별도 시간 사용",
    "autoSchedule.openingCustomStart": "별도 시작 시간",
    "autoSchedule.openingEndTime": "오프닝 종료 시간",
    "autoSchedule.openingRequiredHeadcount": "오프닝 필요 인원",
    "autoSchedule.openingFailureMode": "오프닝 실패 시 처리",
    "autoSchedule.failureWarn": "일반 배정 계속 + 필수 경고",
    "autoSchedule.failureBlock": "등록 차단",
    "autoSchedule.openingWorkItems": "오프닝 가능 근무 항목",
    "autoSchedule.noWorkItems": "선택할 근무 항목이 없습니다.",
    "autoSchedule.pattern.title": "운영 패턴",
    "autoSchedule.pattern.description":
      "요일별 시간 구간에 필요한 최소 인원과 팀 역할 커버리지를 설정합니다.",
    "autoSchedule.pattern.add": "패턴 추가",
    "autoSchedule.pattern.empty":
      "등록된 운영 패턴이 없습니다. 패턴이 없으면 기존 자동 배정 기준을 사용합니다.",
    "autoSchedule.pattern.untitled": "이름 없는 패턴",
    "autoSchedule.pattern.active": "패턴 활성화",
    "autoSchedule.pattern.remove": "패턴 삭제",
    "autoSchedule.pattern.name": "패턴 이름",
    "autoSchedule.pattern.weekdays": "적용 요일",
    "autoSchedule.pattern.weekday.sun": "일",
    "autoSchedule.pattern.weekday.mon": "월",
    "autoSchedule.pattern.weekday.tue": "화",
    "autoSchedule.pattern.weekday.wed": "수",
    "autoSchedule.pattern.weekday.thu": "목",
    "autoSchedule.pattern.weekday.fri": "금",
    "autoSchedule.pattern.weekday.sat": "토",
    "autoSchedule.pattern.segmentName": "구간 이름",
    "autoSchedule.pattern.startTime": "시작 시간",
    "autoSchedule.pattern.endTime": "종료 시간",
    "autoSchedule.pattern.nextDay": "다음 날",
    "autoSchedule.pattern.minHeadcount": "최소 인원",
    "autoSchedule.pattern.requiredRoles": "필요한 역할",
    "autoSchedule.pattern.noRoles": "선택할 활성 직책이 없습니다.",
    "autoSchedule.pattern.removeSegment": "구간 삭제",
    "autoSchedule.pattern.addSegment": "구간 추가",
    "autoSchedule.pattern.fixErrors":
      "운영 패턴의 오류를 수정한 뒤 다시 저장해주세요.",
    "autoSchedule.pattern.issue.duplicateWeekday":
      "같은 요일은 하나의 활성 운영 패턴에만 배정할 수 있습니다.",
    "autoSchedule.pattern.issue.emptyPatternName": "패턴 이름을 입력해주세요.",
    "autoSchedule.pattern.issue.emptySegmentName": "구간 이름을 입력해주세요.",
    "autoSchedule.pattern.issue.invalidTime":
      "구간 시작 시간은 종료 시간보다 빨라야 합니다.",
    "autoSchedule.pattern.issue.invalidHeadcount":
      "최소 인원은 1명 이상이어야 합니다.",
    "autoSchedule.pattern.issue.overlap": "패턴 안의 시간 구간이 겹칩니다.",
    "autoSchedule.pattern.issue.gap":
      "시간 구간 사이에 운영 기준이 없는 빈틈이 있습니다.",
    "autoSchedule.pattern.issue.noRoles":
      "필요한 역할을 선택하지 않았습니다. 최소 인원만 검사합니다.",
    "schedule.morningStaff": "오전 인원",
    "schedule.afternoonStaff": "오후 인원",
    "schedule.morningStaffShort": "오전",
    "schedule.afternoonStaffShort": "오후",
    "schedule.morning": "오전",
    "schedule.afternoon": "오후",
    "home.title": "이번 주 근무표, 더 이상 감으로 짜지 마세요",
    "home.description":
      "바쁜 매장 운영자가 스케줄, 직원 가능 시간, 인력 커버리지를 한눈에 정리하고 빠르게 공유할 수 있는 운영 도구입니다.",
    "home.login": "로그인",
    "home.signup": "회원가입",
    "home.badge": "작은 매장을 위한 스케줄 운영 콘솔",
    "home.nav.features": "기능",
    "home.nav.preview": "미리보기",
    "home.nav.trust": "운영 안정성",
    "home.metric.schedule.value": "10분",
    "home.metric.schedule.label": "주간 근무표 초안 작성",
    "home.metric.coverage.value": "7일",
    "home.metric.coverage.label": "요일별 인력 커버리지 확인",
    "home.metric.team.value": "1곳",
    "home.metric.team.label": "직원 초대부터 배정까지 통합",
    "home.preview.kicker": "스케줄 미리보기",
    "home.preview.title": "이번 주 운영 흐름",
    "home.preview.status": "커버리지 안정",
    "home.preview.week.mon": "월",
    "home.preview.week.tue": "화",
    "home.preview.week.wed": "수",
    "home.preview.week.thu": "목",
    "home.preview.week.fri": "금",
    "home.preview.week.sat": "토",
    "home.preview.week.sun": "일",
    "home.preview.day.mon": "월요일 오픈",
    "home.preview.day.tue": "화요일 점심 피크",
    "home.preview.day.wed": "수요일 마감",
    "home.preview.shift.open": "08:00 - 13:00",
    "home.preview.shift.lunch": "11:00 - 16:00",
    "home.preview.shift.close": "17:00 - 22:00",
    "home.preview.role.barista": "바리스타 2명",
    "home.preview.role.kitchen": "주방 3명",
    "home.preview.role.closing": "마감 2명",
    "home.preview.coverageTitle": "자동 배정 전 점검 완료",
    "home.preview.coverageText":
      "희망 휴무, 최대 근무 시간, 역할별 필요 인원을 함께 확인해 빈 시간대를 빠르게 발견합니다.",
    "home.features.kicker": "Workeasy가 다른 점",
    "home.features.title": "매장 스케줄이 꼬이기 전에 먼저 보입니다",
    "home.feature.schedule.title": "교대 근무표를 빠르게 구성",
    "home.feature.schedule.description":
      "영업 시간, 근무 항목, 휴게 규칙을 기준으로 주간 스케줄 초안을 빠르게 만들고 수정합니다.",
    "home.feature.team.title": "직원 가능 시간을 한 곳에 정리",
    "home.feature.team.description":
      "초대, 역할, 출근 가능일, 휴무 희망을 매장 단위로 관리해 배정 실수를 줄입니다.",
    "home.feature.analytics.title": "근무 시간과 커버리지 확인",
    "home.feature.analytics.description":
      "직원별 근무 시간과 요일별 부족 인원을 확인해 다음 주 운영 결정을 더 쉽게 만듭니다.",
    "home.trust.kicker": "매장 운영을 위해 설계",
    "home.trust.title": "관리자는 빠르게, 직원은 명확하게",
    "home.trust.description":
      "Workeasy는 작은 팀이 매주 반복하는 스케줄 조율을 더 짧고 투명하게 만드는 데 집중합니다.",
    "home.trust.point.one": "매장별 권한과 역할을 분리해 필요한 사람만 관리 화면에 접근합니다.",
    "home.trust.point.two": "스케줄, 직원, 분석 흐름이 같은 매장 범위 안에서 연결됩니다.",
    "home.trust.point.three": "한국어, 영어, 일본어 화면을 함께 제공해 다국적 팀도 같은 정보를 봅니다.",
    "home.inviteError.title": "초대 링크 오류",
    "home.inviteError.description":
      "초대 링크 처리 중 오류가 발생했습니다. 다시 시도해주세요.",
    "auth.login.title": "계정에 로그인하세요",
    "auth.login.noAccount": "아직 계정이 없으신가요?",
    "auth.login.signup": "회원가입",
    "auth.login.loginCard": "로그인",
    "auth.login.description":
      "이메일과 비밀번호를 입력하여 Workeasy에 로그인하세요.",
    "auth.login.forgotPassword": "비밀번호를 잊으셨나요?",
    "auth.login.sessionExpired": "세션이 만료되었습니다. 다시 로그인해주세요.",
    "auth.login.email": "이메일",
    "auth.login.emailPlaceholder": "이메일을 입력하세요",
    "auth.login.password": "비밀번호",
    "auth.login.passwordPlaceholder": "비밀번호를 입력하세요",
    "auth.login.passwordShow": "비밀번호 보기",
    "auth.login.passwordHide": "비밀번호 숨기기",
    "auth.login.submit": "로그인",
    "auth.login.submitting": "로그인 중...",
    "auth.login.success": "로그인 성공",
    "auth.login.successDescription": "로그인이 완료되었습니다",
    "auth.login.error": "로그인 실패",
    "auth.login.errorDescription": "로그인에 실패했습니다",
    "auth.login.validation.invalidData": "입력 데이터가 유효하지 않습니다",
    "auth.login.validation.emailRequired": "이메일을 입력하세요",
    "auth.login.validation.invalidEmail": "유효한 이메일 주소를 입력하세요",
    "auth.login.validation.passwordRequired": "비밀번호를 입력하세요",
    "auth.login.error.general": "로그인 중 오류가 발생했습니다",
    "auth.login.error.invalidCredentials":
      "이메일 또는 비밀번호가 올바르지 않습니다",
    "auth.login.error.emailNotConfirmed":
      "이메일 확인이 필요합니다. 가입 시 받은 이메일을 확인해주세요",
    "auth.login.error.tooManyRequests":
      "너무 많은 로그인 시도입니다. 잠시 후 다시 시도해주세요",
    "auth.login.error.userNotFound": "등록되지 않은 이메일 주소입니다",
    "auth.login.loading": "로딩 중...",
    "auth.logout.success": "로그아웃이 완료되었습니다.",
    "auth.logout.error": "로그아웃 중 오류가 발생했습니다.",
    "auth.signup.title": "새 계정 만들기",
    "auth.signup.hasAccount": "이미 계정이 있으신가요?",
    "auth.signup.login": "로그인",
    "auth.signup.signupCard": "회원가입",
    "auth.signup.description":
      "이메일과 비밀번호를 입력하여 Workeasy에 가입하세요.",
    "auth.signup.email": "이메일",
    "auth.signup.emailPlaceholder": "이메일을 입력하세요",
    "auth.signup.password": "비밀번호",
    "auth.signup.passwordPlaceholder": "비밀번호를 입력하세요",
    "auth.signup.confirmPassword": "비밀번호 확인",
    "auth.signup.confirmPasswordPlaceholder": "비밀번호를 다시 입력하세요",
    "auth.signup.passwordShow": "비밀번호 보기",
    "auth.signup.passwordHide": "비밀번호 숨기기",
    "auth.signup.submit": "회원가입",
    "auth.signup.submitting": "회원가입 중...",
    "auth.signup.success": "회원가입 성공",
    "auth.signup.successDescription": "회원가입이 완료되었습니다",
    "auth.signup.error": "회원가입 실패",
    "auth.signup.errorDescription": "회원가입에 실패했습니다",
    "auth.signup.validation.invalidData": "입력 데이터가 유효하지 않습니다",
    "auth.signup.validation.invalidEmail": "유효한 이메일 주소를 입력하세요",
    "auth.signup.validation.passwordMinLength":
      "비밀번호는 최소 8자 이상이어야 합니다",
    "auth.signup.validation.passwordMismatch": "비밀번호가 일치하지 않습니다",
    "auth.signup.error.general": "회원가입 중 오류가 발생했습니다",
    "auth.signup.error.alreadyRegistered": "이미 등록된 이메일 주소입니다",
    "auth.signup.error.passwordInvalid": "비밀번호 조건을 확인해주세요",
    "auth.signup.error.invalidEmail":
      "유효하지 않은 이메일 주소입니다. 실제 이메일 주소를 사용해주세요",
    "auth.signup.error.emailCheck": "이메일 주소를 확인해주세요",
    "auth.signup.error.signupDisabled": "현재 회원가입이 비활성화되어 있습니다",
    "auth.signup.success.message": "회원가입이 완료되었습니다",
    "auth.signup.error.serverError": "서버 오류가 발생했습니다",
    "auth.signupComplete.title": "회원가입 완료",
    "auth.signupComplete.message": "회원가입이 완료되었습니다",
    "auth.signupComplete.description":
      "이메일 확인이 완료되었습니다. 로그인하여 서비스를 이용해주세요",
    "auth.signupComplete.goToLogin": "로그인 페이지로 이동",
    "auth.verify.title": "이메일 확인",
    "auth.verify.description":
      "가입하신 이메일 주소로 확인 링크를 보내드렸습니다.",
    "auth.verify.instruction":
      "이메일을 확인하고 링크를 클릭하여 계정을 활성화해주세요.",
    "auth.verify.spam": "이메일이 보이지 않나요? 스팸함을 확인해보세요.",
    "auth.verify.backToLogin": "로그인 페이지로 돌아가기",
    "dashboard.title": "대시보드",
    "dashboard.welcome": "안녕하세요, {name}님!",
    "dashboard.logout": "로그아웃",
    "dashboard.loading": "로딩 중...",
    "dashboard.wait": "잠시만 기다려주세요",
    "dashboard.currentStore": "현재 매장: {storeName} ({role})",
    "dashboard.noStores": "접근 가능한 매장이 없습니다",
    "dashboard.noStoresDescription":
      "매장에 초대받거나 매장을 생성해야 합니다.",
    "dashboard.noStoresModal.title": "매장 등록 필요",
    "dashboard.noStoresModal.message":
      "등록된 매장이 없습니다. 매장을 등록하시겠습니까?",
    "dashboard.noStoresModal.description":
      "매장을 등록하면 스케줄 관리 및 팀원 초대 등의 기능을 사용할 수 있습니다.",
    "dashboard.noStoresModal.goToCreate": "매장 등록 페이지로 이동",
    "dashboard.schedule": "스케줄 관리",
    "dashboard.scheduleDescription": "근무표 생성 및 관리",
    "dashboard.viewSchedule": "스케줄 보기",
    "dashboard.createSchedule": "스케줄 생성",
    "dashboard.shiftRequests": "교대 요청",
    "dashboard.shiftRequestsDescription": "교대 요청 관리",
    "dashboard.viewRequests": "요청 보기",
    "dashboard.approveRequests": "요청 승인",
    "dashboard.userManagement": "사용자 관리",
    "dashboard.userManagementDescription": "팀원 초대 및 권한 관리",
    "dashboard.viewUsers": "사용자 보기",
    "dashboard.inviteUsers": "사용자 초대",
    "dashboard.storeManagement": "매장 관리",
    "dashboard.storeManagementDescription": "매장 정보 관리 및 설정",
    "dashboard.viewStores": "매장 보기",
    "dashboard.createStore": "매장 생성",
    "dashboard.settings": "설정",
    "dashboard.settingsDescription": "계정 및 매장 설정",
    "dashboard.profile": "프로필",
    "dashboard.storeSettings": "매장 설정",
    "dashboard.analytics": "분석",
    "dashboard.analyticsDescription": "매장 운영 분석",
    "dashboard.viewAnalytics": "분석 보기",
    "dashboard.systemAdmin": "시스템 관리자",
    "analytics.title": "근무 시간 분석",
    "analytics.weeklyTotalHours": "주간 총 근무 시간",
    "analytics.monthlyTotalHours": "월간 총 근무 시간",
    "analytics.weeklyByUser": "사용자별 주간 근무 시간",
    "analytics.monthlyByUser": "사용자별 월간 근무 시간",
    "analytics.user": "사용자",
    "analytics.hours": "근무 시간",
    "analytics.noData": "표시할 근무 데이터가 없습니다",
    "analytics.loadError": "분석 데이터를 불러오지 못했습니다",
    "analytics.selectWeek": "주 선택",
    "analytics.selectMonth": "월 선택",
    "analytics.chooseWeek": "분석할 주 선택",
    "analytics.chooseMonth": "분석할 월 선택",
    "dashboard.quickActions": "빠른 작업",
    "dashboard.quickCreateSchedule": "스케줄 생성",
    "dashboard.quickInviteUser": "사용자 초대",
    "dashboard.quickApproveShifts": "교대 승인",
    "dashboard.myInfo": "내 정보",
    "dashboard.scheduleManagement": "스케줄 관리",
    "dashboard.chat": "채팅",
    "dashboard.preparing": "준비 중",
    "dashboard.scheduleDesc": "곧 스케줄을 확인하고 관리할 수 있습니다.",
    "dashboard.chatDesc": "팀원들과 실시간으로 소통하세요.",
    "dashboard.thisWeekSchedule": "이번 주 스케줄",
    "dashboard.currentWeekWork": "현재 주의 근무 일정",
    "dashboard.totalHours": "총 근무 시간",
    "dashboard.pendingRequests": "대기 중인 요청",
    "dashboard.shiftSwapRequests": "승인 대기 중인 교대 요청",
    "dashboard.needAttention": "처리 필요",
    "dashboard.activeStores": "활성 매장",
    "dashboard.operatingStores": "현재 운영 중인 매장 수",
    "dashboard.operatingNormally": "정상 운영",
    "dashboard.reviewRequests": "교대 요청 확인",
    "dashboard.teamChat": "팀 채팅",
    "admin.title": "시스템 관리자 대시보드",
    "admin.description": "플랫폼 전체 운영 현황과 이상징후를 통합 관리합니다.",
    "admin.console.badge": "System Console",
    "admin.console.title": "운영 관리자 콘솔",
    "admin.console.description":
      "시스템 상태, 이상징후, 감사 활동을 하나의 운영 콘솔에서 추적합니다.",
    "admin.console.account": "Admin Account",
    "admin.console.surfaceLabel": "Admin Surface",
    "admin.console.kicker": "Platform operations and audit mode",
    "admin.loadError": "관리자 데이터를 불러오지 못했습니다.",
    "admin.login.badge": "관리자 전용 접근",
    "admin.login.title": "시스템 운영 계정으로 로그인하세요",
    "admin.login.description":
      "운영 현황, 감사 로그, 이상징후 대응은 관리자 권한 계정에서만 접근할 수 있습니다.",
    "admin.login.loginCard": "관리자 로그인",
    "admin.login.cardDescription":
      "승인된 플랫폼 관리자 계정으로 로그인해 주세요.",
    "admin.login.helper":
      "일반 매장 계정이라면 일반 로그인 페이지를 이용하면 됩니다.",
    "admin.login.goToUserLogin": "일반 로그인으로 이동",
    "admin.login.accessDenied":
      "이 계정은 시스템 관리자 권한이 없어 관리자 콘솔에 로그인할 수 없습니다.",
    "admin.login.panelOneLabel": "Access",
    "admin.login.panelOneValue": "플랫폼 운영, 감사, 이상징후 대응",
    "admin.login.panelTwoLabel": "Security",
    "admin.login.panelTwoValue":
      "권한이 없는 계정은 관리자 콘솔에 접근할 수 없습니다.",
    "admin.readOnly": "읽기 전용",
    "admin.period": "기간",
    "admin.period.today": "오늘",
    "admin.period.7d": "최근 7일",
    "admin.period.30d": "최근 30일",
    "admin.period.from": "시작일",
    "admin.period.to": "종료일",
    "admin.searchPlaceholder": "사용자, 매장, 이벤트 검색",
    "admin.exportCurrent": "현재 탭 CSV 내보내기",
    "admin.filters.apply": "적용",
    "admin.filters.reset": "초기화",
    "admin.tab.overview": "개요",
    "admin.tab.users": "사용자",
    "admin.tab.stores": "매장",
    "admin.tab.anomalies": "이상징후",
    "admin.tab.logs": "감사로그",
    "admin.overview.description":
      "플랫폼 핵심 지표와 운영 흐름을 빠르게 파악할 수 있는 요약 화면입니다.",
    "admin.overview.trendsDescription":
      "선택한 기간 동안의 주요 생성 활동과 운영 변화량입니다.",
    "admin.kpi.totalUsers": "총 사용자",
    "admin.kpi.activeUsers": "활성 사용자",
    "admin.kpi.activeStores": "활성 매장",
    "admin.kpi.coverage": "배정 커버리지",
    "admin.trends.title": "일자별 추세",
    "admin.trends.date": "일자",
    "admin.trends.users": "신규 사용자",
    "admin.trends.stores": "신규 매장",
    "admin.trends.invitations": "초대 생성",
    "admin.trends.assignments": "배정 생성",
    "admin.users.title": "사용자 현황",
    "admin.users.description":
      "플랫폼 사용자, 관리자 권한 분포, 최근 활동을 한 곳에서 확인합니다.",
    "admin.users.summaryDescription":
      "검색 조건에 맞는 사용자 목록과 플랫폼 권한 상태입니다.",
    "admin.users.inactiveSummary": "비활성 사용자",
    "admin.users.email": "이메일",
    "admin.users.name": "이름",
    "admin.users.platformRole": "플랫폼 권한",
    "admin.users.status": "상태",
    "admin.users.lastSignIn": "최근 로그인",
    "admin.users.storeRoles": "매장 역할",
    "admin.users.activeStoreRoles": "활성",
    "admin.users.inactiveStoreRoles": "비활성",
    "admin.users.allStatuses": "모든 상태",
    "admin.users.allRoles": "모든 권한",
    "admin.users.emptyDescription":
      "현재 조건에 맞는 사용자가 없어 사용자 현황이 비어 있습니다.",
    "admin.stores.title": "매장 현황",
    "admin.stores.description":
      "활성/보관 매장과 운영 위험 징후를 관리자 콘솔 기준으로 조회합니다.",
    "admin.stores.summaryDescription":
      "매장 운영 밀도와 최근 30일 활동량을 함께 확인합니다.",
    "admin.stores.name": "매장명",
    "admin.stores.status": "상태",
    "admin.stores.owner": "소유자",
    "admin.stores.members": "구성원 수",
    "admin.stores.managers": "관리자",
    "admin.stores.pendingInvitations": "대기 초대",
    "admin.stores.assignments30d": "최근 30일 배정",
    "admin.stores.allStatuses": "모든 상태",
    "admin.stores.emptyDescription":
      "현재 조건에 맞는 매장이 없어 운영 현황이 비어 있습니다.",
    "admin.stores.risk": "위험도",
    "admin.anomalies.title": "이상징후",
    "admin.anomalies.description":
      "운영 이상 탐지 규칙에 의해 생성된 항목을 확인하고 조치합니다.",
    "admin.anomalies.summaryDescription":
      "심각도와 상태 기준으로 이상징후를 정렬해 우선 대응 대상을 찾습니다.",
    "admin.anomalies.rule": "규칙",
    "admin.anomalies.severity": "심각도",
    "admin.anomalies.status": "상태",
    "admin.anomalies.metric": "지표/기준",
    "admin.anomalies.detectedAt": "탐지 시각",
    "admin.anomalies.actions": "조치",
    "admin.anomalies.ack": "확인",
    "admin.anomalies.resolve": "해결",
    "admin.anomalies.allStatuses": "모든 상태",
    "admin.anomalies.allSeverities": "모든 심각도",
    "admin.anomalies.emptyDescription":
      "현재 조건에서는 표시할 이상징후가 없습니다.",
    "admin.anomalies.updateError": "이상징후 상태를 변경하지 못했습니다.",
    "admin.logs.title": "감사 로그",
    "admin.logs.description":
      "플랫폼 및 매장 단위의 관리자 활동과 시스템 이벤트를 검색합니다.",
    "admin.logs.summaryDescription":
      "심각도와 이벤트 유형 기준으로 감사 로그를 빠르게 추적합니다.",
    "admin.logs.scope": "범위",
    "admin.logs.eventType": "이벤트",
    "admin.logs.actor": "실행자",
    "admin.logs.severity": "심각도",
    "admin.logs.createdAt": "생성 시각",
    "admin.logs.allSeverities": "모든 심각도",
    "admin.logs.eventTypePlaceholder": "이벤트 타입 입력",
    "admin.logs.emptyDescription":
      "현재 조건에 맞는 감사 로그가 없습니다.",
    "admin.pagination.prev": "이전",
    "admin.pagination.next": "다음",
    "admin.empty.title": "표시할 데이터가 없습니다",
    "admin.empty.description":
      "필터를 조정하거나 기간 범위를 넓혀 다시 확인해 주세요.",
    "admin.role.none": "없음",
    "admin.status.active": "활성",
    "admin.status.inactive": "비활성",
    "admin.status.archived": "보관됨",
    "admin.risk.high": "높음",
    "admin.risk.medium": "보통",
    "admin.risk.low": "낮음",
    "admin.severity.high": "높음",
    "admin.severity.medium": "보통",
    "admin.severity.low": "낮음",
    "admin.anomalyStatus.open": "오픈",
    "admin.anomalyStatus.ack": "확인",
    "admin.anomalyStatus.resolved": "해결",
    "admin.logs.scope.platform": "플랫폼",
    "admin.logs.scope.store": "매장",
    "admin.anomaly.inviteFailure.title": "초대 실패 급증",
    "admin.anomaly.inviteFailure.description":
      "이전 기간 대비 초대 실패/만료 건수가 급증했습니다.",
    "admin.anomaly.deactivation.title": "사용자 비활성 급증",
    "admin.anomaly.deactivation.description":
      "이전 기간 대비 사용자 비활성 처리 건수가 급증했습니다.",
    "admin.anomaly.assignmentGap.title": "스케줄 결손 급증",
    "admin.anomaly.assignmentGap.description":
      "취소된 배정이 증가해 근무 커버리지 저하 위험이 있습니다.",
    "admin.anomaly.roleChange.title": "권한 변경 급증",
    "admin.anomaly.roleChange.description":
      "권한 부여/회수 액션이 이전 기간 대비 급증했습니다.",
    // 초대 관련
    "invites.form.title": "팀원 초대",
    "invites.form.send": "초대 보내기",
    "invites.form.sending": "초대 생성 중...",
    "invites.form.storeRequired": "매장을 선택해주세요",
    "invites.form.invitedDescription": "{email}에게 초대가 발송되었습니다",
    // 직무 역할 관련
    "jobRoles.title": "직무 관리",
    "jobRoles.description": "매장에서 필요한 직무를 관리합니다",
    "jobRoles.create": "직무 생성",
    "jobRoles.edit": "직무 수정",
    "jobRoles.delete": "직무 삭제",
    "jobRoles.name": "직무명",
    "jobRoles.code": "코드",
    "jobRoles.roleDescription": "설명",
    "jobRoles.active": "활성",
    "jobRoles.inactive": "비활성",
    "jobRoles.usageCount": "사용 중인 인원: {count}명",
    "jobRoles.confirmDelete": "이 직무를 삭제하시겠습니까?",
    "jobRoles.deleteError": "사용 중인 직무는 삭제할 수 없습니다",
    "jobRoles.createSuccess": "직무가 생성되었습니다",
    "jobRoles.updateSuccess": "직무가 수정되었습니다",
    "jobRoles.deleteSuccess": "직무가 삭제되었습니다",
    "jobRoles.nameRequired": "직무명은 필수입니다",
    "jobRoles.nameDuplicate": "이미 존재하는 직무명입니다",
    "jobRoles.nameMaxLength": "직무명은 48자 이하여야 합니다",
    "jobRoles.codeMaxLength": "코드는 32자 이하여야 합니다",
    "jobRoles.descriptionMaxLength": "설명은 160자 이하여야 합니다",
    "jobRoles.namePlaceholder": "직무명을 입력하세요",
    "jobRoles.descriptionPlaceholder": "직무에 대한 설명을 입력하세요",
    "jobRoles.list": "직무 목록",
    "jobRoles.listDescription": "현재 매장에서 사용 중인 직무들입니다",
    "jobRoles.createdAt": "생성일",
    "jobRoles.noRoles": "등록된 직무가 없습니다",
    // 유저 직무 역할 관련
    "userJobRoles.title": "직무 역할 설정",
    "userJobRoles.description": "사용자의 직무 역할을 설정합니다",
    "userJobRoles.selectRoles": "역할 선택",
    "userJobRoles.saveSuccess": "역할이 저장되었습니다",
    "userJobRoles.minOneRole": "최소 1개의 역할을 선택해야 합니다",
    // 근무 항목 역할 요구 관련
    "workItemRoles.title": "필요 직무 설정",
    "workItemRoles.description": "이 근무 항목에 필요한 직무를 설정합니다",
    "workItemRoles.addRole": "직무 추가",
    "workItemRoles.removeRole": "직무 제거",
    "workItemRoles.minCount": "최소 인원",
    "workItemRoles.saveSuccess": "직무 요구 사항이 저장되었습니다",
    "workItemRoles.saveFailed": "직무 요구 사항 저장에 실패했습니다",
    "workItemRoles.minCountMin": "최소 인원은 0 이상이어야 합니다",
    "workItemRoles.minCountMax": "최소 인원은 99 이하여야 합니다",
    "workItemRoles.requirementsSet": "요구 사항 설정됨",
    "workItemRoles.currentCount": "현재 {current}명",
    "workItemRoles.requiredCount": "필요 {required}명",
    "workItemRoles.roleCoverage": "직무 커버리지",
    "workItemRoles.insufficient": "직무 인원 부족",
    "workItemRoles.sufficient": "직무 인원 충족",
    "invites.form.store": "매장 선택",
    "invites.form.email": "이메일 주소",
    "invites.form.role": "직무",
    "invites.list.title": "초대 목록",
    "invites.status.pending": "대기중",
    "invites.status.accepted": "수락됨",
    "invites.status.cancelled": "취소됨",
    "invites.status.expired": "만료됨",
    "invites.role.master": "마스터 관리자",
    "invites.role.subManager": "서브 관리자",
    "invites.role.partTimer": "파트타이머",
    // 직무 관리 관련
    "role.change.success": "직무가 성공적으로 변경되었습니다",
    "role.change.description": "사용자의 직무가 업데이트되었습니다",
    "role.change.error": "직무 변경 실패",
    "role.change.errorDescription": "직무 변경 중 오류가 발생했습니다",
    "role.management.title": "직무 관리",
    "role.management.description": "사용자의 직무를 변경할 수 있습니다",
    "role.management.userInfo": "사용자 정보",
    "role.management.currentRole": "현재 직무",
    "role.management.newRole": "새 직무",
    "role.management.selectRole": "직무를 선택하세요",
    "role.management.current": "현재",
    "role.management.noPermission": "직무를 변경할 권한이 없습니다",
    "role.management.changing": "변경 중...",
    "role.management.changeRole": "직무 변경",
    "role.management.userList": "사용자 목록",
    "role.changePermissionDenied": "역할 변경 권한이 없습니다",
    "role.grantPermissionDenied": "역할 부여 권한이 없습니다",
    "role.revokePermissionDenied": "역할 회수 권한이 없습니다",
    // 매장 관련
    "store.noStores": "매장 없음",
    "store.loading": "매장 로딩 중...",
    "store.selectStore": "매장 선택",
    "store.noStoreSelected": "선택된 매장 없음",
    "store.totalStores": "총 {count}개 매장",
    "store.role.master": "마스터",
    "store.role.sub_manager": "서브 매니저",
    "store.role.part_timer": "파트타이머",
    "store.createFirst": "첫 번째 매장 생성",
    "store.name": "매장명",
    "store.namePlaceholder": "매장명을 입력하세요",
    "store.description": "설명",
    "store.descriptionPlaceholder": "매장에 대한 설명을 입력하세요",
    "store.address": "주소",
    "store.addressPlaceholder": "매장 주소를 입력하세요",
    "store.phone": "전화번호",
    "store.phonePlaceholder": "매장 전화번호를 입력하세요",
    "store.timezone": "타임존",
    "store.timezonePlaceholder": "타임존을 선택하세요",
    "store.createTitle": "매장 생성",
    "store.createDescription": "새로운 매장을 생성합니다",
    "store.editTitle": "매장 수정",
    "store.edit": "매장 수정",
    "store.editDescription": "매장 정보를 수정합니다",
    "store.createButton": "매장 생성",
    "store.updateButton": "매장 수정",
    "store.createSuccess": "매장 생성 완료",
    "store.createError": "매장 생성 실패",
    "store.updateSuccess": "매장 수정 완료",
    "store.updateError": "매장 수정 실패",
    "store.saveError": "저장 실패",
    "store.networkError": "네트워크 오류가 발생했습니다",
    "common.email": "이메일",
    "common.name": "이름",
    "common.cancel": "취소",
    "common.save": "저장",
    "common.edit": "수정",
    "common.delete": "삭제",
    "common.confirm": "확인",
    "common.loading": "로딩 중...",
    "common.yes": "예",
    "common.no": "아니오",
    "common.update": "수정",
    "common.create": "생성",
    "common.add": "추가",
    "common.info": "정보",
    "common.error": "오류",
    "common.unknownError": "알 수 없는 오류가 발생했습니다",
    "common.success": "성공",
    "common.back": "뒤로",
    "common.view": "보기",
    "common.close": "닫기",
    "common.noData": "데이터가 없습니다",
    "store.noDescription": "설명 없음",
    "store.loadError": "매장 정보를 불러오는데 실패했습니다",
    "store.notFound": "매장을 찾을 수 없습니다",
    "store.notFoundDescription":
      "요청하신 매장이 존재하지 않거나 접근 권한이 없습니다",
    "store.information": "매장 정보",
    "store.informationDescription": "매장의 기본 정보를 확인할 수 있습니다",
    "store.status": "상태",
    "store.status.active": "활성",
    "store.status.archived": "보관됨",
    "store.createdInfo": "생성 정보",
    "store.createdAt": "생성일",
    "store.updatedAt": "수정일",
    "store.quickActions": "빠른 작업",
    "store.manageUsers": "사용자 관리",
    "store.viewSchedule": "스케줄 보기",
    "store.settings": "설정",
    "store.noPermission": "매장을 수정할 권한이 없습니다",
    // 사용자 관리 관련 번역
    "user.management": "사용자 관리",
    "user.managementForStore": "{storeName} 매장 사용자 관리",
    "user.members": "구성원",
    "user.total": "명",
    "user.name": "이름",
    "user.email": "이메일",
    "user.role": "역할",
    "user.status": "상태",
    "user.joinedAt": "가입일",
    "user.temporaryWork": "임시 근무",
    "user.actions": "작업",
    "user.noName": "이름 없음",
    "user.defaultStore": "기본 매장",
    "user.noMembers": "구성원이 없습니다",
    "user.filters": "필터",
    "user.search": "검색",
    "user.searchPlaceholder": "이름 또는 이메일로 검색",
    "user.allRoles": "모든 역할",
    "user.allStatuses": "모든 상태",
    "user.clearFilters": "필터 초기화",
    "user.invite": "초대",
    "user.temporaryAssign": "임시 근무 배치",
    "user.startDate": "시작일",
    "user.endDate": "종료일",
    "user.reason": "사유",
    "user.reasonPlaceholder": "임시 근무 배치 사유를 입력하세요",
    "user.assign": "배치",
    "user.roleGranted": "역할 부여 완료",
    "user.roleGrantedDescription":
      "사용자에게 역할이 성공적으로 부여되었습니다",
    "user.roleGrantError": "역할 부여 실패",
    "user.roleRevoked": "역할 회수 완료",
    "user.roleRevokedDescription": "사용자의 역할이 성공적으로 회수되었습니다",
    "user.roleRevokeError": "역할 회수 실패",
    "user.demoteSubManager": "서브 매니저 전환",
    "user.demoteSubManagerDescription": "서브 매니저를 파트타이머로 전환합니다",
    "user.demoteSubManagerError": "서브 매니저 전환 실패",
    "user.deactivateUser": "사용자 비활성화",
    "user.deactivateUserDescription": "사용자를 비활성화합니다",
    "user.deactivateUserError": "사용자 비활성화 실패",
    "user.reactivateUser": "사용자 재활성화",
    "user.reactivateUserDescription": "사용자를 재활성화합니다",
    "user.reactivateUserError": "사용자 재활성화 실패",
    "user.deleteUser": "사용자 삭제",
    "user.deleteUserDescription":
      "사용자를 삭제합니다 (스케줄 이력은 보존됩니다)",
    "user.deleteUserError": "사용자 삭제 실패",
    "user.notFound": "사용자를 찾을 수 없습니다",
    "user.alreadyInactive": "이미 비활성화된 사용자입니다",
    "user.alreadyActive": "이미 활성화된 사용자입니다",
    "user.deletePermissionDenied": "사용자 삭제 권한이 없습니다",
    "user.deactivatePermissionDenied": "사용자 비활성화 권한이 없습니다",
    "user.reactivatePermissionDenied": "사용자 재활성화 권한이 없습니다",
    "user.cannotDeleteSelf": "자기 자신을 삭제할 수 없습니다",
    "user.cannotDeleteMaster": "마스터 권한을 가진 사용자는 삭제할 수 없습니다",
    "user.roleNotFound": "사용자의 역할을 찾을 수 없습니다",
    "user.onlySubManagerCanDemote":
      "서브 매니저만 파트타이머로 전환할 수 있습니다",
    "user.onlyActiveSubManagerCanDemote":
      "활성 상태의 서브 매니저만 전환할 수 있습니다",
    "user.deleteSuccessWithHistory":
      "사용자가 성공적으로 삭제되었습니다. 스케줄 이력은 보존됩니다.",
    "user.alreadyDeleted": "이미 삭제된 사용자",
    "user.alreadyDeletedDescription": "이 사용자는 이미 삭제되었습니다.",
    "user.confirmDemote": "서브 매니저 전환 확인",
    "user.confirmDemoteMessage":
      "정말로 이 서브 매니저를 파트타이머로 전환하시겠습니까?",
    "user.promoteToSubManager": "승격",
    "user.confirmPromoteMessage": "이 사용자를 서브 매니저로 승격하시겠습니까?",
    "user.adminActions": "관리자 액션",
    "user.guestUser": "게스트 사용자",
    "user.confirmDeactivate": "사용자 비활성화 확인",
    "user.confirmDeactivateMessage": "정말로 이 사용자를 비활성화하시겠습니까?",
    "user.confirmDelete": "사용자 삭제 확인",
    "user.confirmDeleteMessage":
      "정말로 이 사용자를 삭제하시겠습니까? 스케줄 이력은 보존됩니다.",
    "user.temporaryAssigned": "임시 근무 배치 완료",
    // 초대 관련 번역
    "invite.title": "초대 관리",
    "invite.description": "매장에 새로운 구성원을 초대합니다",
    "invite.createInvite": "초대 생성",
    "invite.email": "이메일",
    "invite.emailPlaceholder": "초대할 사용자의 이메일을 입력하세요",
    "invite.storePlaceholder": "매장을 선택하세요",
    "invite.role": "역할",
    "invite.rolePlaceholder": "부여할 역할을 선택하세요",
    "invite.roleDescription.subManager":
      "스케줄 관리, 교대 승인, 파트타이머 초대 권한",
    "invite.roleDescription.partTimer": "스케줄 확인, 교대 요청 권한",
    "invite.message": "메시지 (선택사항)",
    "invite.messagePlaceholder": "초대 메시지를 입력하세요",
    "invite.send": "초대 보내기",
    "invite.sending": "전송 중...",
    "invite.success": "초대 전송 완료",
    "invite.successDescription": "초대가 성공적으로 전송되었습니다",
    "invite.error": "초대 전송 실패",
    "invite.pending": "대기중",
    "invite.accepted": "수락됨",
    "invite.expired": "만료됨",
    "invite.cancelled": "취소됨",
    "invite.resend": "재전송",
    "invite.cancel": "취소",
    "invite.noInvites": "초대가 없습니다",
    "invite.expiresAt": "만료일",
    "invite.invitedAt": "초대일",
    "invite.status": "상태",
    "invite.actions": "작업",
    "invite.create": "초대 생성",
    "invite.name": "이름",
    "invite.partTimer": "파트타이머",
    "invite.subManager": "서브 매니저",
    "invite.days": "일",
    "invite.createSuccess": "초대가 성공적으로 생성되었습니다",
    "invite.createError": "초대 생성 실패",
    "invite.loadError": "초대 목록 로드 실패",
    "invite.linkCopied": "링크 복사됨",
    "invite.linkCopiedDescription": "초대 링크가 클립보드에 복사되었습니다",
    "invite.copyError": "링크 복사에 실패했습니다",
    "invite.empty": "아직 초대가 없습니다",
    "invite.emptyDescription": "새 팀원을 초대해보세요",
    "invite.accessDenied": "매장에 대한 접근 권한이 없습니다",
    "invite.managePermissionDenied": "해당 매장의 관리 권한이 없습니다",
    "invite.cancelUsed": "이미 사용된 초대는 취소할 수 없습니다",
    "invite.alreadyCancelled": "이미 취소된 초대입니다",
    "invite.duplicateActiveInvite": "이미 해당 이메일로 유효한 초대가 존재합니다",
    "invite.tokenCreateError": "초대 토큰 생성에 실패했습니다",
    "invite.storeIdRequired": "매장 ID가 필요합니다",
    "invite.listLoadError": "초대 목록 조회에 실패했습니다",
    "invite.resendSuccess": "초대가 재전송되었습니다",
    "invite.resendError": "초대 재전송 실패",
    "invite.cancelSuccess": "초대가 취소되었습니다",
    "invite.cancelError": "초대 취소 실패",
    "invite.cancelPermissionDenied": "초대 취소 권한이 없습니다",
    "invite.resendPermissionDenied": "초대 재발송 권한이 없습니다",
    "invite.userDeleteFailed": "기존 사용자 삭제에 실패했습니다",
    "invite.userDeleteRetryFailed":
      "사용자 삭제에 실패했습니다. 잠시 후 다시 시도해주세요.",
    "invite.emailSendFailed": "이메일 발송에 실패했습니다",
    "invite.alreadyRegisteredRetry":
      "이미 등록된 이메일입니다. 잠시 후 다시 시도해주세요.",
    "invite.resendEmailError": "이메일 재발송 중 오류가 발생했습니다",
    "invite.emailRequired": "이메일을 입력해주세요",
    "invite.namePlaceholder": "홍길동",
    "invite.guestNoEmailDescription":
      "게스트 사용자는 이메일 없이 등록됩니다.",
    "invite.expiresIn": "초대 링크 만료",
    "invite.list": "초대 목록",
    "invite.registerWithoutEmail": "이메일 없이 등록 (게스트 사용자)",
    "invite.refresh": "새로고침",
    "invite.mode.email": "이메일 초대",
    "invite.mode.guest": "게스트 등록",
    "invite.mode.import": "다른 지점 유저",
    "invite.importSourceStore": "가져올 지점",
    "invite.importCandidates": "가져오기 대상 유저",
    "invite.importSourceRole": "기존 지점 역할",
    "invite.importEmpty": "가져올 수 있는 유저가 없습니다",
    "invite.importLoadError": "다른 지점 유저 목록을 불러오지 못했습니다",
    "invite.importSelectionRequired": "가져올 유저를 선택해주세요",
    "invite.importSubmit": "현재 지점에 등록",
    "invite.importSuccess": "다른 지점 유저 등록이 완료되었습니다",
    "invite.importError": "다른 지점 유저 등록에 실패했습니다",
    "invite.importNoSourceStores": "가져올 수 있는 다른 관리 지점이 없습니다",
    "invite.importResultImported": "등록",
    "invite.importResultAlreadyExists": "이미 등록됨",
    "invite.importResultSkipped": "건너뜀",
    "user.temporaryAssignedDescription":
      "임시 근무가 성공적으로 배치되었습니다",
    "user.temporaryAssignError": "임시 근무 배치 실패",
    // 사용자 상세 페이지 관련 번역
    "user.detail": "사용자 상세",
    "user.detailForStore": "{storeName} 매장 - {userName} 상세",
    "user.basicInfo": "기본 정보",
    "user.jobRoles": "직무 역할",
    "user.jobRoles.none": "설정된 직무 역할이 없습니다.",
    "user.workPreferences": "근무 선호도",
    "user.resignationDate": "퇴사 예정일",
    "user.desiredWeeklyHours": "희망 주 근무 시간",
    "user.desiredWeeklyHoursPlaceholder":
      "주당 근무 시간을 입력하세요 (예: 20)",
    "user.desiredDailyHours": "희망 일 근무 시간",
    "user.desiredDailyHoursPlaceholder":
      "하루에 일하고 싶은 시간을 입력하세요 (예: 6.5)",
    "user.noResignationDate": "퇴사 예정일 없음",
    "user.setResignationDate": "퇴사 예정일 설정",
    "user.removeResignationDate": "퇴사 예정일 제거",
    "user.updateProfile": "프로필 업데이트",
    "user.profileUpdated": "프로필 업데이트 완료",
    "user.profileUpdatedDescription":
      "사용자 프로필이 성공적으로 업데이트되었습니다",
    "user.profileUpdateError": "프로필 업데이트 실패",
    "user.viewDetail": "상세보기",
    "user.backToUserList": "사용자 목록으로 돌아가기",
    "user.guest": "게스트",
    "user.normal": "일반",
    "user.deactivate": "비활성화",
    "user.reactivate": "재활성화",
    "user.difficultWeekday.updateError":
      "출근 불가 요일 업데이트에 실패했습니다",
    "user.difficultWeekday.deleteError":
      "출근 불가 요일 삭제에 실패했습니다",
    "user.difficultWeekday.insertError":
      "출근 불가 요일 추가에 실패했습니다",
    // 프로필 페이지 관련 번역
    "profile.title": "프로필",
    "profile.myProfile": "내 프로필",
    "profile.personalInfo": "개인 정보",
    "profile.accountInfo": "계정 정보",
    "profile.changePassword": "비밀번호 변경",
    "profile.currentPassword": "현재 비밀번호",
    "profile.newPassword": "새 비밀번호",
    "profile.confirmPassword": "비밀번호 확인",
    "profile.passwordChanged": "비밀번호 변경 완료",
    "profile.passwordChangedDescription":
      "비밀번호가 성공적으로 변경되었습니다",
    "profile.passwordChangeError": "비밀번호 변경 실패",
    "profile.currentPasswordRequired": "현재 비밀번호를 입력해주세요",
    "profile.newPasswordRequired": "새 비밀번호를 입력해주세요",
    "profile.passwordMismatch": "비밀번호가 일치하지 않습니다",
    "profile.passwordTooShort": "비밀번호는 최소 6자 이상이어야 합니다",
    "profile.saveChanges": "변경사항 저장",
    "profile.cancel": "취소",
    "profile.daysRemaining": "남은 일수",
    "profile.resignationPassed": "퇴사일이 지났습니다",
    "profile.resignationToday": "오늘 퇴사일입니다",
    "profile.resignationTomorrow": "내일 퇴사일입니다",
    "profile.daysLeft": "일 남음",
    "user.status.active": "활성",
    "user.status.pending": "대기중",
    "user.status.inactive": "비활성",
    // 초대 수락 페이지
    "invite.accept.title": "초대 수락",
    "invite.accept.description": "매장에 참여하기 위해 계정을 생성해주세요",
    "invite.accept.storeInfo": "매장 정보",
    "invite.accept.storeName": "매장명",
    "invite.accept.role": "역할",
    "invite.accept.expiresAt": "만료일",
    "invite.accept.accountInfo": "계정 정보",
    "invite.accept.name": "이름",
    "invite.accept.namePlaceholder": "이름을 입력하세요",
    "invite.accept.password": "비밀번호",
    "invite.accept.passwordPlaceholder": "비밀번호를 입력하세요",
    "invite.accept.confirmPassword": "비밀번호 확인",
    "invite.accept.confirmPasswordPlaceholder": "비밀번호를 다시 입력하세요",
    "invite.accept.accept": "초대 수락",
    "invite.accept.accepting": "수락 중...",
    "invite.accept.success": "초대 수락 완료",
    "invite.accept.successDescription": "매장에 성공적으로 참여했습니다",
    "invite.accept.error": "초대 수락 실패",
    "invite.accept.invalidToken": "유효하지 않은 초대",
    "invite.accept.expiredToken": "만료된 초대",
    "invite.accept.loading": "초대 정보를 불러오는 중...",
    "invite.accept.existingUser": "기존 사용자",
    "invite.accept.existingUserMessage":
      "이미 등록된 이메일입니다. 로그인 후 초대를 수락해주세요.",
    "invite.accept.loginToAccept": "로그인하여 초대 수락",
    "invite.accept.emailMismatch":
      "초대된 이메일과 현재 로그인한 이메일이 일치하지 않습니다",
    "invite.accept.alreadyMember": "이미 해당 매장의 구성원입니다",
    "invite.accept.processingError": "초대 수락 처리 중 오류가 발생했습니다",
    "invite.accept.tokenRequired": "초대 토큰이 필요합니다",
    "invite.accept.notFound": "초대를 찾을 수 없습니다",
    "invite.accept.status.valid": "유효한 초대입니다",
    "invite.accept.status.cancelled": "취소된 초대입니다",
    "invite.accept.status.used": "이미 사용된 초대입니다",
    "invite.accept.status.expired": "만료된 초대입니다",
    "invite.accept.joinedRoleMessage":
      "{storeName}에 {roleName}로 성공적으로 합류했습니다",
    "invite.accept.signInFailed":
      "로그인 실패: {details}. 기본 패스워드(1q2w3e4r!)로 시도해보세요.",
    "invite.accept.userCreateFailed": "사용자 생성에 실패했습니다",
    "invite.accept.storeAddFailed": "매장 추가에 실패했습니다",
    "invite.setupPassword.title": "패스워드 설정",
    "invite.setupPassword.description":
      "{storeName} 매장 초대를 수락하기 위해 패스워드를 설정해주세요.",
    "invite.setupPassword.unknownStore": "알 수 없는 매장",
    "invite.setupPassword.newPassword": "새 패스워드",
    "invite.setupPassword.passwordPlaceholder": "최소 8자 이상",
    "invite.setupPassword.passwordPlaceholderMin6": "최소 6자 이상",
    "invite.setupPassword.submit": "패스워드 설정 및 초대 수락",
    "invite.setupPassword.successTitle": "패스워드 설정 완료",
    "invite.setupPassword.failureTitle": "패스워드 설정에 실패했습니다",
    "invite.setupPassword.loginRequiredTitle": "로그인이 필요합니다",
    "invite.setupPassword.loginRequired": "초대 링크를 통해 로그인해주세요.",
    "invite.setupPassword.nameRequiredTitle": "이름을 입력해주세요",
    "invite.setupPassword.nameRequiredDescription": "이름은 필수 입력 항목입니다.",
    "invite.setupPassword.passwordMismatchTitle": "패스워드가 일치하지 않습니다",
    "invite.setupPassword.passwordMismatchDescription":
      "패스워드를 다시 확인해주세요.",
    "invite.setupPassword.passwordTooShortTitle": "패스워드가 너무 짧습니다",
    "invite.setupPassword.passwordTooShortDescription":
      "최소 8자 이상 입력해주세요.",
    "invite.setupPassword.passwordTooShortMin6Description":
      "최소 6자 이상 입력해주세요.",
    "invite.setupPassword.loadError": "정보를 불러오는 중 오류가 발생했습니다.",
    "invite.setupPassword.invitationLoadError":
      "초대 정보를 불러오는데 실패했습니다.",
    "invite.setupPassword.userLoadError":
      "사용자 정보를 확인하는 중 오류가 발생했습니다.",
    "invite.setupPassword.checkingUser": "사용자 정보를 확인하는 중...",
    "invite.error.title": "초대 링크 오류",
    "invite.error.processing":
      "초대 링크 처리 중 오류가 발생했습니다. 다시 시도해주세요.",
    "invite.error.requestNew": "새로운 초대 요청",
    "invite.error.requestNewDescription": "관리자에게 새로운 초대를 요청해주세요.",
    "invite.error.message.otpExpired": "이메일 링크가 만료되었습니다.",
    "invite.error.message.accessDenied": "접근이 거부되었습니다.",
    "invite.error.message.invalidInvite": "초대 링크에 문제가 있습니다.",
    "invite.error.description.default": "초대 링크를 다시 확인해주세요.",
    "invite.error.description.otpExpired":
      "초대 링크의 유효기간이 만료되었습니다. 새로운 초대를 요청해주세요.",
    "invite.error.description.accessDenied":
      "이 초대 링크에 대한 접근 권한이 없습니다. 관리자에게 문의해주세요.",
    "invite.error.description.invalidInvite":
      "초대 링크가 유효하지 않거나 손상되었습니다.",
    "invite.error.code": "오류 코드",
    "invite.error.type": "오류 타입",
    "invite.error.detail": "상세",
    "invite.error.goHome": "홈으로 돌아가기",
    "invite.verify.title": "이메일 인증",
    "invite.verify.description": "이메일 인증이 완료되었습니다.",
    "invite.verify.verifying": "인증 확인 중...",
    "invite.verify.completedTitle": "이메일 인증 완료",
    "invite.verify.processedTitle": "초대 처리 완료",
    "invite.verify.processedDescription":
      "이미 처리된 초대이거나 만료된 초대입니다.",
    "invite.verify.sessionNotFound": "세션 정보를 찾을 수 없습니다.",
    "invite.verify.userNotFound": "사용자 정보를 찾을 수 없습니다.",
    "invite.verify.checkError": "인증 확인 중 오류가 발생했습니다.",
    "invite.verify.passwordSetAndAccepted":
      "패스워드가 설정되고 초대가 수락되었습니다.",
    "invite.verify.passwordSetupPrompt":
      "안전한 패스워드를 설정하고 초대를 수락해주세요.",
    "invite.verify.linkVerifiedPrompt":
      "초대 링크가 성공적으로 확인되었습니다. 패스워드를 설정해주세요.",
    "invite.verify.infoTitle": "인증 정보",
    "invite.verify.safePasswordNote": "안전한 패스워드를 설정해주세요.",
    "invite.verify.setPassword": "패스워드 설정하기",
    "invite.verify.userInfoTitle": "사용자 정보",
    "invite.verify.newPasswordPlaceholder":
      "새 패스워드를 입력하세요 (최소 8자)",
    "invite.verify.error": "인증 오류",
    "invite.verify.errorDescription": "이메일 인증이 필요합니다.",
    "invite.verify.acceptTitle": "초대 수락",
    "invite.verify.acceptDescription":
      "매장 초대를 수락하고 서비스를 이용하세요.",
    "invite.verify.loginInfo": "로그인 정보",
    "invite.verify.defaultPassword": "기본 패스워드",
    "invite.verify.passwordChangeNote":
      "로그인 후 설정에서 패스워드를 변경하세요.",
    "invite.verify.acceptButton": "초대 수락하기",
    "invite.verify.accepting": "수락 중...",
    "invite.verify.success":
      "초대가 성공적으로 수락되었습니다. 로그인하여 서비스를 이용하세요.",
    "invite.verify.acceptError": "초대 수락에 실패했습니다.",
    "dashboard.passwordChangeRequired": "패스워드 변경이 필요합니다",
    "dashboard.passwordChangeDescription":
      "보안을 위해 기본 패스워드를 변경해주세요.",
    "dashboard.passwordChangeButton": "패스워드 변경하기",
    // settings.store (편집 탭)
    "settings.store.title": "매장 설정",
    "settings.store.basic": "기본 정보",
    "settings.store.hours": "영업시간·휴무",
    "settings.store.workItems": "근무 항목",
    "settings.store.roles": "직무 관리",
    "settings.store.targets": "커버리지",
    "settings.store.breakRules": "브레이크",
    "settings.store.policies": "운영 규정",
    "settings.store.notifications": "알림",
    "settings.store.saveSuccess": "저장 완료",
    "settings.store.saveError": "저장 실패",
    "settings.store.validationError": "검증 오류",
    "settings.store.open": "시작 시간",
    "settings.store.close": "종료 시간",
    "settings.store.holidays": "휴무일",
    "settings.store.date": "날짜",
    "settings.store.policiesDescription":
      "운영에 필요한 핵심 규정만 설정합니다. 복잡한 규칙은 제거하여 사용하기 쉽게 만들었습니다.",
    "settings.store.currencyUnitDesc":
      "인건비 예산 및 급여 계산에 사용되는 통화 단위",
    "settings.store.publishCutoffDesc": "스케줄 게시 전 최소 시간 (시간 단위)",
    "settings.store.swapLeadTimeDesc": "교대 요청 최소 리드타임 (시간 단위)",
    "settings.store.minRestDesc": "연속 근무 간 최소 휴식 시간",
    "settings.store.maxHoursPerDayDesc": "하루 최대 근무 시간 (0 = 제한 없음)",
    "settings.store.maxHoursPerWeekDesc": "주 최대 근무 시간 (0 = 제한 없음)",
    "settings.store.maxHoursPerMonthDesc":
      "월 최대 근무 시간 (기본값: 160시간)",
    "settings.store.maxConsecutiveDaysDesc":
      "연속 근무 최대 일수 (0 = 제한 없음)",
    "settings.store.weeklyBudgetDesc":
      "주간 인건비 예산 ({currency} 단위, 0 = 제한 없음)",
    // workItems 탭
    "workItems.title": "근무 항목",
    "workItems.name": "이름",
    "workItems.start": "시작(분)",
    "workItems.end": "종료(분)",
    "workItems.break": "무급 휴식(분)",
    "workItems.maxHeadcount": "최대 인원",
    "workItems.roleHint": "역할",
    "workItems.roles": "직무 관리",
    "workItems.create": "근무 추가",
    "workItems.description": "매장에서 필요한 근무 항목을 관리합니다",
    "workItems.namePlaceholder": "근무 항목명을 입력하세요",
    "workItems.timeSettings": "시간 설정",
    "workItems.startTime": "시작 시간",
    "workItems.endTime": "종료 시간",
    "workItems.nextDay": "다음날",
    "workItems.nextDayLabel": "다음날까지",
    "workItems.roleRequirements": "직무 요구 사항",
    "workItems.availableJobs": "사용 가능한 직무",
    "workItems.selectedJobs": "선택된 직무",
    "workItems.noRolesSelected": "선택된 직무가 없습니다",
    "workItems.list": "근무 항목 목록",
    "workItems.listDescription": "현재 매장에서 사용 중인 근무 항목들입니다",
    "workItems.noItems": "등록된 근무 항목이 없습니다",
    "workItems.editNotImplemented": "편집 기능은 향후 구현 예정입니다",
    "workItems.deleteNotImplemented": "삭제 기능은 향후 구현 예정입니다",
    "workItems.edit": "편집",
    "workItems.delete": "삭제",
    "workItems.editTitle": "근무항목 편집",
    "workItems.deleteConfirm": "이 근무항목을 삭제하시겠습니까?",
    "workItems.deleteConfirmDescription":
      "삭제된 근무항목은 복구할 수 없습니다.",
    "workItems.updateSuccess": "근무항목이 성공적으로 수정되었습니다",
    "workItems.deleteSuccess": "근무항목이 성공적으로 삭제되었습니다",
    // 운영 규정 (Policies)
    "policies.publish_cutoff_hours": "스케줄 발행 마감 시간",
    "policies.swap_lead_time_hours": "교대 요청 선행 시간",
    "policies.min_rest_hours_between_shifts": "근무 간 최소 휴식 시간",
    "policies.max_hours_per_day": "일일 최대 근무 시간",
    "policies.max_hours_per_week": "주간 최대 근무 시간",
    "policies.max_consecutive_days": "최대 연속 근무 일수",
    "policies.max_morning_staff": "오전 최대 근무 인원",
    "policies.max_morning_staff_desc":
      "오전 시간대 최대 근무 인원 수 (0 = 제한 없음)",
    "policies.min_morning_staff": "오전 최소 근무 인원",
    "policies.min_morning_staff_desc":
      "오전 시간대 최소 근무 인원 수 (0 = 제한 없음)",
    "policies.max_afternoon_staff": "오후 최대 근무 인원",
    "policies.max_afternoon_staff_desc":
      "오후 시간대 최대 근무 인원 수 (0 = 제한 없음)",
    "policies.min_afternoon_staff": "오후 최소 근무 인원",
    "policies.min_afternoon_staff_desc":
      "오후 시간대 최소 근무 인원 수 (0 = 제한 없음)",
    "policies.weekly_labor_budget_cents": "주간 인건비 예산 (원)",
    "policies.schedule_unit": "스케줄 단위",
    "policies.schedule_unit_week": "주 단위",
    "policies.schedule_unit_month": "월 단위",
    "policies.currency_unit": "통화 단위",
    "policies.currency_krw": "한국 원",
    "policies.currency_usd": "미국 달러",
    "policies.currency_jpy": "일본 엔",
    "policies.currency_eur": "유로",
    "policies.currency_cad": "캐나다 달러",
    "policies.currency_aud": "호주 달러",
    "policies.shift_boundary_time": "오전/오후 구분 시간",
    "policies.shift_boundary_time_desc":
      "스케줄 그리드에서 오전/오후를 구분하는 기준 시간입니다. 이 시간을 기준으로 오전/오후 인원수가 계산됩니다.",
    "policies.group.basicSettings": "기본 설정",
    "policies.group.basicSettingsDesc": "스케줄 단위, 통화 단위, 시간 구분 등 기본 설정",
    "policies.group.schedulePublishing": "스케줄 게시",
    "policies.group.schedulePublishingDesc": "스케줄 게시 관련 설정",
    "policies.group.workHoursLimits": "근무 시간 제한",
    "policies.group.workHoursLimitsDesc": "주간, 월간 근무 시간 및 연속 근무 일수 제한",
    "policies.group.staffManagement": "인원 관리",
    "policies.group.staffManagementDesc": "오전/오후 시간대별 최소/최대 근무 인원 설정",
    "policies.group.unusedSettings": "미사용 항목",
    "policies.group.unusedSettingsDesc": "현재 사용하지 않는 설정 항목들",
    "workItems.nameRequired": "근무 항목명을 입력해주세요",
    "workItems.endAfterStart": "종료 시간은 시작 시간보다 늦어야 합니다",
    "workItems.breakTooLong": "휴식 시간은 근무 시간보다 길 수 없습니다",
    "workItems.calculatedMaxHeadcount": "계산된 최대 인원",
    "workItems.autoCalculated": "직무별 합계로 자동 계산",
    "workItems.maxHeadcountLimit": "직무별 최소 인원 합계는 99를 초과할 수 없습니다",
    "workItems.endTimeLimit": "종료 시간은 자정(24:00) 이내로 설정해주세요",
    "workItems.updateFailed": "근무항목 수정에 실패했습니다",
    "workItems.createFailed": "근무항목 생성에 실패했습니다",
    "workItems.timeInfo":
      "오전 10시 ~ 오전 12시와 같이 설정하면 자동으로 다음날까지 계산됩니다",
    // targets 탭
    "targets.title": "커버리지 타겟",
    "targets.weekday": "요일",
    "targets.start": "시작(분)",
    "targets.end": "종료(분)",
    "targets.min": "최소 인원",
    "targets.max": "최대 인원",
    "targets.roleHint": "역할",
    "targets.create": "타겟 추가",
    // rules / break-rules 탭
    "rules.title": "운영 규정",
    "rules.break.threshold": "임계(시간)",
    "rules.break.breakMin": "휴식(분)",
    "rules.break.paid": "유급",
    "rules.compliance.maxPerMonth": "월 최대 근무(시간)",
    // user preferred weekdays
    "user.preferredWeekdays": "출근 불가 요일",
    "user.preferredWeekdays.description": "출근 불가 요일을 선택하세요",
    "user.weekdays.sunday": "일요일",
    "user.weekdays.monday": "월요일",
    "user.weekdays.tuesday": "화요일",
    "user.weekdays.wednesday": "수요일",
    "user.weekdays.thursday": "목요일",
    "user.weekdays.friday": "금요일",
    "user.weekdays.saturday": "토요일",
    // schedule errors
    "schedule.errors.endAfterStart": "종료는 시작보다 커야 합니다",
    "schedule.errors.breakTooLong": "휴식 시간이 근무 시간을 초과합니다",
    "schedule.errors.insufficientRoleCoverage": "역할 인원 부족",
    "schedule.errors.roleValidationError": "역할 검증 오류",
    // schedule warnings
    "schedule.warning.exceedDesiredHours": "희망 주 근무 시간 초과 경고",
    "schedule.warning.exceedDesiredHoursMessage":
      "님의 이번 주 총 근무 시간이 희망 주 근무 시간을 초과합니다.",
    "schedule.warning.currentHours": "현재 근무 시간",
    "schedule.warning.newHours": "추가될 근무 시간",
    "schedule.warning.totalHours": "총 근무 시간",
    "schedule.warning.desiredHours": "희망 주 근무 시간",
    "schedule.warning.exceedBy": "초과 시간",
    "schedule.warning.difficultDay": "출근 불가 요일 경고",
    "schedule.warning.difficultDayMessage":
      "이 스케줄은 해당 사용자의 출근 불가 요일입니다.",
    "schedule.warning.difficultDayName": "출근 불가 요일",
    "schedule.warning.maxStaff": "최대 근무 인원 초과 경고",
    "schedule.warning.maxStaffMessage":
      "이 스케줄을 추가하면 해당 시간대의 최대 근무 인원을 초과합니다.",
    "schedule.warning.maxMorningStaff": "오전 최대 인원",
    "schedule.warning.maxAfternoonStaff": "오후 최대 인원",
    "schedule.warning.currentMorningStaff": "현재 오전 인원",
    "schedule.warning.currentAfternoonStaff": "현재 오후 인원",
    "schedule.warning.unavailable": "출근 불가 경고",
    "schedule.warning.unavailableMessage":
      "님은 해당 날짜에 출근 불가로 표시되어 있습니다.",
    "schedule.warning.crossStoreConflict": "다른 지점 근무 충돌",
    "schedule.warning.crossStoreConflictMessage":
      "다른 지점 스케줄과 시간이 겹쳐 현재 지점에 배정할 수 없습니다.",
    "schedule.warning.transferDesiredHours":
      "스케줄 이전 - 희망 주 근무 시간 초과 경고",
    "schedule.warning.transferDesiredHoursMessage":
      "이 스케줄을 이전하면 님의 이번 주 총 근무 시간이 희망 주 근무 시간을 초과합니다.",
    "schedule.warning.transferDifficultDay":
      "스케줄 이전 - 출근 불가 요일 경고",
    "schedule.warning.transferDifficultDayMessage":
      "이 스케줄은 대상 사용자의 출근 불가 요일입니다.",
    // schedule management
    "schedule.title": "스케줄 관리",
    "schedule.noStoreSelected": "매장을 선택해주세요",
    "schedule.export": "내보내기",
    "schedule.currentWeek": "이번 주",
    "schedule.weekOfMonth": "{month}월 {week}주차",
    "schedule.weekRangeWithMeta": "{weekMeta} ({range})",
    "schedule.weekView": "주간 보기",
    "schedule.monthView": "월간 보기",
    "schedule.weekGrid": "주간 그리드",
    "schedule.noData": "스케줄 데이터가 없습니다",
    "schedule.noDataDescription":
      "아직 생성된 스케줄이 없습니다. 첫 번째 스케줄을 생성해보세요.",
    "schedule.createFirst": "스케줄 생성하기",
    "schedule.monthViewComingSoon": "월간 보기 준비 중",
    "schedule.monthViewDescription": "월간 캘린더 뷰는 곧 제공될 예정입니다.",
    "schedule.copyWeek": "주 복사",
    "schedule.selectWeek": "주 선택",
    "schedule.chooseWeek": "선택할 주를 고르세요",
    "schedule.copyTargetWeek": "복사 적용 대상 주",
    "schedule.selectWeekToCopy": "복사할 주 선택",
    "schedule.copyPreviewTitle": "복사 원본 주 미리보기",
    "schedule.copyPreviewLoading": "원본 주 스케줄을 불러오는 중...",
    "schedule.copyPreviewEmpty": "원본 주에 등록된 스케줄이 없습니다",
    "schedule.copyPreviewLoadError": "원본 주 미리보기를 불러오지 못했습니다",
    "schedule.copyPreviousWeek": "이전 주 스케줄 복사",
    "schedule.copyWarning": "현재 주의 모든 스케줄이 삭제됩니다",
    "schedule.copySuccess": "스케줄이 복사되었습니다",
    "schedule.copyError": "스케줄 복사 중 오류가 발생했습니다",
    "schedule.copyConfirm": "스케줄을 복사하시겠습니까?",
    "schedule.copyConfirmDescription":
      "현재 주의 모든 스케줄이 삭제되고, 선택한 주의 스케줄이 복사됩니다.",
    "schedule.noWorkItemsTitle": "근무 항목 설정이 필요합니다",
    "schedule.noWorkItemsDescription":
      "스케줄을 등록하려면 먼저 근무 항목을 설정해야 합니다.",
    "schedule.noWorkItemsMessage":
      "지점 설정에서 근무 항목을 추가한 뒤 다시 스케줄을 등록해 주세요.",
    "schedule.goToWorkItems": "근무 항목 설정으로 이동",
    // availability calendar
    "availability.title": "휴일 희망 캘린더",
    "availability.markUnavailable": "휴일 희망일 등록",
    "availability.markAvailable": "휴일 희망일 해제",
    "availability.removeUnavailable": "휴일 희망일 해제",
    "availability.selectAction": "액션 선택",
    "availability.selectUser": "사용자 선택",
    "availability.currentUser": "현재 사용자",
    "availability.user": "사용자",
    "availability.selectUserToRemove": "해제할 사용자 선택",
    "availability.confirmRemove": "해제",
    "availability.reason": "사유 (선택사항)",
    "availability.reasonPlaceholder": "휴일 희망 사유를 입력하세요",
    "availability.confirmMarkUnavailable":
      "이 날짜를 휴일 희망일로 등록하시겠습니까?",
    "availability.confirmMarkAvailable":
      "이 날짜의 휴일 희망일을 해제하시겠습니까?",
    "availability.alreadyAssigned": "이미 배정된 날짜입니다",
    "availability.cannotModifyAfterPublish": "게시 후에는 수정할 수 없습니다",
    "availability.notificationTitle": "휴일 희망 상태 변경",
    "availability.notificationMessage":
      "{userName}님이 {date} 휴일 희망 상태를 {action}했습니다.",
    // export
    "export.title": "스케줄 내보내기",
    "export.format": "파일 형식",
    "export.xlsx": "Excel (XLSX)",
    "export.csv": "CSV",
    "export.png": "PNG 이미지",
    "export.scope": "내보내기 범위",
    "export.scopeAll": "전체 스케줄",
    "export.scopeMe": "내 스케줄만",
    "export.includePrivateInfo": "개인정보 포함",
    "export.includePrivateInfoDescription":
      "이메일, 전화번호 등 개인정보를 포함합니다",
    "export.download": "다운로드",
    "export.success": "파일이 성공적으로 생성되었습니다",
    "export.error": "파일 생성에 실패했습니다",
    "export.advanced": "고급 설정",
    "export.fileDownloaded": "파일이 다운로드되었습니다",
    "export.scopeMeOnly": "관리자만 전체 내보내기가 가능합니다",
    "export.period": "기간",
    "export.exporting": "내보내는 중...",
    "export.noDataToExport": "내보낼 데이터가 없습니다",
    "export.storeIdRequired": "매장 ID가 필요합니다",
    "export.defaultScheduleTitle": "스케줄",
    "export.weeklySheetTitle": "주간 스케줄",
    "export.crossStoreLegend": "다른 지점 근무 범례",
    // week grid
    "schedule.user": "사용자",
    "schedule.clickToAssign": "클릭하여 배정",
    "schedule.noUsers": "사용자가 없습니다",
    "schedule.selectAction": "작업을 선택하세요",
    "schedule.currentSchedule": "현재 스케줄",
    "schedule.selectWorkItem": "근무 항목 선택",
    "schedule.selectWorkItemPlaceholder": "근무 항목을 선택하세요",
    "schedule.selectWorkItemFirst": "먼저 근무 항목을 선택해주세요",
    "schedule.processing": "처리 중...",
    "schedule.scheduleAdded": "스케줄이 추가되었습니다",
    "schedule.scheduleAddError": "스케줄 추가 중 오류가 발생했습니다",
    "schedule.unavailableWarning":
      "경고: 해당 날짜에 사용자가 휴일 희망일로 등록되어 있습니다.",
    "schedule.scheduleDeleted": "스케줄이 삭제되었습니다",
    "schedule.scheduleDeleteError": "스케줄 삭제 중 오류가 발생했습니다",
    "schedule.noScheduleToDelete": "삭제할 스케줄이 없습니다",
    "schedule.userAlreadyHasAssignment": "이미 해당 날짜에 스케줄이 있습니다",
    "schedule.addSchedule": "스케줄 추가/수정",
    "schedule.setAvailability": "가용성 설정",
    "schedule.requestSwap": "교대 요청",
    "schedule.deleteSchedule": "스케줄 삭제",
    "schedule.transferSchedule": "다른 유저로 이전",
    "schedule.transferTo": "이전할 유저 선택",
    "schedule.transferToPlaceholder": "유저를 선택하세요",
    "schedule.transferSuccess": "스케줄이 성공적으로 이전되었습니다",
    "schedule.transferError": "스케줄 이전 중 오류가 발생했습니다",
    "schedule.multiDaySchedule": "복수 요일 스케줄 등록",
    "schedule.selectDays": "요일 선택",
    "schedule.weekdayPreferences.manage": "요일 선호 설정",
    "schedule.weekdayPreferences.title": "요일 선호 설정",
    "schedule.weekdayPreferences.description":
      "선호 요일과 출근 불가 요일을 함께 설정하세요.",
    "schedule.weekdayPreferences.preferredDays": "출근 선호 요일",
    "schedule.weekdayPreferences.difficultDays": "출근 불가 요일",
    "schedule.weekdayPreferences.loadError":
      "요일 선호 정보를 불러오지 못했습니다",
    "schedule.weekdayPreferences.saveSuccess":
      "요일 선호 정보가 저장되었습니다",
    "schedule.weekdayPreferences.saveError":
      "요일 선호 정보 저장에 실패했습니다",
    "schedule.warnings": "경고",
    "schedule.register": "등록",
    "schedule.warning.multiDayMessage":
      "님의 복수 요일 스케줄 등록 시 다음 경고가 있습니다.",
    "schedule.crossStoreBadge": "타지점",
    "schedule.crossStoreTooltipTitle": "다른 지점 근무",
    "availability.unavailable": "휴일 희망일",
    "availability.currentMonth": "이번 달",
    "availability.selectMonth": "월 선택",
    "availability.chooseMonth": "선택할 달을 고르세요",
    "availability.date": "날짜",
    "availability.today": "오늘",
    "availability.pastDate": "과거 날짜",
    "availability.pastDateDescription": "과거 날짜는 수정할 수 없습니다",
    "availability.success": "상태가 성공적으로 변경되었습니다",
    "availability.detailsTitle": "날짜 상세",
    "availability.noUnavailableDetails":
      "해당 날짜에 등록된 휴일 희망 내역이 없습니다",
    "availability.multiDateMode": "다른 날짜도 함께 등록",
    "availability.selectAdditionalDates": "추가 날짜 선택",
    "availability.selectedDates": "선택한 날짜",
    "availability.dateAlreadyUnavailable":
      "이미 휴일 희망일로 등록된 날짜입니다",
    "availability.hasTimeRestriction": "시간 제한",
    "availability.timePeriod": "시간대",
    "availability.startTime": "시작 시간",
    "availability.endTime": "종료 시간",
    "availability.multiDateRegisterResult":
      "{successCount}개 날짜가 등록되었습니다{failText}",
    "availability.multiDateRegisterFailSuffix": " ({failCount}개 실패)",
    "availability.allDateRegisterFailed": "모든 날짜 등록에 실패했습니다",
  },
  en: {
    "schedule.autoAssign": "Auto Assign",
    "schedule.autoAssignError": "An error occurred during automatic assignment.",
    "schedule.autoAssignResultTitle": "Automatic Assignment Result",
    "schedule.autoAssignResultDescription":
      "Created and saved {created} new schedule assignments.",
    "schedule.autoAssignUnmetTitle": "Operating coverage warnings ({count})",
    "schedule.autoAssignUnmetGuidance":
      "Review these segments and adjust the schedule manually.",
    "schedule.autoAssignUnmetHeadcount":
      "Assigned {assigned} of {required} required staff",
    "schedule.autoAssignMissingRoles": "Missing roles: {roles}",
    "schedule.autoAssignComplete":
      "All operating pattern segments meet their minimum staff and role requirements.",
    "schedule.deleteWeek": "Delete Week",
    "schedule.deleteWeekConfirm": "Delete",
    "schedule.deleteWeekConfirmTitle": "Delete this week's schedules?",
    "schedule.deleteWeekConfirmDescription":
      "All {count} schedules registered in {week} will be deleted. This action cannot be undone.",
    "schedule.deleteWeekSuccess": "{count} schedules deleted.",
    "schedule.deleteWeekError":
      "An error occurred while deleting this week's schedules.",
    "settings.store.autoSchedule": "Auto Schedule",
    "autoSchedule.title": "Auto Schedule Settings",
    "autoSchedule.description":
      "Manage automatic assignment priorities, operating patterns, and staff order for this store.",
    "autoSchedule.loadError": "Failed to load auto schedule settings.",
    "autoSchedule.saveSuccess": "Auto schedule settings saved.",
    "autoSchedule.saveError": "Failed to save auto schedule settings.",
    "autoSchedule.conditionPriorities": "Condition Priorities",
    "autoSchedule.conditionPrioritiesDescription":
      "Set the priority of preference conditions applied after required constraints pass.",
    "autoSchedule.condition.desiredWeeklyHours": "Desired Weekly Hours",
    "autoSchedule.condition.dayOffPreference": "Day Off Preference",
    "autoSchedule.condition.preferredWeekday": "Preferred Work Day",
    "autoSchedule.points": " pts",
    "autoSchedule.requiredConditions": "Locked Required Conditions",
    "autoSchedule.required.weeklyMax": "Weekly maximum work hours",
    "autoSchedule.required.monthlyMax": "Monthly maximum work hours",
    "autoSchedule.required.unavailable": "Unavailable dates or times",
    "autoSchedule.required.branchConflict": "Other branch work conflict",
    "autoSchedule.required.missingRole": "Missing required role",
    "autoSchedule.required.openingAnchor": "Opening anchor shift",
    "autoSchedule.required.staffLimit": "AM/PM maximum staff",
    "autoSchedule.required.coverageGap": "No business-hour coverage gaps",
    "autoSchedule.userPriorities": "Staff Assignment Priority",
    "autoSchedule.userPrioritiesDescription":
      "Drag staff or use the buttons to set the helper-score order for automatic assignment.",
    "autoSchedule.noMembers": "No active staff to prioritize.",
    "autoSchedule.moveUp": "Move up",
    "autoSchedule.moveDown": "Move down",
    "autoSchedule.openingPolicy": "Opening Policy",
    "autoSchedule.openingPolicyDescription":
      "Configure opening anchor usage, time, required headcount, and failure handling.",
    "autoSchedule.openingEnabled": "Use opening anchor",
    "autoSchedule.openingEnabledDescription":
      "Assign opening-capable staff first for the business opening period.",
    "autoSchedule.openingStartSource": "Opening start source",
    "autoSchedule.startSourceBusinessOpen": "Use business opening time",
    "autoSchedule.startSourceCustom": "Use custom time",
    "autoSchedule.openingCustomStart": "Custom start time",
    "autoSchedule.openingEndTime": "Opening end time",
    "autoSchedule.openingRequiredHeadcount": "Required opening staff",
    "autoSchedule.openingFailureMode": "When opening assignment fails",
    "autoSchedule.failureWarn": "Continue assignment + required warning",
    "autoSchedule.failureBlock": "Block commit",
    "autoSchedule.openingWorkItems": "Opening-capable work items",
    "autoSchedule.noWorkItems": "No work items available.",
    "autoSchedule.pattern.title": "Operating Patterns",
    "autoSchedule.pattern.description":
      "Set the minimum headcount and team role coverage required in each weekday time segment.",
    "autoSchedule.pattern.add": "Add pattern",
    "autoSchedule.pattern.empty":
      "No operating patterns are configured. The existing assignment rules will be used instead.",
    "autoSchedule.pattern.untitled": "Untitled pattern",
    "autoSchedule.pattern.active": "Enable pattern",
    "autoSchedule.pattern.remove": "Delete pattern",
    "autoSchedule.pattern.name": "Pattern name",
    "autoSchedule.pattern.weekdays": "Applicable weekdays",
    "autoSchedule.pattern.weekday.sun": "Sun",
    "autoSchedule.pattern.weekday.mon": "Mon",
    "autoSchedule.pattern.weekday.tue": "Tue",
    "autoSchedule.pattern.weekday.wed": "Wed",
    "autoSchedule.pattern.weekday.thu": "Thu",
    "autoSchedule.pattern.weekday.fri": "Fri",
    "autoSchedule.pattern.weekday.sat": "Sat",
    "autoSchedule.pattern.segmentName": "Segment name",
    "autoSchedule.pattern.startTime": "Start time",
    "autoSchedule.pattern.endTime": "End time",
    "autoSchedule.pattern.nextDay": "Next day",
    "autoSchedule.pattern.minHeadcount": "Minimum staff",
    "autoSchedule.pattern.requiredRoles": "Required roles",
    "autoSchedule.pattern.noRoles": "No active job roles are available.",
    "autoSchedule.pattern.removeSegment": "Delete segment",
    "autoSchedule.pattern.addSegment": "Add segment",
    "autoSchedule.pattern.fixErrors":
      "Fix the operating pattern errors before saving again.",
    "autoSchedule.pattern.issue.duplicateWeekday":
      "A weekday can belong to only one active operating pattern.",
    "autoSchedule.pattern.issue.emptyPatternName": "Enter a pattern name.",
    "autoSchedule.pattern.issue.emptySegmentName": "Enter a segment name.",
    "autoSchedule.pattern.issue.invalidTime":
      "The segment start time must be earlier than its end time.",
    "autoSchedule.pattern.issue.invalidHeadcount":
      "Minimum staff must be at least one.",
    "autoSchedule.pattern.issue.overlap":
      "Time segments overlap within this pattern.",
    "autoSchedule.pattern.issue.gap":
      "There is a gap without an operating requirement between segments.",
    "autoSchedule.pattern.issue.noRoles":
      "No required roles are selected. Only minimum staff will be checked.",
    "schedule.morningStaff": "Morning Staff",
    "schedule.afternoonStaff": "Afternoon Staff",
    "schedule.morningStaffShort": "AM",
    "schedule.afternoonStaffShort": "PM",
    "schedule.morning": "Morning",
    "schedule.afternoon": "Afternoon",
    "home.title": "Stop building this week's schedule from guesswork",
    "home.description":
      "An operations workspace for busy store owners to organize schedules, staff availability, and staffing coverage in one clear weekly flow.",
    "home.login": "Sign In",
    "home.signup": "Sign Up",
    "home.badge": "A scheduling console for small stores",
    "home.nav.features": "Features",
    "home.nav.preview": "Preview",
    "home.nav.trust": "Operations",
    "home.metric.schedule.value": "10 min",
    "home.metric.schedule.label": "Draft a weekly schedule",
    "home.metric.coverage.value": "7 days",
    "home.metric.coverage.label": "Check daily staffing coverage",
    "home.metric.team.value": "1 place",
    "home.metric.team.label": "Invite, assign, and manage staff",
    "home.preview.kicker": "Live schedule preview",
    "home.preview.title": "This week's operating flow",
    "home.preview.status": "Coverage stable",
    "home.preview.week.mon": "Mon",
    "home.preview.week.tue": "Tue",
    "home.preview.week.wed": "Wed",
    "home.preview.week.thu": "Thu",
    "home.preview.week.fri": "Fri",
    "home.preview.week.sat": "Sat",
    "home.preview.week.sun": "Sun",
    "home.preview.day.mon": "Monday opening",
    "home.preview.day.tue": "Tuesday lunch rush",
    "home.preview.day.wed": "Wednesday closing",
    "home.preview.shift.open": "08:00 - 13:00",
    "home.preview.shift.lunch": "11:00 - 16:00",
    "home.preview.shift.close": "17:00 - 22:00",
    "home.preview.role.barista": "2 baristas",
    "home.preview.role.kitchen": "3 kitchen",
    "home.preview.role.closing": "2 closing",
    "home.preview.coverageTitle": "Pre-assignment checks complete",
    "home.preview.coverageText":
      "Spot gaps quickly by reviewing requested days off, hour limits, and role-based staffing needs together.",
    "home.features.kicker": "Why Workeasy",
    "home.features.title": "See schedule issues before the week gets messy",
    "home.feature.schedule.title": "Build shift schedules faster",
    "home.feature.schedule.description":
      "Create and adjust weekly drafts around business hours, work items, and break rules.",
    "home.feature.team.title": "Keep staff availability in one place",
    "home.feature.team.description":
      "Manage invitations, roles, available days, and time-off preferences by store to reduce assignment mistakes.",
    "home.feature.analytics.title": "Check hours and coverage",
    "home.feature.analytics.description":
      "Review staff hours and daily gaps so next week's operating decisions are easier to make.",
    "home.trust.kicker": "Built for store operations",
    "home.trust.title": "Fast for managers, clear for staff",
    "home.trust.description":
      "Workeasy focuses on making the weekly scheduling loop shorter, clearer, and easier for small teams to repeat.",
    "home.trust.point.one": "Store roles and permissions keep management screens limited to the right people.",
    "home.trust.point.two": "Schedules, staff, and analytics stay connected inside the same store scope.",
    "home.trust.point.three": "Korean, English, and Japanese screens help multilingual teams share the same information.",
    "home.inviteError.title": "Invitation link error",
    "home.inviteError.description":
      "An error occurred while processing the invitation link. Please try again.",
    "auth.login.title": "Sign in to your account",
    "auth.login.noAccount": "Don't have an account?",
    "auth.login.signup": "Sign up",
    "auth.login.loginCard": "Login",
    "auth.login.description":
      "Enter your email and password to sign in to Workeasy.",
    "auth.login.forgotPassword": "Forgot your password?",
    "auth.login.sessionExpired":
      "Your session has expired. Please sign in again.",
    "auth.login.email": "Email",
    "auth.login.emailPlaceholder": "Enter your email",
    "auth.login.password": "Password",
    "auth.login.passwordPlaceholder": "Enter your password",
    "auth.login.passwordShow": "Show password",
    "auth.login.passwordHide": "Hide password",
    "auth.login.submit": "Sign In",
    "auth.login.submitting": "Signing in...",
    "auth.login.success": "Sign in successful",
    "auth.login.successDescription": "Sign in completed successfully",
    "auth.login.error": "Sign in failed",
    "auth.login.errorDescription": "Failed to sign in",
    "auth.login.validation.invalidData": "Invalid input data",
    "auth.login.validation.emailRequired": "Please enter your email",
    "auth.login.validation.invalidEmail": "Please enter a valid email address",
    "auth.login.validation.passwordRequired": "Please enter your password",
    "auth.login.error.general": "An error occurred during sign in",
    "auth.login.error.invalidCredentials":
      "The email or password you entered is incorrect",
    "auth.login.error.emailNotConfirmed":
      "Email verification is required. Please check your inbox",
    "auth.login.error.tooManyRequests":
      "Too many sign-in attempts. Please try again later",
    "auth.login.error.userNotFound": "No account found for this email address",
    "auth.login.loading": "Loading...",
    "auth.logout.success": "You have been signed out.",
    "auth.logout.error": "An error occurred while signing out.",
    "auth.signup.title": "Create new account",
    "auth.signup.hasAccount": "Already have an account?",
    "auth.signup.login": "Sign in",
    "auth.signup.signupCard": "Sign up",
    "auth.signup.description":
      "Enter your email and password to join Workeasy.",
    "auth.signup.email": "Email",
    "auth.signup.emailPlaceholder": "Enter your email",
    "auth.signup.password": "Password",
    "auth.signup.passwordPlaceholder": "Enter your password",
    "auth.signup.confirmPassword": "Confirm Password",
    "auth.signup.confirmPasswordPlaceholder": "Re-enter your password",
    "auth.signup.passwordShow": "Show password",
    "auth.signup.passwordHide": "Hide password",
    "auth.signup.submit": "Sign Up",
    "auth.signup.submitting": "Signing up...",
    "auth.signup.success": "Sign up successful",
    "auth.signup.successDescription": "Account created successfully",
    "auth.signup.error": "Sign up failed",
    "auth.signup.errorDescription": "Failed to create account",
    "auth.signup.validation.invalidData": "Invalid input data",
    "auth.signup.validation.invalidEmail": "Please enter a valid email address",
    "auth.signup.validation.passwordMinLength":
      "Password must be at least 8 characters",
    "auth.signup.validation.passwordMismatch": "Passwords do not match",
    "auth.signup.error.general": "An error occurred during sign up",
    "auth.signup.error.alreadyRegistered":
      "This email address is already registered",
    "auth.signup.error.passwordInvalid": "Please check password requirements",
    "auth.signup.error.invalidEmail":
      "Invalid email address. Please use a real email address",
    "auth.signup.error.emailCheck": "Please check your email address",
    "auth.signup.error.signupDisabled": "Sign up is currently disabled",
    "auth.signup.success.message": "Sign up completed successfully",
    "auth.signup.error.serverError": "A server error occurred",
    "auth.signupComplete.title": "Sign Up Complete",
    "auth.signupComplete.message": "Your account has been created successfully",
    "auth.signupComplete.description":
      "Email verification is complete. Please sign in to use the service",
    "auth.signupComplete.goToLogin": "Go to Sign In Page",
    "auth.verify.title": "Verify Email",
    "auth.verify.description":
      "We've sent a verification link to your email address.",
    "auth.verify.instruction":
      "Please check your email and click the link to activate your account.",
    "auth.verify.spam": "Don't see the email? Check your spam folder.",
    "auth.verify.backToLogin": "Back to Login",
    "dashboard.title": "Dashboard",
    "dashboard.welcome": "Hello, {name}!",
    "dashboard.logout": "Logout",
    "dashboard.loading": "Loading...",
    "dashboard.wait": "Please wait",
    "dashboard.currentStore": "Current Store: {storeName} ({role})",
    "dashboard.noStores": "No accessible stores",
    "dashboard.noStoresDescription":
      "You need to be invited to a store or create one.",
    "dashboard.noStoresModal.title": "Store Registration Required",
    "dashboard.noStoresModal.message":
      "You don't have any registered stores. Would you like to register a store?",
    "dashboard.noStoresModal.description":
      "By registering a store, you can use features such as schedule management and team member invitations.",
    "dashboard.noStoresModal.goToCreate": "Go to Store Registration Page",
    "dashboard.schedule": "Schedule Management",
    "dashboard.scheduleDescription": "Create and manage work schedules",
    "dashboard.viewSchedule": "View Schedule",
    "dashboard.createSchedule": "Create Schedule",
    "dashboard.shiftRequests": "Shift Requests",
    "dashboard.shiftRequestsDescription": "Manage shift requests",
    "dashboard.viewRequests": "View Requests",
    "dashboard.approveRequests": "Approve Requests",
    "dashboard.userManagement": "User Management",
    "dashboard.userManagementDescription":
      "Invite team members and manage permissions",
    "dashboard.viewUsers": "View Users",
    "dashboard.inviteUsers": "Invite Users",
    "dashboard.storeManagement": "Store Management",
    "dashboard.storeManagementDescription": "Store settings and management",
    "dashboard.viewStores": "View Stores",
    "dashboard.createStore": "Create Store",
    "dashboard.settings": "Settings",
    "dashboard.settingsDescription": "Account and store settings",
    "dashboard.profile": "Profile",
    "dashboard.storeSettings": "Store Settings",
    "dashboard.analytics": "Analytics",
    "dashboard.analyticsDescription": "Store operation analytics",
    "dashboard.viewAnalytics": "View Analytics",
    "dashboard.systemAdmin": "System Admin",
    "analytics.title": "Work Hours Analytics",
    "analytics.weeklyTotalHours": "Weekly Total Work Hours",
    "analytics.monthlyTotalHours": "Monthly Total Work Hours",
    "analytics.weeklyByUser": "Weekly Work Hours by User",
    "analytics.monthlyByUser": "Monthly Work Hours by User",
    "analytics.user": "User",
    "analytics.hours": "Hours",
    "analytics.noData": "No work-hour data to display",
    "analytics.loadError": "Failed to load analytics data",
    "analytics.selectWeek": "Select Week",
    "analytics.selectMonth": "Select Month",
    "analytics.chooseWeek": "Choose Week to Analyze",
    "analytics.chooseMonth": "Choose Month to Analyze",
    "dashboard.quickActions": "Quick Actions",
    "dashboard.quickCreateSchedule": "Create Schedule",
    "dashboard.quickInviteUser": "Invite User",
    "dashboard.quickApproveShifts": "Approve Shifts",
    "dashboard.myInfo": "My Info",
    "dashboard.scheduleManagement": "Schedule Management",
    "dashboard.chat": "Chat",
    "dashboard.preparing": "Coming Soon",
    "dashboard.scheduleDesc":
      "You'll be able to check and manage schedules soon.",
    "dashboard.chatDesc": "Communicate with team members in real-time.",
    "dashboard.thisWeekSchedule": "This Week Schedule",
    "dashboard.currentWeekWork": "Current week work schedule",
    "dashboard.totalHours": "Total hours",
    "dashboard.pendingRequests": "Pending Requests",
    "dashboard.shiftSwapRequests": "Shift swap requests awaiting approval",
    "dashboard.needAttention": "Need attention",
    "dashboard.activeStores": "Active Stores",
    "dashboard.operatingStores": "Currently operating stores",
    "dashboard.operatingNormally": "Operating normally",
    "dashboard.reviewRequests": "Review Shift Requests",
    "dashboard.teamChat": "Team Chat",
    "admin.title": "System Admin Dashboard",
    "admin.description":
      "Monitor platform-wide operations and anomalies in a single console.",
    "admin.console.badge": "System Console",
    "admin.console.title": "Operations Admin Console",
    "admin.console.description":
      "Track system status, anomalies, and audit activity from one operator-focused workspace.",
    "admin.console.account": "Admin Account",
    "admin.console.surfaceLabel": "Admin Surface",
    "admin.console.kicker": "Platform operations and audit mode",
    "admin.loadError": "Failed to load admin dashboard data.",
    "admin.login.badge": "Admin Access Only",
    "admin.login.title": "Sign in with your system admin account",
    "admin.login.description":
      "Platform operations, audit review, and anomaly handling are available only to approved admin accounts.",
    "admin.login.loginCard": "Admin Sign In",
    "admin.login.cardDescription":
      "Use an approved platform admin account to continue.",
    "admin.login.helper":
      "If this is a regular store account, use the standard sign-in page instead.",
    "admin.login.goToUserLogin": "Go to User Sign In",
    "admin.login.accessDenied":
      "This account does not have system admin access to sign in to the admin console.",
    "admin.login.panelOneLabel": "Access",
    "admin.login.panelOneValue":
      "Platform ops, audits, and anomaly response",
    "admin.login.panelTwoLabel": "Security",
    "admin.login.panelTwoValue":
      "Accounts without admin rights cannot enter the admin console.",
    "admin.readOnly": "Read-only",
    "admin.period": "Period",
    "admin.period.today": "Today",
    "admin.period.7d": "Last 7 days",
    "admin.period.30d": "Last 30 days",
    "admin.period.from": "From",
    "admin.period.to": "To",
    "admin.searchPlaceholder": "Search users, stores, events",
    "admin.exportCurrent": "Export Current Tab CSV",
    "admin.filters.apply": "Apply",
    "admin.filters.reset": "Reset",
    "admin.tab.overview": "Overview",
    "admin.tab.users": "Users",
    "admin.tab.stores": "Stores",
    "admin.tab.anomalies": "Anomalies",
    "admin.tab.logs": "Audit Logs",
    "admin.overview.description":
      "A concise snapshot of the core platform KPIs and operating flow.",
    "admin.overview.trendsDescription":
      "Major creation activity and operational change across the selected period.",
    "admin.kpi.totalUsers": "Total Users",
    "admin.kpi.activeUsers": "Active Users",
    "admin.kpi.activeStores": "Active Stores",
    "admin.kpi.coverage": "Assignment Coverage",
    "admin.trends.title": "Daily Trends",
    "admin.trends.date": "Date",
    "admin.trends.users": "New Users",
    "admin.trends.stores": "New Stores",
    "admin.trends.invitations": "Invitations",
    "admin.trends.assignments": "Assignments",
    "admin.users.title": "User Overview",
    "admin.users.description":
      "Review platform users, admin-role distribution, and recent activity in one place.",
    "admin.users.summaryDescription":
      "User list and platform-role state for the current filters.",
    "admin.users.inactiveSummary": "Inactive Users",
    "admin.users.email": "Email",
    "admin.users.name": "Name",
    "admin.users.platformRole": "Platform Role",
    "admin.users.status": "Status",
    "admin.users.lastSignIn": "Last Sign In",
    "admin.users.storeRoles": "Store Roles",
    "admin.users.activeStoreRoles": "Active",
    "admin.users.inactiveStoreRoles": "Inactive",
    "admin.users.allStatuses": "All Statuses",
    "admin.users.allRoles": "All Roles",
    "admin.users.emptyDescription":
      "No users match the current filters.",
    "admin.stores.title": "Store Overview",
    "admin.stores.description":
      "Inspect active and archived stores with operational risk signals.",
    "admin.stores.summaryDescription":
      "Review store density and the last 30 days of activity together.",
    "admin.stores.name": "Store",
    "admin.stores.status": "Status",
    "admin.stores.owner": "Owner",
    "admin.stores.members": "Members",
    "admin.stores.managers": "Managers",
    "admin.stores.pendingInvitations": "Pending Invites",
    "admin.stores.assignments30d": "Assignments (30d)",
    "admin.stores.allStatuses": "All Statuses",
    "admin.stores.emptyDescription":
      "No stores match the current filters.",
    "admin.stores.risk": "Risk",
    "admin.anomalies.title": "Anomalies",
    "admin.anomalies.description":
      "Review anomaly items created by operational detection rules and take action.",
    "admin.anomalies.summaryDescription":
      "Sort anomalies by severity and status to focus on the highest-priority issues.",
    "admin.anomalies.rule": "Rule",
    "admin.anomalies.severity": "Severity",
    "admin.anomalies.status": "Status",
    "admin.anomalies.metric": "Metric/Baseline",
    "admin.anomalies.detectedAt": "Detected At",
    "admin.anomalies.actions": "Actions",
    "admin.anomalies.ack": "Acknowledge",
    "admin.anomalies.resolve": "Resolve",
    "admin.anomalies.allStatuses": "All Statuses",
    "admin.anomalies.allSeverities": "All Severities",
    "admin.anomalies.emptyDescription":
      "No anomalies match the current filters.",
    "admin.anomalies.updateError": "Failed to update anomaly status.",
    "admin.logs.title": "Audit Logs",
    "admin.logs.description":
      "Search platform and store-level admin activity alongside system events.",
    "admin.logs.summaryDescription":
      "Track audit logs quickly by severity and event type.",
    "admin.logs.scope": "Scope",
    "admin.logs.eventType": "Event",
    "admin.logs.actor": "Actor",
    "admin.logs.severity": "Severity",
    "admin.logs.createdAt": "Created At",
    "admin.logs.allSeverities": "All Severities",
    "admin.logs.eventTypePlaceholder": "Enter event type",
    "admin.logs.emptyDescription":
      "No audit logs match the current filters.",
    "admin.pagination.prev": "Prev",
    "admin.pagination.next": "Next",
    "admin.empty.title": "No data to display",
    "admin.empty.description":
      "Adjust the filters or widen the date range and try again.",
    "admin.role.none": "None",
    "admin.status.active": "Active",
    "admin.status.inactive": "Inactive",
    "admin.status.archived": "Archived",
    "admin.risk.high": "High",
    "admin.risk.medium": "Medium",
    "admin.risk.low": "Low",
    "admin.severity.high": "High",
    "admin.severity.medium": "Medium",
    "admin.severity.low": "Low",
    "admin.anomalyStatus.open": "Open",
    "admin.anomalyStatus.ack": "Acknowledged",
    "admin.anomalyStatus.resolved": "Resolved",
    "admin.logs.scope.platform": "Platform",
    "admin.logs.scope.store": "Store",
    "admin.anomaly.inviteFailure.title": "Invitation failures spiked",
    "admin.anomaly.inviteFailure.description":
      "Invitation failures and expirations increased compared to the previous period.",
    "admin.anomaly.deactivation.title": "User deactivations spiked",
    "admin.anomaly.deactivation.description":
      "User deactivations increased sharply compared to the previous period.",
    "admin.anomaly.assignmentGap.title": "Schedule assignment gaps increased",
    "admin.anomaly.assignmentGap.description":
      "Cancelled assignments increased and may indicate coverage risk.",
    "admin.anomaly.roleChange.title": "Role changes spiked",
    "admin.anomaly.roleChange.description":
      "Role grant/revoke activity increased compared to the previous period.",
    // Invites related
    "invites.form.title": "Invite Team Member",
    "invites.form.send": "Send Invitation",
    "invites.form.sending": "Creating invitation...",
    "invites.form.storeRequired": "Please select a store",
    "invites.form.invitedDescription": "Invitation sent to {email}",
    "invites.form.store": "Select Store",
    "invites.form.email": "Email Address",
    "invites.form.role": "Role",
    "invites.list.title": "Invite List",
    "invites.status.pending": "Pending",
    "invites.status.accepted": "Accepted",
    "invites.status.cancelled": "Cancelled",
    "invites.status.expired": "Expired",
    "invites.role.master": "Master Administrator",
    "invites.role.subManager": "Sub Manager",
    "invites.role.partTimer": "Part Timer",
    // Role management related
    "role.change.success": "Role changed successfully",
    "role.change.description": "User role has been updated",
    "role.change.error": "Role change failed",
    "role.change.errorDescription": "An error occurred while changing the role",
    "role.management.title": "Role Management",
    "role.management.description": "You can change the user's role",
    "role.management.userInfo": "User Information",
    "role.management.currentRole": "Current Role",
    "role.management.newRole": "New Role",
    "role.management.selectRole": "Select a role",
    "role.management.current": "Current",
    "role.management.noPermission": "You don't have permission to change roles",
    "role.management.changing": "Changing...",
    "role.management.changeRole": "Change Role",
    "role.management.userList": "User List",
    "role.changePermissionDenied": "You do not have permission to change roles",
    "role.grantPermissionDenied": "You do not have permission to grant roles",
    "role.revokePermissionDenied": "You do not have permission to revoke roles",
    // Store related
    "store.noStores": "No Stores",
    "store.loading": "Loading stores...",
    "store.selectStore": "Select Store",
    "store.noStoreSelected": "No store selected",
    "store.totalStores": "Total {count} stores",
    "store.role.master": "Master",
    "store.role.sub_manager": "Sub Manager",
    "store.role.part_timer": "Part Timer",
    "store.createFirst": "Create First Store",
    "store.name": "Store Name",
    "store.namePlaceholder": "Enter store name",
    "store.description": "Description",
    "store.descriptionPlaceholder": "Enter store description",
    "store.address": "Address",
    "store.addressPlaceholder": "Enter store address",
    "store.phone": "Phone",
    "store.phonePlaceholder": "Enter store phone number",
    "store.timezone": "Timezone",
    "store.timezonePlaceholder": "Select timezone",
    "store.createTitle": "Create Store",
    "store.createDescription": "Create a new store",
    "store.editTitle": "Edit Store",
    "store.edit": "Edit Store",
    "store.editDescription": "Edit store information",
    "store.createButton": "Create Store",
    "store.updateButton": "Update Store",
    "store.createSuccess": "Store created successfully",
    "store.createError": "Failed to create store",
    "store.updateSuccess": "Store updated successfully",
    "store.updateError": "Failed to update store",
    "store.saveError": "Save failed",
    "store.networkError": "Network error occurred",
    "common.email": "Email",
    "common.name": "Name",
    "common.cancel": "Cancel",
    "common.save": "Save",
    "common.edit": "Edit",
    "common.delete": "Delete",
    "common.confirm": "Confirm",
    "common.loading": "Loading...",
    "common.yes": "Yes",
    "common.no": "No",
    "common.update": "Update",
    "common.create": "Create",
    "common.add": "Add",
    "common.info": "Info",
    "common.error": "Error",
    "common.unknownError": "An unknown error occurred",
    "common.success": "Success",
    "common.back": "Back",
    "common.view": "View",
    "common.close": "Close",
    "common.noData": "No data available",
    "store.noDescription": "No description",
    "store.loadError": "Failed to load store information",
    "store.notFound": "Store not found",
    "store.notFoundDescription":
      "The requested store does not exist or you do not have access.",
    "store.information": "Store Information",
    "store.informationDescription":
      "You can view the basic information of the store.",
    "store.status": "Status",
    "store.status.active": "Active",
    "store.status.archived": "Archived",
    "store.createdInfo": "Creation Info",
    "store.createdAt": "Created At",
    "store.updatedAt": "Updated At",
    "store.quickActions": "Quick Actions",
    "store.manageUsers": "Manage Users",
    "store.viewSchedule": "View Schedule",
    "store.settings": "Settings",
    "store.noPermission": "You don't have permission to edit this store",
    // 사용자 관리 관련 번역
    "user.management": "User Management",
    "user.managementForStore": "{storeName} Store User Management",
    "user.members": "Members",
    "user.total": "Total",
    "user.name": "Name",
    "user.email": "Email",
    "user.role": "Role",
    "user.status": "Status",
    "user.joinedAt": "Joined At",
    "user.temporaryWork": "Temporary Work",
    "user.actions": "Actions",
    "user.noName": "No Name",
    "user.defaultStore": "Default Store",
    "user.noMembers": "No members",
    "user.filters": "Filters",
    "user.search": "Search",
    "user.searchPlaceholder": "Search by name or email",
    "user.allRoles": "All Roles",
    "user.allStatuses": "All Statuses",
    "user.clearFilters": "Clear Filters",
    "user.invite": "Invite",
    "user.temporaryAssign": "Temporary Assignment",
    "user.startDate": "Start Date",
    "user.endDate": "End Date",
    "user.reason": "Reason",
    "user.reasonPlaceholder": "Enter reason for temporary assignment",
    "user.assign": "Assign",
    "user.roleGranted": "Role Granted",
    "user.roleGrantedDescription": "Role granted successfully to the user",
    "user.roleGrantError": "Role grant failed",
    "user.roleRevoked": "Role Revoked",
    "user.roleRevokedDescription": "Role revoked successfully from the user",
    "user.roleRevokeError": "Role revoke failed",
    "user.demoteSubManager": "Demote Sub Manager",
    "user.demoteSubManagerDescription":
      "Demote a sub manager to part timer",
    "user.demoteSubManagerError": "Failed to demote sub manager",
    "user.deactivateUser": "Deactivate User",
    "user.deactivateUserDescription": "Deactivate this user",
    "user.deactivateUserError": "Failed to deactivate user",
    "user.reactivateUser": "Reactivate User",
    "user.reactivateUserDescription": "Reactivate this user",
    "user.reactivateUserError": "Failed to reactivate user",
    "user.deleteUser": "Delete User",
    "user.deleteUserDescription":
      "Delete this user (schedule history is preserved)",
    "user.deleteUserError": "Failed to delete user",
    "user.notFound": "User not found",
    "user.alreadyInactive": "User is already inactive",
    "user.alreadyActive": "User is already active",
    "user.deletePermissionDenied": "You do not have permission to delete users",
    "user.deactivatePermissionDenied":
      "You do not have permission to deactivate users",
    "user.reactivatePermissionDenied":
      "You do not have permission to reactivate users",
    "user.cannotDeleteSelf": "You cannot delete your own account",
    "user.cannotDeleteMaster": "Users with master role cannot be deleted",
    "user.roleNotFound": "User role not found",
    "user.onlySubManagerCanDemote": "Only sub managers can be demoted",
    "user.onlyActiveSubManagerCanDemote":
      "Only active sub managers can be demoted",
    "user.deleteSuccessWithHistory":
      "User deleted successfully. Schedule history is preserved.",
    "user.temporaryAssigned": "Temporary assignment completed",
    "user.temporaryAssignedDescription": "Temporary work assigned successfully",
    "user.temporaryAssignError": "Temporary assignment failed",
    // User detail page translations
    "user.detail": "User Detail",
    "user.detailForStore": "{storeName} Store - {userName} Detail",
    "user.basicInfo": "Basic Information",
    "user.jobRoles": "Job Roles",
    "user.jobRoles.none": "No active job roles have been configured.",
    "user.workPreferences": "Work Preferences",
    "user.resignationDate": "Resignation Date",
    "user.desiredWeeklyHours": "Desired Weekly Hours",
    "user.desiredWeeklyHoursPlaceholder": "Enter weekly work hours (e.g., 20)",
    "user.desiredDailyHours": "Desired Daily Work Hours",
    "user.desiredDailyHoursPlaceholder":
      "Enter preferred daily work hours (e.g., 6.5)",
    "user.noResignationDate": "No resignation date",
    "user.setResignationDate": "Set resignation date",
    "user.removeResignationDate": "Remove resignation date",
    "user.updateProfile": "Update Profile",
    "user.profileUpdated": "Profile Updated",
    "user.profileUpdatedDescription":
      "User profile has been successfully updated",
    "user.profileUpdateError": "Profile update failed",
    "user.viewDetail": "View Detail",
    "user.backToUserList": "Back to User List",
    "user.guest": "Guest",
    "user.normal": "Normal",
    "user.deactivate": "Deactivate",
    "user.reactivate": "Reactivate",
    "user.difficultWeekday.updateError":
      "Failed to update unavailable work days",
    "user.difficultWeekday.deleteError":
      "Failed to delete unavailable work days",
    "user.difficultWeekday.insertError":
      "Failed to insert unavailable work days",
    // Profile page translations
    "profile.title": "Profile",
    "profile.myProfile": "My Profile",
    "profile.personalInfo": "Personal Information",
    "profile.accountInfo": "Account Information",
    "profile.changePassword": "Change Password",
    "profile.currentPassword": "Current Password",
    "profile.newPassword": "New Password",
    "profile.confirmPassword": "Confirm Password",
    "profile.passwordChanged": "Password Changed",
    "profile.passwordChangedDescription":
      "Password has been successfully changed",
    "profile.passwordChangeError": "Password change failed",
    "profile.currentPasswordRequired": "Please enter current password",
    "profile.newPasswordRequired": "Please enter new password",
    "profile.passwordMismatch": "Passwords do not match",
    "profile.passwordTooShort": "Password must be at least 6 characters",
    "profile.saveChanges": "Save Changes",
    "profile.cancel": "Cancel",
    "profile.daysRemaining": "Days Remaining",
    "profile.resignationPassed": "Resignation date has passed",
    "profile.resignationToday": "Today is the resignation date",
    "profile.resignationTomorrow": "Tomorrow is the resignation date",
    "profile.daysLeft": "days left",
    "user.status.active": "Active",
    "user.status.pending": "Pending",
    "user.status.inactive": "Inactive",
    "user.alreadyDeleted": "Already Deleted User",
    "user.alreadyDeletedDescription": "This user has already been deleted.",
    "user.promoteToSubManager": "Promote",
    "user.confirmPromoteMessage":
      "Are you sure you want to promote this user to sub manager?",
    "user.adminActions": "Admin Actions",
    "user.guestUser": "Guest User",
    // 초대 수락 페이지
    "invite.accept.title": "Accept Invitation",
    "invite.accept.description": "Create an account to join the store",
    "invite.accept.storeInfo": "Store Information",
    "invite.accept.storeName": "Store Name",
    "invite.accept.role": "Role",
    "invite.accept.expiresAt": "Expires At",
    "invite.accept.accountInfo": "Account Information",
    "invite.accept.name": "Name",
    "invite.accept.namePlaceholder": "Enter your name",
    "invite.accept.password": "Password",
    "invite.accept.passwordPlaceholder": "Enter your password",
    "invite.accept.confirmPassword": "Confirm Password",
    "invite.accept.confirmPasswordPlaceholder": "Re-enter your password",
    "invite.accept.accept": "Accept Invitation",
    "invite.accept.accepting": "Accepting...",
    "invite.accept.success": "Invitation Accepted",
    "invite.accept.successDescription": "Successfully joined the store",
    "invite.accept.error": "Failed to Accept Invitation",
    "invite.accept.invalidToken": "Invalid Invitation",
    "invite.accept.expiredToken": "Expired Invitation",
    "invite.accept.loading": "Loading invitation information...",
    "invite.accept.existingUser": "Existing User",
    "invite.accept.existingUserMessage":
      "This email is already registered. Please sign in to accept the invitation.",
    "invite.accept.loginToAccept": "Sign in to Accept Invitation",
    "invite.accept.emailMismatch":
      "The invited email and the currently signed-in email do not match",
    "invite.accept.alreadyMember": "You are already a member of this store",
    "invite.accept.processingError":
      "An error occurred while processing invitation acceptance",
    "invite.accept.tokenRequired": "Invitation token is required",
    "invite.accept.notFound": "Invitation not found",
    "invite.accept.status.valid": "Valid invitation",
    "invite.accept.status.cancelled": "Cancelled invitation",
    "invite.accept.status.used": "Already used invitation",
    "invite.accept.status.expired": "Expired invitation",
    "invite.accept.joinedRoleMessage":
      "Successfully joined {storeName} as {roleName}",
    "invite.accept.signInFailed":
      "Sign-in failed: {details}. Try the default password (1q2w3e4r!).",
    "invite.accept.userCreateFailed": "Failed to create user",
    "invite.accept.storeAddFailed": "Failed to add user to store",
    "invite.setupPassword.title": "Set Password",
    "invite.setupPassword.description":
      "Set your password to accept the invitation to {storeName}.",
    "invite.setupPassword.unknownStore": "Unknown store",
    "invite.setupPassword.newPassword": "New password",
    "invite.setupPassword.passwordPlaceholder": "At least 8 characters",
    "invite.setupPassword.passwordPlaceholderMin6": "At least 6 characters",
    "invite.setupPassword.submit": "Set Password and Accept Invitation",
    "invite.setupPassword.successTitle": "Password set successfully",
    "invite.setupPassword.failureTitle": "Failed to set password",
    "invite.setupPassword.loginRequiredTitle": "Login required",
    "invite.setupPassword.loginRequired":
      "Please sign in through the invitation link.",
    "invite.setupPassword.nameRequiredTitle": "Please enter your name",
    "invite.setupPassword.nameRequiredDescription": "Name is required.",
    "invite.setupPassword.passwordMismatchTitle": "Passwords do not match",
    "invite.setupPassword.passwordMismatchDescription":
      "Please check your password again.",
    "invite.setupPassword.passwordTooShortTitle": "Password is too short",
    "invite.setupPassword.passwordTooShortDescription":
      "Please enter at least 8 characters.",
    "invite.setupPassword.passwordTooShortMin6Description":
      "Please enter at least 6 characters.",
    "invite.setupPassword.loadError":
      "An error occurred while loading the information.",
    "invite.setupPassword.invitationLoadError":
      "Failed to load invitation information.",
    "invite.setupPassword.userLoadError":
      "An error occurred while checking user information.",
    "invite.setupPassword.checkingUser": "Checking user information...",
    "invite.error.title": "Invitation Link Error",
    "invite.error.processing":
      "An error occurred while processing the invitation link. Please try again.",
    "invite.error.requestNew": "Request New Invitation",
    "invite.error.requestNewDescription":
      "Please ask the administrator for a new invitation.",
    "invite.error.message.otpExpired": "The email link has expired.",
    "invite.error.message.accessDenied": "Access denied.",
    "invite.error.message.invalidInvite":
      "There is a problem with the invitation link.",
    "invite.error.description.default":
      "Please check the invitation link again.",
    "invite.error.description.otpExpired":
      "The invitation link has expired. Please request a new invitation.",
    "invite.error.description.accessDenied":
      "You do not have access to this invitation link. Please contact the administrator.",
    "invite.error.description.invalidInvite":
      "The invitation link is invalid or corrupted.",
    "invite.error.code": "Error Code",
    "invite.error.type": "Error Type",
    "invite.error.detail": "Details",
    "invite.error.goHome": "Go to Home",
    "invite.verify.title": "Email Verification",
    "invite.verify.description": "Email verification completed.",
    "invite.verify.verifying": "Verifying...",
    "invite.verify.completedTitle": "Email Verification Complete",
    "invite.verify.processedTitle": "Invitation Already Processed",
    "invite.verify.processedDescription":
      "This invitation has already been processed or has expired.",
    "invite.verify.sessionNotFound": "Session information was not found.",
    "invite.verify.userNotFound": "User information was not found.",
    "invite.verify.checkError": "An error occurred while checking verification.",
    "invite.verify.passwordSetAndAccepted":
      "Password has been set and invitation accepted.",
    "invite.verify.passwordSetupPrompt":
      "Set a secure password and accept the invitation.",
    "invite.verify.linkVerifiedPrompt":
      "Invitation link has been verified. Please set your password.",
    "invite.verify.infoTitle": "Verification Information",
    "invite.verify.safePasswordNote": "Please set a secure password.",
    "invite.verify.setPassword": "Set Password",
    "invite.verify.userInfoTitle": "User Information",
    "invite.verify.newPasswordPlaceholder":
      "Enter a new password (minimum 8 characters)",
    "invite.verify.error": "Verification Error",
    "invite.verify.errorDescription": "Email verification required.",
    "invite.verify.acceptTitle": "Accept Invitation",
    "invite.verify.acceptDescription":
      "Accept the store invitation and use the service.",
    "invite.verify.loginInfo": "Login Information",
    "invite.verify.defaultPassword": "Default Password",
    "invite.verify.passwordChangeNote":
      "Please change your password in settings after login.",
    "invite.verify.acceptButton": "Accept Invitation",
    "invite.verify.accepting": "Accepting...",
    "invite.verify.success":
      "Invitation accepted successfully. Please login to use the service.",
    "invite.verify.acceptError": "Failed to accept invitation.",
    "invite.title": "Invitation Management",
    "invite.email": "Email",
    "invite.emailPlaceholder": "Enter the email to invite",
    "invite.storePlaceholder": "Select a store",
    "invite.role": "Role",
    "invite.rolePlaceholder": "Select a role to assign",
    "invite.roleDescription.subManager":
      "Can manage schedules, approve swaps, and invite part-timers",
    "invite.roleDescription.partTimer":
      "Can view schedules and request shift swaps",
    "invite.status": "Status",
    "invite.actions": "Actions",
    "invite.invitedAt": "Invited At",
    "invite.expiresAt": "Expires At",
    "invite.accepted": "Accepted",
    "invite.pending": "Pending",
    "invite.expired": "Expired",
    "invite.cancelled": "Cancelled",
    "invite.resend": "Resend",
    "invite.cancel": "Cancel",
    "invite.create": "Create Invitation",
    "invite.name": "Name",
    "invite.partTimer": "Part Timer",
    "invite.subManager": "Sub Manager",
    "invite.days": "days",
    "invite.createSuccess": "Invitation created successfully",
    "invite.createError": "Failed to create invitation",
    "invite.loadError": "Failed to load invitations",
    "invite.linkCopied": "Link copied",
    "invite.linkCopiedDescription": "Invitation link copied to clipboard",
    "invite.copyError": "Failed to copy link",
    "invite.empty": "No invitations yet",
    "invite.emptyDescription": "Invite a new team member",
    "invite.accessDenied": "You do not have access to this store",
    "invite.managePermissionDenied":
      "You do not have management permission for this store",
    "invite.cancelUsed": "Used invitations cannot be cancelled",
    "invite.alreadyCancelled": "This invitation is already cancelled",
    "invite.duplicateActiveInvite":
      "An active invitation already exists for this email",
    "invite.tokenCreateError": "Failed to generate invitation token",
    "invite.storeIdRequired": "Store ID is required",
    "invite.listLoadError": "Failed to load invitation list",
    "invite.resendSuccess": "Invitation resent successfully",
    "invite.resendError": "Failed to resend invitation",
    "invite.cancelSuccess": "Invitation cancelled successfully",
    "invite.cancelError": "Failed to cancel invitation",
    "invite.cancelPermissionDenied": "You do not have permission to cancel invitations",
    "invite.resendPermissionDenied":
      "You do not have permission to resend invitations",
    "invite.userDeleteFailed": "Failed to delete existing user",
    "invite.userDeleteRetryFailed":
      "Failed to delete user. Please try again later.",
    "invite.emailSendFailed": "Failed to send email",
    "invite.alreadyRegisteredRetry":
      "This email is already registered. Please try again later.",
    "invite.resendEmailError":
      "An error occurred while resending the invitation email",
    "invite.emailRequired": "Please enter email",
    "invite.namePlaceholder": "John Doe",
    "invite.guestNoEmailDescription":
      "Guest users can be registered without an email address.",
    "invite.expiresIn": "Invitation Link Expires",
    "invite.list": "Invitation List",
    "invite.registerWithoutEmail": "Register without email (Guest user)",
    "invite.refresh": "Refresh",
    "invite.mode.email": "Email Invite",
    "invite.mode.guest": "Guest Registration",
    "invite.mode.import": "Other Store Users",
    "invite.importSourceStore": "Source Store",
    "invite.importCandidates": "Import Candidates",
    "invite.importSourceRole": "Source Store Role",
    "invite.importEmpty": "There are no users available to import",
    "invite.importLoadError": "Failed to load users from another store",
    "invite.importSelectionRequired": "Please select at least one user to import",
    "invite.importSubmit": "Add to Current Store",
    "invite.importSuccess": "Users from another store were added successfully",
    "invite.importError": "Failed to add users from another store",
    "invite.importNoSourceStores":
      "There are no other managed stores available for import",
    "invite.importResultImported": "Imported",
    "invite.importResultAlreadyExists": "Already exists",
    "invite.importResultSkipped": "Skipped",
    "dashboard.passwordChangeRequired": "Password Change Required",
    "dashboard.passwordChangeDescription":
      "Please change your default password for security.",
    "dashboard.passwordChangeButton": "Change Password",
    // settings.store (edit tabs)
    "settings.store.title": "Store Settings",
    "settings.store.basic": "Basic",
    "settings.store.hours": "Hours & Holidays",
    "settings.store.workItems": "Work Items",
    "settings.store.roles": "Job Management",
    "settings.store.targets": "Coverage",
    "settings.store.breakRules": "Break Rules",
    "settings.store.policies": "Policies",
    "settings.store.notifications": "Notifications",
    "settings.store.saveSuccess": "Saved",
    "settings.store.saveError": "Save failed",
    "settings.store.validationError": "Validation Error",
    "settings.store.open": "Open",
    "settings.store.close": "Close",
    "settings.store.holidays": "Holidays",
    "settings.store.date": "Date",
    "settings.store.policiesDescription":
      "Set only essential operational rules. Complex rules have been removed for ease of use.",
    "settings.store.currencyUnitDesc":
      "Currency unit used for labor budget and salary calculations",
    "settings.store.publishCutoffDesc":
      "Minimum time before schedule publication (in hours)",
    "settings.store.swapLeadTimeDesc":
      "Minimum lead time for shift swap requests (in hours)",
    "settings.store.minRestDesc":
      "Minimum rest time between consecutive shifts",
    "settings.store.maxHoursPerDayDesc":
      "Maximum work hours per day (0 = no limit)",
    "settings.store.maxHoursPerWeekDesc":
      "Maximum work hours per week (0 = no limit)",
    "settings.store.maxHoursPerMonthDesc":
      "Maximum work hours per month (default: 160 hours)",
    "settings.store.maxConsecutiveDaysDesc":
      "Maximum consecutive work days (0 = no limit)",
    "settings.store.weeklyBudgetDesc":
      "Weekly labor budget ({currency} unit, 0 = no limit)",
    // workItems tab
    "workItems.title": "Work Items",
    "workItems.name": "Name",
    "workItems.start": "Start (min)",
    "workItems.end": "End (min)",
    "workItems.break": "Unpaid Break (min)",
    "workItems.maxHeadcount": "Max Headcount",
    "workItems.roleHint": "Role",
    "workItems.roles": "Job Management",
    "workItems.create": "Add Work Item",
    "workItems.description": "Manage work items required for the store",
    "workItems.namePlaceholder": "Enter work item name",
    "workItems.timeSettings": "Time Settings",
    "workItems.startTime": "Start Time",
    "workItems.endTime": "End Time",
    "workItems.nextDay": "Next Day",
    "workItems.nextDayLabel": "Until next day",
    "workItems.roleRequirements": "Job Requirements",
    "workItems.availableJobs": "Available Jobs",
    "workItems.selectedJobs": "Selected Jobs",
    "workItems.noRolesSelected": "No jobs selected",
    "workItems.list": "Work Items List",
    "workItems.listDescription": "Currently active work items in this store",
    "workItems.noItems": "No work items found",
    "workItems.editNotImplemented": "Edit feature will be implemented later",
    "workItems.deleteNotImplemented":
      "Delete feature will be implemented later",
    "workItems.edit": "Edit",
    "workItems.delete": "Delete",
    "workItems.editTitle": "Edit Work Item",
    "workItems.deleteConfirm":
      "Are you sure you want to delete this work item?",
    "workItems.deleteConfirmDescription":
      "Deleted work items cannot be recovered.",
    "workItems.updateSuccess": "Work item updated successfully",
    "workItems.deleteSuccess": "Work item deleted successfully",
    // 운영 규정 (Policies)
    "policies.publish_cutoff_hours": "Schedule Publish Cutoff Hours",
    "policies.swap_lead_time_hours": "Swap Lead Time Hours",
    "policies.min_rest_hours_between_shifts": "Min Rest Hours Between Shifts",
    "policies.max_hours_per_day": "Max Hours Per Day",
    "policies.max_hours_per_week": "Max Hours Per Week",
    "policies.max_consecutive_days": "Max Consecutive Days",
    "policies.max_morning_staff": "Max Morning Staff",
    "policies.max_morning_staff_desc":
      "Maximum number of staff working in the morning shift (0 = no limit)",
    "policies.min_morning_staff": "Min Morning Staff",
    "policies.min_morning_staff_desc":
      "Minimum number of staff working in the morning shift (0 = no limit)",
    "policies.max_afternoon_staff": "Max Afternoon Staff",
    "policies.max_afternoon_staff_desc":
      "Maximum number of staff working in the afternoon shift (0 = no limit)",
    "policies.min_afternoon_staff": "Min Afternoon Staff",
    "policies.min_afternoon_staff_desc":
      "Minimum number of staff working in the afternoon shift (0 = no limit)",
    "policies.weekly_labor_budget_cents": "Weekly Labor Budget (Cents)",
    "policies.schedule_unit": "Schedule Unit",
    "policies.schedule_unit_week": "Weekly",
    "policies.schedule_unit_month": "Monthly",
    "policies.currency_unit": "Currency Unit",
    "policies.currency_krw": "Korean Won",
    "policies.currency_usd": "US Dollar",
    "policies.currency_jpy": "Japanese Yen",
    "policies.currency_eur": "Euro",
    "policies.currency_cad": "Canadian Dollar",
    "policies.currency_aud": "Australian Dollar",
    "policies.shift_boundary_time": "Morning/Afternoon Boundary Time",
    "policies.shift_boundary_time_desc":
      "The time that divides morning and afternoon shifts in the schedule grid. Staff counts are calculated based on this time.",
    "policies.group.basicSettings": "Basic Settings",
    "policies.group.basicSettingsDesc": "Basic settings such as schedule unit, currency unit, and time boundaries",
    "policies.group.schedulePublishing": "Schedule Publishing",
    "policies.group.schedulePublishingDesc": "Settings related to schedule publishing",
    "policies.group.workHoursLimits": "Work Hours Limits",
    "policies.group.workHoursLimitsDesc": "Weekly, monthly work hours and consecutive work days limits",
    "policies.group.staffManagement": "Staff Management",
    "policies.group.staffManagementDesc": "Minimum and maximum staff settings for morning and afternoon shifts",
    "policies.group.unusedSettings": "Unused Settings",
    "policies.group.unusedSettingsDesc": "Settings that are currently not in use",
    "workItems.nameRequired": "Work item name is required",
    "workItems.endAfterStart": "End time must be after start time",
    "workItems.breakTooLong": "Break time cannot be longer than work time",
    "workItems.calculatedMaxHeadcount": "Calculated Max Headcount",
    "workItems.autoCalculated": "Auto-calculated from job requirements",
    "workItems.maxHeadcountLimit":
      "Total required headcount by roles cannot exceed 99",
    "workItems.endTimeLimit": "End time must be within 24:00",
    "workItems.updateFailed": "Failed to update work item",
    "workItems.createFailed": "Failed to create work item",
    "workItems.timeInfo":
      "Setting like 10 AM ~ 12 AM will automatically calculate to the next day",
    // targets tab
    "targets.title": "Staffing Targets",
    "targets.weekday": "Weekday",
    "targets.start": "Start (min)",
    "targets.end": "End (min)",
    "targets.min": "Min",
    "targets.max": "Max",
    "targets.roleHint": "Role",
    "targets.create": "Add Target",
    // rules / break
    "rules.title": "Policies",
    "rules.break.threshold": "Threshold (hrs)",
    "rules.break.breakMin": "Break (min)",
    "rules.break.paid": "Paid",
    "rules.compliance.maxPerMonth": "Max Hours / Month",
    // user preferred weekdays
    "user.preferredWeekdays": "Unavailable Work Days",
    "user.preferredWeekdays.description":
      "Select the days that are unavailable to work",
    "user.weekdays.sunday": "Sunday",
    "user.weekdays.monday": "Monday",
    "user.weekdays.tuesday": "Tuesday",
    "user.weekdays.wednesday": "Wednesday",
    "user.weekdays.thursday": "Thursday",
    "user.weekdays.friday": "Friday",
    "user.weekdays.saturday": "Saturday",
    // schedule errors
    "schedule.errors.endAfterStart": "End must be after Start",
    "schedule.errors.breakTooLong": "Break exceeds duration",
    "schedule.errors.insufficientRoleCoverage": "Insufficient role coverage",
    "schedule.errors.roleValidationError": "Role validation error",
    // schedule warnings
    "schedule.warning.exceedDesiredHours":
      "Exceed Desired Weekly Hours Warning",
    "schedule.warning.exceedDesiredHoursMessage":
      "'s total weekly hours will exceed their desired weekly hours.",
    "schedule.warning.currentHours": "Current Hours",
    "schedule.warning.newHours": "New Hours",
    "schedule.warning.totalHours": "Total Hours",
    "schedule.warning.desiredHours": "Desired Weekly Hours",
    "schedule.warning.exceedBy": "Exceed By",
    "schedule.warning.difficultDay": "Unavailable Work Day Warning",
    "schedule.warning.difficultDayMessage":
      "This schedule falls on a work-unavailable day for this user.",
    "schedule.warning.difficultDayName": "Unavailable Work Day",
    "schedule.warning.maxStaff": "Maximum Staff Limit Warning",
    "schedule.warning.maxStaffMessage":
      "Adding this schedule will exceed the maximum staff limit for this time period.",
    "schedule.warning.maxMorningStaff": "Max Morning Staff",
    "schedule.warning.maxAfternoonStaff": "Max Afternoon Staff",
    "schedule.warning.currentMorningStaff": "Current Morning Staff",
    "schedule.warning.currentAfternoonStaff": "Current Afternoon Staff",
    "schedule.warning.unavailable": "Unavailable Warning",
    "schedule.warning.unavailableMessage":
      "is marked as unavailable for this date.",
    "schedule.warning.crossStoreConflict": "Cross-store Schedule Conflict",
    "schedule.warning.crossStoreConflictMessage":
      "This shift overlaps with an assignment in another store and cannot be added.",
    "schedule.warning.transferDesiredHours":
      "Transfer Schedule - Exceed Desired Hours Warning",
    "schedule.warning.transferDesiredHoursMessage":
      "'s total weekly hours will exceed their desired weekly hours after transferring this schedule.",
    "schedule.warning.transferDifficultDay":
      "Transfer Schedule - Unavailable Work Day Warning",
    "schedule.warning.transferDifficultDayMessage":
      "This schedule falls on a work-unavailable day for the target user.",
    // schedule management
    "schedule.title": "Schedule Management",
    "schedule.noStoreSelected": "Please select a store",
    "schedule.export": "Export",
    "schedule.currentWeek": "Current Week",
    "schedule.weekOfMonth": "Week {week} of Month {month}",
    "schedule.weekRangeWithMeta": "{weekMeta} ({range})",
    "schedule.weekView": "Week View",
    "schedule.monthView": "Month View",
    "schedule.weekGrid": "Week Grid",
    "schedule.noData": "No schedule data",
    "schedule.noDataDescription":
      "No schedules have been created yet. Create your first schedule.",
    "schedule.createFirst": "Create Schedule",
    "schedule.monthViewComingSoon": "Month View Coming Soon",
    "schedule.monthViewDescription":
      "Monthly calendar view will be available soon.",
    "schedule.copyWeek": "Copy Week",
    "schedule.selectWeek": "Select Week",
    "schedule.chooseWeek": "Choose a Week",
    "schedule.copyTargetWeek": "Target Week to Apply Copy",
    "schedule.selectWeekToCopy": "Select Week to Copy",
    "schedule.copyPreviewTitle": "Source Week Preview",
    "schedule.copyPreviewLoading": "Loading source week schedules...",
    "schedule.copyPreviewEmpty": "No schedules registered for the source week",
    "schedule.copyPreviewLoadError": "Failed to load source week preview",
    "schedule.copyPreviousWeek": "Copy Previous Week Schedule",
    "schedule.copyWarning": "All schedules in the current week will be deleted",
    "schedule.copySuccess": "Schedule copied successfully",
    "schedule.copyError": "An error occurred while copying the schedule",
    "schedule.copyConfirm": "Do you want to copy the schedule?",
    "schedule.copyConfirmDescription":
      "All schedules in the current week will be deleted and the selected week's schedules will be copied.",
    "schedule.noWorkItemsTitle": "Work items need to be set up",
    "schedule.noWorkItemsDescription":
      "You need to configure work items before creating a schedule.",
    "schedule.noWorkItemsMessage":
      "Add work items in store settings, then return to create the schedule.",
    "schedule.goToWorkItems": "Go to Work Item Settings",
    // availability calendar
    "availability.title": "Day Off Preference Calendar",
    "availability.markUnavailable": "Register Day Off Preference",
    "availability.markAvailable": "Remove Day Off Preference",
    "availability.removeUnavailable": "Remove Day Off Preference",
    "availability.selectAction": "Select Action",
    "availability.selectUser": "Select User",
    "availability.currentUser": "Current User",
    "availability.user": "User",
    "availability.selectUserToRemove": "Select User to Remove",
    "availability.confirmRemove": "Remove",
    "availability.reason": "Reason (Optional)",
    "availability.reasonPlaceholder": "Enter reason for day off preference",
    "availability.confirmMarkUnavailable":
      "Register this date as a day off preference?",
    "availability.confirmMarkAvailable":
      "Remove day off preference for this date?",
    "availability.alreadyAssigned": "Already assigned for this date",
    "availability.cannotModifyAfterPublish": "Cannot modify after publishing",
    "availability.notificationTitle": "Day Off Preference Status Changed",
    "availability.notificationMessage":
      "{userName} has {action} day off preference for {date}.",
    // export
    "export.title": "Export Schedule",
    "export.format": "File Format",
    "export.xlsx": "Excel (XLSX)",
    "export.csv": "CSV",
    "export.png": "PNG Image",
    "export.scope": "Export Scope",
    "export.scopeAll": "All Schedules",
    "export.scopeMe": "My Schedule Only",
    "export.includePrivateInfo": "Include Private Info",
    "export.includePrivateInfoDescription": "Include email, phone number, etc.",
    "export.download": "Download",
    "export.success": "File generated successfully",
    "export.error": "Failed to generate file",
    "export.advanced": "Advanced Settings",
    "export.fileDownloaded": "File downloaded",
    "export.scopeMeOnly": "Only administrators can export all schedules",
    "export.period": "Period",
    "export.exporting": "Exporting...",
    "export.noDataToExport": "There is no data to export",
    "export.storeIdRequired": "Store ID is required",
    "export.defaultScheduleTitle": "Schedule",
    "export.weeklySheetTitle": "Weekly Schedule",
    "export.crossStoreLegend": "Cross-store Work Legend",
    // week grid
    "schedule.user": "User",
    "schedule.clickToAssign": "Click to assign",
    "schedule.noUsers": "No users",
    "schedule.selectAction": "Select an action",
    "schedule.currentSchedule": "Current Schedule",
    "schedule.selectWorkItem": "Select Work Item",
    "schedule.selectWorkItemPlaceholder": "Select a work item",
    "schedule.selectWorkItemFirst": "Please select a work item first",
    "schedule.processing": "Processing...",
    "schedule.scheduleAdded": "Schedule has been added",
    "schedule.scheduleAddError": "Error occurred while adding schedule",
    "schedule.unavailableWarning":
      "Warning: User has a day off preference for this date.",
    "schedule.scheduleDeleted": "Schedule has been deleted",
    "schedule.scheduleDeleteError": "Error occurred while deleting schedule",
    "schedule.noScheduleToDelete": "No schedule to delete",
    "schedule.userAlreadyHasAssignment":
      "User already has an assignment for this date",
    "schedule.addSchedule": "Add/Edit Schedule",
    "schedule.setAvailability": "Set Availability",
    "schedule.requestSwap": "Request Swap",
    "schedule.deleteSchedule": "Delete Schedule",
    "schedule.transferSchedule": "Transfer to Another User",
    "schedule.transferTo": "Select User to Transfer",
    "schedule.transferToPlaceholder": "Select a user",
    "schedule.transferSuccess": "Schedule transferred successfully",
    "schedule.transferError": "Error occurred while transferring schedule",
    "schedule.multiDaySchedule": "Multi-Day Schedule Registration",
    "schedule.selectDays": "Select Days",
    "schedule.weekdayPreferences.manage": "Set Day Preferences",
    "schedule.weekdayPreferences.title": "Day Preferences",
    "schedule.weekdayPreferences.description":
      "Set preferred work days and unavailable work days together.",
    "schedule.weekdayPreferences.preferredDays": "Preferred Work Days",
    "schedule.weekdayPreferences.difficultDays": "Unavailable Work Days",
    "schedule.weekdayPreferences.loadError":
      "Failed to load day preference settings",
    "schedule.weekdayPreferences.saveSuccess":
      "Day preference settings saved",
    "schedule.weekdayPreferences.saveError":
      "Failed to save day preference settings",
    "schedule.warnings": "Warnings",
    "schedule.register": "Register",
    "schedule.warning.multiDayMessage":
      " has the following warnings for multi-day schedule registration:",
    "schedule.crossStoreBadge": "Other",
    "schedule.crossStoreTooltipTitle": "Working in another store",
    "availability.unavailable": "Day Off Preference",
    "availability.currentMonth": "Current Month",
    "availability.selectMonth": "Select Month",
    "availability.chooseMonth": "Choose a Month",
    "availability.date": "Date",
    "availability.today": "Today",
    "availability.pastDate": "Past Date",
    "availability.pastDateDescription": "Cannot modify past dates",
    "availability.success": "Status updated successfully",
    "availability.detailsTitle": "Date Details",
    "availability.noUnavailableDetails":
      "No day off preference records are registered for this date",
    "availability.multiDateMode": "Register additional dates together",
    "availability.selectAdditionalDates": "Select Additional Dates",
    "availability.selectedDates": "Selected Dates",
    "availability.dateAlreadyUnavailable":
      "This date is already registered as a day off preference",
    "availability.hasTimeRestriction": "Time Restriction",
    "availability.timePeriod": "Time Period",
    "availability.startTime": "Start Time",
    "availability.endTime": "End Time",
    "availability.multiDateRegisterResult":
      "{successCount} dates registered{failText}",
    "availability.multiDateRegisterFailSuffix": " ({failCount} failed)",
    "availability.allDateRegisterFailed": "Failed to register all selected dates",
    // work item roles
    "workItemRoles.title": "Work Item Role Requirements",
    "workItemRoles.description": "Set required roles for this work item",
    "workItemRoles.addRole": "Add Role",
    "workItemRoles.removeRole": "Remove Role",
    "workItemRoles.minCount": "Min Count",
    "workItemRoles.saveSuccess": "Role requirements saved successfully",
    "workItemRoles.saveFailed": "Failed to save role requirements",
    "workItemRoles.minCountMin": "Min count must be 0 or greater",
    "workItemRoles.minCountMax": "Min count must be 99 or less",
    "workItemRoles.requirementsSet": "Requirements Set",
    "workItemRoles.currentCount": "Current: {current}",
    "workItemRoles.requiredCount": "Required: {required}",
    "workItemRoles.roleCoverage": "Role Coverage",
    "workItemRoles.insufficient": "Insufficient role coverage",
    "workItemRoles.sufficient": "Sufficient role coverage",
    // job roles
    "jobRoles.title": "Job Management",
    "jobRoles.description": "Manage jobs required for the store",
    "jobRoles.create": "Create Job",
    "jobRoles.edit": "Edit Job",
    "jobRoles.delete": "Delete Job",
    "jobRoles.name": "Job Name",
    "jobRoles.code": "Code",
    "jobRoles.roleDescription": "Description",
    "jobRoles.active": "Active",
    "jobRoles.inactive": "Inactive",
    "jobRoles.usageCount": "Users: {count}",
    "jobRoles.confirmDelete": "Are you sure you want to delete this job?",
    "jobRoles.deleteError": "Cannot delete job that is in use",
    "jobRoles.createSuccess": "Job created successfully",
    "jobRoles.updateSuccess": "Job updated successfully",
    "jobRoles.deleteSuccess": "Job deleted successfully",
    "jobRoles.nameRequired": "Job name is required",
    "jobRoles.nameDuplicate": "Job name already exists",
    "jobRoles.nameMaxLength": "Job name must be 48 characters or less",
    "jobRoles.codeMaxLength": "Code must be 32 characters or less",
    "jobRoles.descriptionMaxLength":
      "Description must be 160 characters or less",
    "jobRoles.namePlaceholder": "Enter job name",
    "jobRoles.descriptionPlaceholder": "Enter job description",
    "jobRoles.list": "Job List",
    "jobRoles.listDescription": "Currently active jobs in this store",
    "jobRoles.createdAt": "Created",
    "jobRoles.noRoles": "No jobs found",
    invite: {
      title: "Invite Part-timer",
      create: "Create Invitation",
      list: "Invitation List",
      email: "Email Address",
      role: "Role",
      expiresIn: "Expires In",
      status: "Status",
      actions: "Actions",
      resend: "Resend",
      cancel: "Cancel",
      accept: "Accept",
      pending: "Pending",
      accepted: "Accepted",
      expired: "Expired",
      cancelled: "Cancelled",
      partTimer: "Part-timer",
      subManager: "Sub Manager",
      days: "days",
      createSuccess: "Invitation created successfully",
      createError: "Failed to create invitation",
      resendSuccess: "Invitation resent successfully",
      resendError: "Failed to resend invitation",
      cancelSuccess: "Invitation cancelled successfully",
      cancelError: "Failed to cancel invitation",
      acceptSuccess: "Invitation accepted successfully",
      acceptError: "Failed to accept invitation",
      invalidToken: "Invalid or expired invitation",
      noPermission: "No permission to manage invitations",
      onlyPendingResend: "Only pending invitations can be resent",
      onlyPendingCancel: "Only pending invitations can be cancelled",
      emailRequired: "Email address is required",
      invalidEmail: "Please enter a valid email address",
      roleRequired: "Role is required",
      expiresInRequired: "Expiration period is required",
      expiresInRange: "Expiration period must be between 1 and 30 days",
      tokenRequired: "Invitation token is required",
      subject: "Store Invitation",
      body: {
        welcome: "Hello!",
        invited: "You have been invited to join a store.",
        storeName: "Store Name",
        role: "Role",
        expiresAt: "Expires At",
        cta: "Accept Invitation",
        footer: "This link expires in 7 days.",
      },
    },
  },
  ja: {
    "schedule.autoAssign": "自動割当",
    "schedule.autoAssignError": "自動割当中にエラーが発生しました。",
    "schedule.autoAssignResultTitle": "自動割当結果",
    "schedule.autoAssignResultDescription":
      "新しいスケジュールを{created}件登録して保存しました。",
    "schedule.autoAssignUnmetTitle": "運営カバー警告 {count}件",
    "schedule.autoAssignUnmetGuidance":
      "以下の時間帯はスケジュールで手動調整してください。",
    "schedule.autoAssignUnmetHeadcount":
      "必要人数{required}人のうち{assigned}人を割当",
    "schedule.autoAssignMissingRoles": "不足している役割: {roles}",
    "schedule.autoAssignComplete":
      "すべての運営パターン時間帯で最低人数と役割条件を満たしました。",
    "schedule.deleteWeek": "週を削除",
    "schedule.deleteWeekConfirm": "削除",
    "schedule.deleteWeekConfirmTitle": "この週のスケジュールを削除しますか？",
    "schedule.deleteWeekConfirmDescription":
      "{week}に登録されたスケジュール{count}件がすべて削除されます。この操作は元に戻せません。",
    "schedule.deleteWeekSuccess": "スケジュール{count}件を削除しました。",
    "schedule.deleteWeekError":
      "この週のスケジュール削除中にエラーが発生しました。",
    "settings.store.autoSchedule": "自動スケジュール",
    "autoSchedule.title": "自動スケジュール設定",
    "autoSchedule.description":
      "自動割当の条件優先度、運営パターン、スタッフ順序を店舗別に管理します。",
    "autoSchedule.loadError": "自動スケジュール設定を読み込めませんでした。",
    "autoSchedule.saveSuccess": "自動スケジュール設定を保存しました。",
    "autoSchedule.saveError": "自動スケジュール設定の保存に失敗しました。",
    "autoSchedule.conditionPriorities": "条件優先度",
    "autoSchedule.conditionPrioritiesDescription":
      "必須条件を通過した候補に適用する希望条件の優先度を設定します。",
    "autoSchedule.condition.desiredWeeklyHours": "希望週勤務時間",
    "autoSchedule.condition.dayOffPreference": "休日希望日",
    "autoSchedule.condition.preferredWeekday": "出勤希望曜日",
    "autoSchedule.points": "点",
    "autoSchedule.requiredConditions": "固定必須条件",
    "autoSchedule.required.weeklyMax": "週最大勤務時間",
    "autoSchedule.required.monthlyMax": "月最大勤務時間",
    "autoSchedule.required.unavailable": "出勤不可日時",
    "autoSchedule.required.branchConflict": "他店舗勤務との衝突",
    "autoSchedule.required.missingRole": "必要職務なし",
    "autoSchedule.required.openingAnchor": "オープニング担当",
    "autoSchedule.required.staffLimit": "午前/午後最大人数",
    "autoSchedule.required.coverageGap": "営業時間の空きなし",
    "autoSchedule.userPriorities": "スタッフ割当優先度",
    "autoSchedule.userPrioritiesDescription":
      "スタッフをドラッグまたはボタンで移動し、自動割当の補助点順を設定します。",
    "autoSchedule.noMembers": "優先度を設定できる有効スタッフがいません。",
    "autoSchedule.moveUp": "上へ移動",
    "autoSchedule.moveDown": "下へ移動",
    "autoSchedule.openingPolicy": "オープニングポリシー",
    "autoSchedule.openingPolicyDescription":
      "オープニング担当の使用有無、時間、必要人数、失敗時の処理を設定します。",
    "autoSchedule.openingEnabled": "オープニング担当を使用",
    "autoSchedule.openingEnabledDescription":
      "営業開始時間帯にオープニング可能なスタッフを先に割り当てます。",
    "autoSchedule.openingStartSource": "開始時間の基準",
    "autoSchedule.startSourceBusinessOpen": "営業時間の開始を使用",
    "autoSchedule.startSourceCustom": "別の時間を使用",
    "autoSchedule.openingCustomStart": "別の開始時間",
    "autoSchedule.openingEndTime": "オープニング終了時間",
    "autoSchedule.openingRequiredHeadcount": "必要オープニング人数",
    "autoSchedule.openingFailureMode": "割当失敗時の処理",
    "autoSchedule.failureWarn": "通常割当を続行 + 必須警告",
    "autoSchedule.failureBlock": "登録をブロック",
    "autoSchedule.openingWorkItems": "オープニング可能な勤務項目",
    "autoSchedule.noWorkItems": "選択できる勤務項目がありません。",
    "autoSchedule.pattern.title": "運営パターン",
    "autoSchedule.pattern.description":
      "曜日ごとの時間帯に必要な最低人数とチームの役割カバーを設定します。",
    "autoSchedule.pattern.add": "パターンを追加",
    "autoSchedule.pattern.empty":
      "運営パターンがありません。既存の自動割当基準を使用します。",
    "autoSchedule.pattern.untitled": "名称未設定のパターン",
    "autoSchedule.pattern.active": "パターンを有効化",
    "autoSchedule.pattern.remove": "パターンを削除",
    "autoSchedule.pattern.name": "パターン名",
    "autoSchedule.pattern.weekdays": "適用曜日",
    "autoSchedule.pattern.weekday.sun": "日",
    "autoSchedule.pattern.weekday.mon": "月",
    "autoSchedule.pattern.weekday.tue": "火",
    "autoSchedule.pattern.weekday.wed": "水",
    "autoSchedule.pattern.weekday.thu": "木",
    "autoSchedule.pattern.weekday.fri": "金",
    "autoSchedule.pattern.weekday.sat": "土",
    "autoSchedule.pattern.segmentName": "時間帯名",
    "autoSchedule.pattern.startTime": "開始時間",
    "autoSchedule.pattern.endTime": "終了時間",
    "autoSchedule.pattern.nextDay": "翌日",
    "autoSchedule.pattern.minHeadcount": "最低人数",
    "autoSchedule.pattern.requiredRoles": "必要な役割",
    "autoSchedule.pattern.noRoles": "選択できる有効な役割がありません。",
    "autoSchedule.pattern.removeSegment": "時間帯を削除",
    "autoSchedule.pattern.addSegment": "時間帯を追加",
    "autoSchedule.pattern.fixErrors":
      "運営パターンのエラーを修正してから保存してください。",
    "autoSchedule.pattern.issue.duplicateWeekday":
      "同じ曜日は1つの有効な運営パターンにのみ設定できます。",
    "autoSchedule.pattern.issue.emptyPatternName": "パターン名を入力してください。",
    "autoSchedule.pattern.issue.emptySegmentName": "時間帯名を入力してください。",
    "autoSchedule.pattern.issue.invalidTime":
      "開始時間は終了時間より前に設定してください。",
    "autoSchedule.pattern.issue.invalidHeadcount":
      "最低人数は1人以上に設定してください。",
    "autoSchedule.pattern.issue.overlap": "パターン内の時間帯が重複しています。",
    "autoSchedule.pattern.issue.gap":
      "時間帯の間に運営基準がない空きがあります。",
    "autoSchedule.pattern.issue.noRoles":
      "必要な役割が未選択です。最低人数のみ確認します。",
    "schedule.morningStaff": "午前人員",
    "schedule.afternoonStaff": "午後人員",
    "schedule.morningStaffShort": "午前",
    "schedule.afternoonStaffShort": "午後",
    "schedule.morning": "午前",
    "schedule.afternoon": "午後",
    "home.title": "今週のシフト表を勘だけで作らない",
    "home.description":
      "忙しい店舗運営者がシフト、スタッフの勤務可能時間、人員カバレッジをひとつの週間フローで整理できる運用ツールです。",
    "home.login": "ログイン",
    "home.signup": "サインアップ",
    "home.badge": "小規模店舗のためのシフト運用コンソール",
    "home.nav.features": "機能",
    "home.nav.preview": "プレビュー",
    "home.nav.trust": "運用安定性",
    "home.metric.schedule.value": "10分",
    "home.metric.schedule.label": "週間シフト案を作成",
    "home.metric.coverage.value": "7日",
    "home.metric.coverage.label": "曜日別の人員カバレッジ確認",
    "home.metric.team.value": "1か所",
    "home.metric.team.label": "招待から割当まで一元管理",
    "home.preview.kicker": "シフトプレビュー",
    "home.preview.title": "今週の運用フロー",
    "home.preview.status": "カバレッジ安定",
    "home.preview.week.mon": "月",
    "home.preview.week.tue": "火",
    "home.preview.week.wed": "水",
    "home.preview.week.thu": "木",
    "home.preview.week.fri": "金",
    "home.preview.week.sat": "土",
    "home.preview.week.sun": "日",
    "home.preview.day.mon": "月曜オープン",
    "home.preview.day.tue": "火曜ランチピーク",
    "home.preview.day.wed": "水曜クローズ",
    "home.preview.shift.open": "08:00 - 13:00",
    "home.preview.shift.lunch": "11:00 - 16:00",
    "home.preview.shift.close": "17:00 - 22:00",
    "home.preview.role.barista": "バリスタ 2名",
    "home.preview.role.kitchen": "キッチン 3名",
    "home.preview.role.closing": "締め 2名",
    "home.preview.coverageTitle": "自動割当前チェック完了",
    "home.preview.coverageText":
      "希望休、勤務時間上限、役割別の必要人数をまとめて確認し、空き時間帯をすばやく見つけます。",
    "home.features.kicker": "Workeasyの強み",
    "home.features.title": "シフトが乱れる前に課題が見えます",
    "home.feature.schedule.title": "シフト表をすばやく作成",
    "home.feature.schedule.description":
      "営業時間、勤務項目、休憩ルールをもとに週間シフト案を作成し、すばやく調整できます。",
    "home.feature.team.title": "勤務可能時間を一か所に整理",
    "home.feature.team.description":
      "招待、役割、勤務可能日、希望休を店舗単位で管理し、割当ミスを減らします。",
    "home.feature.analytics.title": "勤務時間とカバレッジを確認",
    "home.feature.analytics.description":
      "スタッフ別の勤務時間と曜日別の不足人数を確認し、翌週の運用判断をしやすくします。",
    "home.trust.kicker": "店舗運用のための設計",
    "home.trust.title": "管理者には速く、スタッフには明確に",
    "home.trust.description":
      "Workeasyは、小さなチームが毎週くり返すシフト調整を短く、透明にすることに集中しています。",
    "home.trust.point.one": "店舗別の権限と役割により、必要な人だけが管理画面にアクセスできます。",
    "home.trust.point.two": "シフト、スタッフ、分析の流れが同じ店舗スコープ内でつながります。",
    "home.trust.point.three": "韓国語、英語、日本語の画面で、多国籍チームも同じ情報を共有できます。",
    "home.inviteError.title": "招待リンクエラー",
    "home.inviteError.description":
      "招待リンクの処理中にエラーが発生しました。もう一度お試しください。",
    "auth.login.title": "アカウントにログイン",
    "auth.login.noAccount": "アカウントをお持ちでないですか？",
    "auth.login.signup": "サインアップ",
    "auth.login.loginCard": "ログイン",
    "auth.login.description":
      "メールアドレスとパスワードを入力してWorkeasyにログインしてください。",
    "auth.login.forgotPassword": "パスワードをお忘れですか？",
    "auth.login.sessionExpired":
      "セッションが期限切れです。再度ログインしてください。",
    "auth.login.email": "メールアドレス",
    "auth.login.emailPlaceholder": "メールアドレスを入力してください",
    "auth.login.password": "パスワード",
    "auth.login.passwordPlaceholder": "パスワードを入力してください",
    "auth.login.passwordShow": "パスワードを表示",
    "auth.login.passwordHide": "パスワードを非表示",
    "auth.login.submit": "ログイン",
    "auth.login.submitting": "ログイン中...",
    "auth.login.success": "ログイン成功",
    "auth.login.successDescription": "ログインが正常に完了しました",
    "auth.login.error": "ログイン失敗",
    "auth.login.errorDescription": "ログインに失敗しました",
    "auth.login.validation.invalidData": "入力データが無効です",
    "auth.login.validation.emailRequired": "メールアドレスを入力してください",
    "auth.login.validation.invalidEmail":
      "有効なメールアドレスを入力してください",
    "auth.login.validation.passwordRequired": "パスワードを入力してください",
    "auth.login.error.general": "ログイン中にエラーが発生しました",
    "auth.login.error.invalidCredentials":
      "メールアドレスまたはパスワードが正しくありません",
    "auth.login.error.emailNotConfirmed":
      "メール確認が必要です。登録時のメールをご確認ください",
    "auth.login.error.tooManyRequests":
      "ログイン試行回数が多すぎます。しばらくしてから再試行してください",
    "auth.login.error.userNotFound":
      "このメールアドレスのアカウントは見つかりませんでした",
    "auth.login.loading": "読み込み中...",
    "auth.logout.success": "ログアウトが完了しました。",
    "auth.logout.error": "ログアウト中にエラーが発生しました。",
    "auth.signup.title": "新しいアカウントを作成",
    "auth.signup.hasAccount": "既にアカウントをお持ちですか？",
    "auth.signup.login": "ログイン",
    "auth.signup.signupCard": "サインアップ",
    "auth.signup.description":
      "メールアドレスとパスワードを入力してWorkeasyに参加してください。",
    "auth.signup.email": "メールアドレス",
    "auth.signup.emailPlaceholder": "メールアドレスを入力してください",
    "auth.signup.password": "パスワード",
    "auth.signup.passwordPlaceholder": "パスワードを入力してください",
    "auth.signup.confirmPassword": "パスワード確認",
    "auth.signup.confirmPasswordPlaceholder": "パスワードを再入力してください",
    "auth.signup.passwordShow": "パスワードを表示",
    "auth.signup.passwordHide": "パスワードを非表示",
    "auth.signup.submit": "サインアップ",
    "auth.signup.submitting": "サインアップ中...",
    "auth.signup.success": "サインアップ成功",
    "auth.signup.successDescription": "アカウントが正常に作成されました",
    "auth.signup.error": "サインアップ失敗",
    "auth.signup.errorDescription": "アカウントの作成に失敗しました",
    "auth.signup.validation.invalidData": "入力データが無効です",
    "auth.signup.validation.invalidEmail":
      "有効なメールアドレスを入力してください",
    "auth.signup.validation.passwordMinLength":
      "パスワードは最低8文字以上である必要があります",
    "auth.signup.validation.passwordMismatch": "パスワードが一致しません",
    "auth.signup.error.general": "サインアップ中にエラーが発生しました",
    "auth.signup.error.alreadyRegistered":
      "このメールアドレスは既に登録されています",
    "auth.signup.error.passwordInvalid": "パスワードの要件を確認してください",
    "auth.signup.error.invalidEmail":
      "無効なメールアドレスです。実際のメールアドレスを使用してください",
    "auth.signup.error.emailCheck": "メールアドレスを確認してください",
    "auth.signup.error.signupDisabled": "現在サインアップが無効になっています",
    "auth.signup.success.message": "サインアップが正常に完了しました",
    "auth.signup.error.serverError": "サーバーエラーが発生しました",
    "auth.signupComplete.title": "サインアップ完了",
    "auth.signupComplete.message": "アカウントが正常に作成されました",
    "auth.signupComplete.description":
      "メール確認が完了しました。ログインしてサービスをご利用ください",
    "auth.signupComplete.goToLogin": "ログインページへ移動",
    "auth.verify.title": "メール確認",
    "auth.verify.description":
      "ご登録のメールアドレスに確認リンクをお送りしました。",
    "auth.verify.instruction":
      "メールをご確認いただき、リンクをクリックしてアカウントを有効化してください。",
    "auth.verify.spam":
      "メールが見つからない場合は、迷惑メールフォルダをご確認ください。",
    "auth.verify.backToLogin": "ログインページに戻る",
    "dashboard.title": "ダッシュボード",
    "dashboard.welcome": "こんにちは、{name}さん！",
    "dashboard.logout": "ログアウト",
    "dashboard.loading": "読み込み中...",
    "dashboard.wait": "少々お待ちください",
    "dashboard.currentStore": "現在の店舗: {storeName} ({role})",
    "dashboard.noStores": "アクセス可能な店舗がありません",
    "dashboard.noStoresDescription":
      "店舗に招待されるか、店舗を作成する必要があります。",
    "dashboard.noStoresModal.title": "店舗登録が必要です",
    "dashboard.noStoresModal.message":
      "登録された店舗がありません。店舗を登録しますか？",
    "dashboard.noStoresModal.description":
      "店舗を登録すると、スケジュール管理やチームメンバー招待などの機能を使用できます。",
    "dashboard.noStoresModal.goToCreate": "店舗登録ページへ移動",
    "dashboard.schedule": "スケジュール管理",
    "dashboard.scheduleDescription": "勤務表の作成と管理",
    "dashboard.viewSchedule": "スケジュール表示",
    "dashboard.createSchedule": "スケジュール作成",
    "dashboard.shiftRequests": "シフト交換リクエスト",
    "dashboard.shiftRequestsDescription": "シフト交換リクエストの管理",
    "dashboard.viewRequests": "リクエスト表示",
    "dashboard.approveRequests": "リクエスト承認",
    "dashboard.userManagement": "ユーザー管理",
    "dashboard.userManagementDescription": "チームメンバーの招待と権限管理",
    "dashboard.viewUsers": "ユーザー表示",
    "dashboard.inviteUsers": "ユーザー招待",
    "dashboard.storeManagement": "店舗管理",
    "dashboard.storeManagementDescription": "店舗設定と管理",
    "dashboard.viewStores": "店舗表示",
    "dashboard.createStore": "店舗作成",
    "dashboard.settings": "設定",
    "dashboard.settingsDescription": "アカウントと店舗設定",
    "dashboard.profile": "プロフィール",
    "dashboard.storeSettings": "店舗設定",
    "dashboard.analytics": "分析",
    "dashboard.analyticsDescription": "店舗運営分析",
    "dashboard.viewAnalytics": "分析表示",
    "dashboard.systemAdmin": "システム管理",
    "analytics.title": "勤務時間分析",
    "analytics.weeklyTotalHours": "週間総勤務時間",
    "analytics.monthlyTotalHours": "月間総勤務時間",
    "analytics.weeklyByUser": "ユーザー別週間勤務時間",
    "analytics.monthlyByUser": "ユーザー別月間勤務時間",
    "analytics.user": "ユーザー",
    "analytics.hours": "勤務時間",
    "analytics.noData": "表示する勤務データがありません",
    "analytics.loadError": "分析データの読み込みに失敗しました",
    "analytics.selectWeek": "週を選択",
    "analytics.selectMonth": "月を選択",
    "analytics.chooseWeek": "分析する週を選択",
    "analytics.chooseMonth": "分析する月を選択",
    "dashboard.quickActions": "クイックアクション",
    "dashboard.quickCreateSchedule": "スケジュール作成",
    "dashboard.quickInviteUser": "ユーザー招待",
    "dashboard.quickApproveShifts": "シフト承認",
    "dashboard.myInfo": "マイ情報",
    "dashboard.scheduleManagement": "スケジュール管理",
    "dashboard.chat": "チャット",
    "dashboard.preparing": "準備中",
    "dashboard.scheduleDesc":
      "まもなくスケジュールの確認と管理ができるようになります。",
    "dashboard.chatDesc":
      "チームメンバーとリアルタイムでコミュニケーションを取りましょう。",
    "dashboard.thisWeekSchedule": "今週のスケジュール",
    "dashboard.currentWeekWork": "今週の勤務スケジュール",
    "dashboard.totalHours": "総勤務時間",
    "dashboard.pendingRequests": "保留中のリクエスト",
    "dashboard.shiftSwapRequests": "承認待ちのシフト交換リクエスト",
    "dashboard.needAttention": "処理が必要",
    "dashboard.activeStores": "アクティブ店舗",
    "dashboard.operatingStores": "現在運営中の店舗数",
    "dashboard.operatingNormally": "正常運営",
    "dashboard.reviewRequests": "シフト交換リクエスト確認",
    "dashboard.teamChat": "チームチャット",
    "admin.title": "システム管理ダッシュボード",
    "admin.description":
      "プラットフォーム全体の運営状況と異常兆候を統合管理します。",
    "admin.console.badge": "System Console",
    "admin.console.title": "運用管理コンソール",
    "admin.console.description":
      "システム状態、異常兆候、監査活動を運用者向けコンソールで追跡します。",
    "admin.console.account": "Admin Account",
    "admin.console.surfaceLabel": "Admin Surface",
    "admin.console.kicker": "Platform operations and audit mode",
    "admin.loadError": "管理者データの読み込みに失敗しました。",
    "admin.login.badge": "管理者専用アクセス",
    "admin.login.title": "システム運営アカウントでログイン",
    "admin.login.description":
      "運営状況、監査ログ、異常兆候対応は管理者権限アカウントのみ利用できます。",
    "admin.login.loginCard": "管理者ログイン",
    "admin.login.cardDescription":
      "承認済みのプラットフォーム管理者アカウントでログインしてください。",
    "admin.login.helper":
      "通常の店舗アカウントであれば、一般ログインページを利用してください。",
    "admin.login.goToUserLogin": "一般ログインへ移動",
    "admin.login.accessDenied":
      "このアカウントにはシステム管理者権限がないため、管理コンソールにログインできません。",
    "admin.login.panelOneLabel": "Access",
    "admin.login.panelOneValue": "プラットフォーム運営、監査、異常対応",
    "admin.login.panelTwoLabel": "Security",
    "admin.login.panelTwoValue":
      "管理者権限のないアカウントは管理コンソールに入れません。",
    "admin.readOnly": "閲覧専用",
    "admin.period": "期間",
    "admin.period.today": "今日",
    "admin.period.7d": "直近7日",
    "admin.period.30d": "直近30日",
    "admin.period.from": "開始日",
    "admin.period.to": "終了日",
    "admin.searchPlaceholder": "ユーザー・店舗・イベント検索",
    "admin.exportCurrent": "現在タブをCSV出力",
    "admin.filters.apply": "適用",
    "admin.filters.reset": "リセット",
    "admin.tab.overview": "概要",
    "admin.tab.users": "ユーザー",
    "admin.tab.stores": "店舗",
    "admin.tab.anomalies": "異常兆候",
    "admin.tab.logs": "監査ログ",
    "admin.overview.description":
      "主要KPIと運用フローを素早く把握するための概要画面です。",
    "admin.overview.trendsDescription":
      "選択期間における主要アクティビティと運用変化量です。",
    "admin.kpi.totalUsers": "総ユーザー",
    "admin.kpi.activeUsers": "アクティブユーザー",
    "admin.kpi.activeStores": "稼働店舗",
    "admin.kpi.coverage": "割当カバレッジ",
    "admin.trends.title": "日次トレンド",
    "admin.trends.date": "日付",
    "admin.trends.users": "新規ユーザー",
    "admin.trends.stores": "新規店舗",
    "admin.trends.invitations": "招待生成",
    "admin.trends.assignments": "割当生成",
    "admin.users.title": "ユーザー状況",
    "admin.users.description":
      "プラットフォーム利用者、管理権限分布、最近の活動を一覧で確認します。",
    "admin.users.summaryDescription":
      "現在の条件に一致するユーザー一覧とプラットフォーム権限の状態です。",
    "admin.users.inactiveSummary": "非アクティブユーザー",
    "admin.users.email": "メール",
    "admin.users.name": "名前",
    "admin.users.platformRole": "プラットフォーム権限",
    "admin.users.status": "状態",
    "admin.users.lastSignIn": "最終ログイン",
    "admin.users.storeRoles": "店舗ロール",
    "admin.users.activeStoreRoles": "有効",
    "admin.users.inactiveStoreRoles": "無効",
    "admin.users.allStatuses": "すべての状態",
    "admin.users.allRoles": "すべての権限",
    "admin.users.emptyDescription":
      "現在の条件に一致するユーザーはありません。",
    "admin.stores.title": "店舗状況",
    "admin.stores.description":
      "稼働中・保管中の店舗と運用リスク兆候を確認します。",
    "admin.stores.summaryDescription":
      "店舗運営密度と直近30日の活動量をまとめて把握します。",
    "admin.stores.name": "店舗名",
    "admin.stores.status": "状態",
    "admin.stores.owner": "オーナー",
    "admin.stores.members": "メンバー数",
    "admin.stores.managers": "管理者",
    "admin.stores.pendingInvitations": "保留中招待",
    "admin.stores.assignments30d": "直近30日割当",
    "admin.stores.allStatuses": "すべての状態",
    "admin.stores.emptyDescription":
      "現在の条件に一致する店舗はありません。",
    "admin.stores.risk": "リスク",
    "admin.anomalies.title": "異常兆候",
    "admin.anomalies.description":
      "運用検知ルールで生成された異常兆候を確認し、対応します。",
    "admin.anomalies.summaryDescription":
      "重大度と状態で並び替えて優先対応対象を見つけます。",
    "admin.anomalies.rule": "ルール",
    "admin.anomalies.severity": "重大度",
    "admin.anomalies.status": "状態",
    "admin.anomalies.metric": "指標/基準",
    "admin.anomalies.detectedAt": "検知時刻",
    "admin.anomalies.actions": "対応",
    "admin.anomalies.ack": "確認",
    "admin.anomalies.resolve": "解決",
    "admin.anomalies.allStatuses": "すべての状態",
    "admin.anomalies.allSeverities": "すべての重大度",
    "admin.anomalies.emptyDescription":
      "現在の条件に一致する異常兆候はありません。",
    "admin.anomalies.updateError": "異常兆候の状態更新に失敗しました。",
    "admin.logs.title": "監査ログ",
    "admin.logs.description":
      "プラットフォームおよび店舗単位の管理操作とシステムイベントを検索します。",
    "admin.logs.summaryDescription":
      "重大度とイベント種別で監査ログを素早く追跡します。",
    "admin.logs.scope": "範囲",
    "admin.logs.eventType": "イベント",
    "admin.logs.actor": "実行者",
    "admin.logs.severity": "重大度",
    "admin.logs.createdAt": "作成時刻",
    "admin.logs.allSeverities": "すべての重大度",
    "admin.logs.eventTypePlaceholder": "イベントタイプを入力",
    "admin.logs.emptyDescription":
      "現在の条件に一致する監査ログはありません。",
    "admin.pagination.prev": "前へ",
    "admin.pagination.next": "次へ",
    "admin.empty.title": "表示できるデータがありません",
    "admin.empty.description":
      "フィルターを調整するか、期間を広げて再度確認してください。",
    "admin.role.none": "なし",
    "admin.status.active": "有効",
    "admin.status.inactive": "無効",
    "admin.status.archived": "保管済み",
    "admin.risk.high": "高",
    "admin.risk.medium": "中",
    "admin.risk.low": "低",
    "admin.severity.high": "高",
    "admin.severity.medium": "中",
    "admin.severity.low": "低",
    "admin.anomalyStatus.open": "オープン",
    "admin.anomalyStatus.ack": "確認済み",
    "admin.anomalyStatus.resolved": "解決済み",
    "admin.logs.scope.platform": "プラットフォーム",
    "admin.logs.scope.store": "店舗",
    "admin.anomaly.inviteFailure.title": "招待失敗が急増",
    "admin.anomaly.inviteFailure.description":
      "前期間と比べて招待失敗/期限切れ件数が急増しました。",
    "admin.anomaly.deactivation.title": "ユーザー無効化が急増",
    "admin.anomaly.deactivation.description":
      "前期間と比べてユーザー無効化件数が急増しました。",
    "admin.anomaly.assignmentGap.title": "スケジュール欠損が増加",
    "admin.anomaly.assignmentGap.description":
      "キャンセルされた割当が増え、カバレッジ低下のリスクがあります。",
    "admin.anomaly.roleChange.title": "権限変更が急増",
    "admin.anomaly.roleChange.description":
      "権限付与/回収アクションが前期間と比べて急増しました。",
    // 招待関連
    "invites.form.title": "チームメンバー招待",
    "invites.form.send": "招待を送信",
    "invites.form.sending": "招待を作成中...",
    "invites.form.storeRequired": "店舗を選択してください",
    "invites.form.invitedDescription": "{email}に招待を送信しました",
    "invites.form.store": "店舗選択",
    "invites.form.email": "メールアドレス",
    "invites.form.role": "役割",
    "invites.list.title": "招待リスト",
    "invites.status.pending": "待機中",
    "invites.status.accepted": "承認済み",
    "invites.status.cancelled": "キャンセル済み",
    "invites.status.expired": "期限切れ",
    "invites.role.master": "マスター管理者",
    "invites.role.subManager": "サブ管理者",
    "invites.role.partTimer": "アルバイト",
    // 初期関連翻訳 (日本語)
    "invite.title": "招待管理",
    "invite.description": "店舗に新しいメンバーを招待します",
    "invite.createInvite": "招待作成",
    "invite.email": "メールアドレス",
    "invite.emailPlaceholder":
      "招待するユーザーのメールアドレスを入力してください",
    "invite.storePlaceholder": "店舗を選択してください",
    "invite.role": "役割",
    "invite.rolePlaceholder": "付与する役割を選択してください",
    "invite.roleDescription.subManager":
      "スケジュール管理、交代承認、アルバイト招待が可能です",
    "invite.roleDescription.partTimer":
      "スケジュール確認、交代申請が可能です",
    "invite.message": "メッセージ (任意)",
    "invite.messagePlaceholder": "招待メッセージを入力してください",
    "invite.send": "招待送信",
    "invite.sending": "送信中...",
    "invite.success": "招待送信完了",
    "invite.successDescription": "招待が正常に送信されました",
    "invite.error": "招待送信失敗",
    "invite.pending": "待機中",
    "invite.accepted": "承認済み",
    "invite.expired": "期限切れ",
    "invite.cancelled": "キャンセル済み",
    "invite.resend": "再送信",
    "invite.cancel": "キャンセル",
    "invite.noInvites": "招待がありません",
    "invite.expiresAt": "有効期限",
    "invite.invitedAt": "招待日",
    "invite.status": "ステータス",
    "invite.actions": "操作",
    "invite.create": "招待作成",
    "invite.name": "名前",
    "invite.partTimer": "アルバイト",
    "invite.subManager": "サブ管理者",
    "invite.days": "日",
    "invite.createSuccess": "招待が正常に作成されました",
    "invite.createError": "招待作成に失敗しました",
    "invite.loadError": "招待一覧の読み込みに失敗しました",
    "invite.linkCopied": "リンクをコピーしました",
    "invite.linkCopiedDescription":
      "招待リンクがクリップボードにコピーされました",
    "invite.copyError": "リンクのコピーに失敗しました",
    "invite.empty": "まだ招待がありません",
    "invite.emptyDescription": "新しいチームメンバーを招待しましょう",
    "invite.accessDenied": "この店舗へのアクセス権限がありません",
    "invite.managePermissionDenied":
      "この店舗の管理権限がありません",
    "invite.cancelUsed": "使用済みの招待はキャンセルできません",
    "invite.alreadyCancelled": "この招待は既にキャンセルされています",
    "invite.duplicateActiveInvite":
      "このメールアドレスには有効な招待が既に存在します",
    "invite.tokenCreateError": "招待トークンの生成に失敗しました",
    "invite.storeIdRequired": "店舗IDが必要です",
    "invite.listLoadError": "招待一覧の取得に失敗しました",
    "invite.resendSuccess": "招待が再送信されました",
    "invite.resendError": "招待再送信に失敗しました",
    "invite.cancelSuccess": "招待がキャンセルされました",
    "invite.cancelError": "招待キャンセルに失敗しました",
    "invite.cancelPermissionDenied": "招待をキャンセルする権限がありません",
    "invite.resendPermissionDenied": "招待を再送信する権限がありません",
    "invite.userDeleteFailed": "既存ユーザーの削除に失敗しました",
    "invite.userDeleteRetryFailed":
      "ユーザーの削除に失敗しました。しばらくしてから再試行してください。",
    "invite.emailSendFailed": "メール送信に失敗しました",
    "invite.alreadyRegisteredRetry":
      "このメールアドレスは既に登録されています。しばらくしてから再試行してください。",
    "invite.resendEmailError": "メール再送信中にエラーが発生しました",
    "invite.emailRequired": "メールアドレスを入力してください",
    "invite.namePlaceholder": "山田太郎",
    "invite.guestNoEmailDescription":
      "ゲストユーザーはメールアドレスなしで登録できます。",
    "invite.expiresIn": "招待リンク有効期限",
    "invite.list": "招待リスト",
    "invite.registerWithoutEmail": "メールなしで登録 (ゲストユーザー)",
    "invite.refresh": "更新",
    "invite.mode.email": "メール招待",
    "invite.mode.guest": "ゲスト登録",
    "invite.mode.import": "他店舗ユーザー",
    "invite.importSourceStore": "取り込む店舗",
    "invite.importCandidates": "取り込み対象ユーザー",
    "invite.importSourceRole": "元店舗の役割",
    "invite.importEmpty": "取り込めるユーザーがいません",
    "invite.importLoadError": "他店舗ユーザー一覧の読み込みに失敗しました",
    "invite.importSelectionRequired": "取り込むユーザーを選択してください",
    "invite.importSubmit": "現在の店舗に登録",
    "invite.importSuccess": "他店舗ユーザーの登録が完了しました",
    "invite.importError": "他店舗ユーザーの登録に失敗しました",
    "invite.importNoSourceStores": "取り込み可能な他の管理店舗がありません",
    "invite.importResultImported": "登録",
    "invite.importResultAlreadyExists": "既に登録済み",
    "invite.importResultSkipped": "スキップ",
    // ロール管理関連
    "role.change.success": "ロールが正常に変更されました",
    "role.change.description": "ユーザーのロールが更新されました",
    "role.change.error": "ロール変更に失敗しました",
    "role.change.errorDescription": "ロール変更中にエラーが発生しました",
    "role.management.title": "ロール管理",
    "role.management.description": "ユーザーのロールを変更できます",
    "role.management.userInfo": "ユーザー情報",
    "role.management.currentRole": "現在のロール",
    "role.management.newRole": "新しいロール",
    "role.management.selectRole": "ロールを選択してください",
    "role.management.current": "現在",
    "role.management.noPermission": "ロールを変更する権限がありません",
    "role.management.changing": "変更中...",
    "role.management.changeRole": "ロール変更",
    "role.management.userList": "ユーザー一覧",
    "role.changePermissionDenied": "ロールを変更する権限がありません",
    "role.grantPermissionDenied": "ロールを付与する権限がありません",
    "role.revokePermissionDenied": "ロールを取り消す権限がありません",
    // 店舗関連
    "store.noStores": "店舗なし",
    "store.loading": "店舗読み込み中...",
    "store.selectStore": "店舗選択",
    "store.noStoreSelected": "選択された店舗なし",
    "store.totalStores": "合計{count}店舗",
    "store.role.master": "マスター",
    "store.role.sub_manager": "サブマネージャー",
    "store.role.part_timer": "アルバイト",
    "store.createFirst": "最初の店舗を作成",
    "store.name": "店舗名",
    "store.namePlaceholder": "店舗名を入力してください",
    "store.description": "説明",
    "store.descriptionPlaceholder": "店舗の説明を入力してください",
    "store.address": "住所",
    "store.addressPlaceholder": "店舗の住所を入力してください",
    "store.phone": "電話番号",
    "store.phonePlaceholder": "店舗の電話番号を入力してください",
    "store.timezone": "タイムゾーン",
    "store.timezonePlaceholder": "タイムゾーンを選択してください",
    "store.createTitle": "店舗作成",
    "store.createDescription": "新しい店舗を作成します",
    "store.editTitle": "店舗編集",
    "store.edit": "店舗編集",
    "store.editDescription": "店舗情報を編集します",
    "store.createButton": "店舗作成",
    "store.updateButton": "店舗更新",
    "store.createSuccess": "店舗が正常に作成されました",
    "store.createError": "店舗の作成に失敗しました",
    "store.updateSuccess": "店舗が正常に更新されました",
    "store.updateError": "店舗の更新に失敗しました",
    "store.saveError": "保存に失敗しました",
    "store.networkError": "ネットワークエラーが発生しました",
    "common.email": "メール",
    "common.name": "名前",
    "common.cancel": "キャンセル",
    "common.save": "保存",
    "common.edit": "編集",
    "common.delete": "削除",
    "common.confirm": "確認",
    "common.loading": "読み込み中...",
    "common.yes": "はい",
    "common.no": "いいえ",
    "common.update": "更新",
    "common.create": "作成",
    "common.add": "追加",
    "common.info": "情報",
    "common.error": "エラー",
    "common.unknownError": "不明なエラーが発生しました",
    "common.success": "成功",
    "common.back": "戻る",
    "common.view": "表示",
    "common.close": "閉じる",
    "common.noData": "データがありません",
    "store.noDescription": "説明なし",
    "store.loadError": "店舗情報の読み込みに失敗しました",
    "store.notFound": "店舗が見つかりません",
    "store.notFoundDescription":
      "要求された店舗が存在しないか、アクセス権がありません",
    "store.information": "店舗情報",
    "store.informationDescription": "店舗の基本情報を確認できます",
    "store.status": "状態",
    "store.status.active": "アクティブ",
    "store.status.archived": "アーカイブ済み",
    "store.createdInfo": "作成情報",
    "store.createdAt": "作成日",
    "store.updatedAt": "更新日",
    "store.quickActions": "クイックアクション",
    "store.manageUsers": "ユーザー管理",
    "store.viewSchedule": "スケジュール表示",
    "store.settings": "設定",
    "store.noPermission": "この店舗を編集する権限がありません",
    // 사용자 관리 관련 번역
    "user.management": "ユーザー管理",
    "user.managementForStore": "{storeName} 店舗ユーザー管理",
    "user.members": "メンバー",
    "user.total": "名",
    "user.name": "名前",
    "user.email": "メール",
    "user.role": "役割",
    "user.status": "状態",
    "user.joinedAt": "参加日",
    "user.temporaryWork": "臨時勤務",
    "user.actions": "操作",
    "user.noName": "名前なし",
    "user.defaultStore": "デフォルト店舗",
    "user.noMembers": "メンバーがいません",
    "user.filters": "フィルター",
    "user.search": "検索",
    "user.searchPlaceholder": "名前またはメールアドレスで検索",
    "user.allRoles": "すべての役割",
    "user.allStatuses": "すべての状態",
    "user.clearFilters": "フィルターをクリア",
    "user.invite": "招待",
    "user.temporaryAssign": "臨時勤務配置",
    "user.startDate": "開始日",
    "user.endDate": "終了日",
    "user.reason": "理由",
    "user.reasonPlaceholder": "臨時勤務配置の理由を入力してください",
    "user.assign": "配置",
    "user.roleGranted": "役割が付与されました",
    "user.roleGrantedDescription": "ユーザーに役割が正常に付与されました",
    "user.roleGrantError": "役割付与に失敗しました",
    "user.roleRevoked": "役割が回収されました",
    "user.roleRevokedDescription": "ユーザーの役割が正常に回収されました",
    "user.roleRevokeError": "役割回収に失敗しました",
    "user.demoteSubManager": "サブマネージャー降格",
    "user.demoteSubManagerDescription":
      "サブマネージャーをアルバイトに変更します",
    "user.demoteSubManagerError": "サブマネージャーの降格に失敗しました",
    "user.deactivateUser": "ユーザー無効化",
    "user.deactivateUserDescription": "このユーザーを無効化します",
    "user.deactivateUserError": "ユーザーの無効化に失敗しました",
    "user.reactivateUser": "ユーザー再有効化",
    "user.reactivateUserDescription": "このユーザーを再有効化します",
    "user.reactivateUserError": "ユーザーの再有効化に失敗しました",
    "user.deleteUser": "ユーザー削除",
    "user.deleteUserDescription":
      "このユーザーを削除します（スケジュール履歴は保持されます）",
    "user.deleteUserError": "ユーザーの削除に失敗しました",
    "user.notFound": "ユーザーが見つかりません",
    "user.alreadyInactive": "既に無効化されているユーザーです",
    "user.alreadyActive": "既に有効化されているユーザーです",
    "user.deletePermissionDenied": "ユーザーを削除する権限がありません",
    "user.deactivatePermissionDenied": "ユーザーを無効化する権限がありません",
    "user.reactivatePermissionDenied":
      "ユーザーを再有効化する権限がありません",
    "user.cannotDeleteSelf": "自分自身を削除することはできません",
    "user.cannotDeleteMaster":
      "マスターロールを持つユーザーは削除できません",
    "user.roleNotFound": "ユーザーのロールが見つかりません",
    "user.onlySubManagerCanDemote":
      "サブマネージャーのみアルバイトに変更できます",
    "user.onlyActiveSubManagerCanDemote":
      "有効状態のサブマネージャーのみ変更できます",
    "user.deleteSuccessWithHistory":
      "ユーザーを削除しました。スケジュール履歴は保持されます。",
    "user.temporaryAssigned": "臨時勤務配置が完了しました",
    "user.temporaryAssignedDescription": "臨時勤務が正常に配置されました",
    "user.temporaryAssignError": "臨時勤務配置に失敗しました",
    // ユーザー詳細ページ関連の翻訳
    "user.detail": "ユーザー詳細",
    "user.detailForStore": "{storeName} 店舗 - {userName} 詳細",
    "user.basicInfo": "基本情報",
    "user.jobRoles": "職務役割",
    "user.jobRoles.none": "設定された職務役割がありません。",
    "user.workPreferences": "勤務希望",
    "user.resignationDate": "退職予定日",
    "user.desiredWeeklyHours": "希望週勤務時間",
    "user.desiredWeeklyHoursPlaceholder":
      "週勤務時間を入力してください（例：20）",
    "user.desiredDailyHours": "希望日勤務時間",
    "user.desiredDailyHoursPlaceholder":
      "1日に働きたい時間を入力してください（例：6.5）",
    "user.noResignationDate": "退職予定日なし",
    "user.setResignationDate": "退職予定日を設定",
    "user.removeResignationDate": "退職予定日を削除",
    "user.updateProfile": "プロフィール更新",
    "user.profileUpdated": "プロフィール更新完了",
    "user.profileUpdatedDescription":
      "ユーザープロフィールが正常に更新されました",
    "user.profileUpdateError": "プロフィール更新失敗",
    "user.viewDetail": "詳細表示",
    "user.backToUserList": "ユーザー一覧に戻る",
    "user.guest": "ゲスト",
    "user.normal": "一般",
    "user.deactivate": "無効化",
    "user.reactivate": "再有効化",
    "user.difficultWeekday.updateError": "出勤不可曜日の更新に失敗しました",
    "user.difficultWeekday.deleteError": "出勤不可曜日の削除に失敗しました",
    "user.difficultWeekday.insertError": "出勤不可曜日の追加に失敗しました",
    // プロフィールページ関連の翻訳
    "profile.title": "プロフィール",
    "profile.myProfile": "マイプロフィール",
    "profile.personalInfo": "個人情報",
    "profile.accountInfo": "アカウント情報",
    "profile.changePassword": "パスワード変更",
    "profile.currentPassword": "現在のパスワード",
    "profile.newPassword": "新しいパスワード",
    "profile.confirmPassword": "パスワード確認",
    "profile.passwordChanged": "パスワード変更完了",
    "profile.passwordChangedDescription": "パスワードが正常に変更されました",
    "profile.passwordChangeError": "パスワード変更失敗",
    "profile.currentPasswordRequired": "現在のパスワードを入力してください",
    "profile.newPasswordRequired": "新しいパスワードを入力してください",
    "profile.passwordMismatch": "パスワードが一致しません",
    "profile.passwordTooShort": "パスワードは最低6文字以上である必要があります",
    "profile.saveChanges": "変更を保存",
    "profile.cancel": "キャンセル",
    "profile.daysRemaining": "残り日数",
    "profile.resignationPassed": "退職日が過ぎました",
    "profile.resignationToday": "今日が退職日です",
    "profile.resignationTomorrow": "明日が退職日です",
    "profile.daysLeft": "日残り",
    "user.status.active": "アクティブ",
    "user.status.pending": "待機中",
    "user.status.inactive": "非アクティブ",
    "user.alreadyDeleted": "既に削除されたユーザー",
    "user.alreadyDeletedDescription": "このユーザーは既に削除されています。",
    "user.promoteToSubManager": "昇格",
    "user.confirmPromoteMessage":
      "このユーザーをサブマネージャーに昇格させますか？",
    "user.adminActions": "管理者アクション",
    "user.guestUser": "ゲストユーザー",
    // 초대 수락 페이지
    "invite.accept.title": "招待受諾",
    "invite.accept.description":
      "店舗に参加するためにアカウントを作成してください",
    "invite.accept.storeInfo": "店舗情報",
    "invite.accept.storeName": "店舗名",
    "invite.accept.role": "役割",
    "invite.accept.expiresAt": "有効期限",
    "invite.accept.accountInfo": "アカウント情報",
    "invite.accept.name": "名前",
    "invite.accept.namePlaceholder": "名前を入力してください",
    "invite.accept.password": "パスワード",
    "invite.accept.passwordPlaceholder": "パスワードを入力してください",
    "invite.accept.confirmPassword": "パスワード確認",
    "invite.accept.confirmPasswordPlaceholder":
      "パスワードを再入力してください",
    "invite.accept.accept": "招待受諾",
    "invite.accept.accepting": "受諾中...",
    "invite.accept.success": "招待受諾完了",
    "invite.accept.successDescription": "店舗に正常に参加しました",
    "invite.accept.error": "招待受諾失敗",
    "invite.accept.invalidToken": "無効な招待",
    "invite.accept.expiredToken": "期限切れの招待",
    "invite.accept.loading": "招待情報を読み込み中...",
    "invite.accept.existingUser": "既存ユーザー",
    "invite.accept.existingUserMessage":
      "このメールアドレスは既に登録されています。ログインして招待を受諾してください。",
    "invite.accept.loginToAccept": "ログインして招待受諾",
    "invite.accept.emailMismatch":
      "招待されたメールアドレスと現在ログイン中のメールアドレスが一致しません",
    "invite.accept.alreadyMember": "既にこの店舗のメンバーです",
    "invite.accept.processingError": "招待受諾処理中にエラーが発生しました",
    "invite.accept.tokenRequired": "招待トークンが必要です",
    "invite.accept.notFound": "招待が見つかりません",
    "invite.accept.status.valid": "有効な招待です",
    "invite.accept.status.cancelled": "キャンセルされた招待です",
    "invite.accept.status.used": "既に使用された招待です",
    "invite.accept.status.expired": "期限切れの招待です",
    "invite.accept.joinedRoleMessage":
      "{storeName}に{roleName}として正常に参加しました",
    "invite.accept.signInFailed":
      "ログイン失敗: {details}。デフォルトパスワード(1q2w3e4r!)をお試しください。",
    "invite.accept.userCreateFailed": "ユーザーの作成に失敗しました",
    "invite.accept.storeAddFailed": "店舗への追加に失敗しました",
    "invite.setupPassword.title": "パスワード設定",
    "invite.setupPassword.description":
      "{storeName}への招待を受諾するためにパスワードを設定してください。",
    "invite.setupPassword.unknownStore": "不明な店舗",
    "invite.setupPassword.newPassword": "新しいパスワード",
    "invite.setupPassword.passwordPlaceholder": "8文字以上",
    "invite.setupPassword.passwordPlaceholderMin6": "6文字以上",
    "invite.setupPassword.submit": "パスワード設定して招待受諾",
    "invite.setupPassword.successTitle": "パスワード設定完了",
    "invite.setupPassword.failureTitle": "パスワード設定に失敗しました",
    "invite.setupPassword.loginRequiredTitle": "ログインが必要です",
    "invite.setupPassword.loginRequired":
      "招待リンクからログインしてください。",
    "invite.setupPassword.nameRequiredTitle": "名前を入力してください",
    "invite.setupPassword.nameRequiredDescription": "名前は必須項目です。",
    "invite.setupPassword.passwordMismatchTitle":
      "パスワードが一致しません",
    "invite.setupPassword.passwordMismatchDescription":
      "パスワードをもう一度確認してください。",
    "invite.setupPassword.passwordTooShortTitle":
      "パスワードが短すぎます",
    "invite.setupPassword.passwordTooShortDescription":
      "8文字以上で入力してください。",
    "invite.setupPassword.passwordTooShortMin6Description":
      "6文字以上で入力してください。",
    "invite.setupPassword.loadError":
      "情報の読み込み中にエラーが発生しました。",
    "invite.setupPassword.invitationLoadError":
      "招待情報の読み込みに失敗しました。",
    "invite.setupPassword.userLoadError":
      "ユーザー情報の確認中にエラーが発生しました。",
    "invite.setupPassword.checkingUser": "ユーザー情報を確認中...",
    "invite.error.title": "招待リンクエラー",
    "invite.error.processing":
      "招待リンクの処理中にエラーが発生しました。もう一度お試しください。",
    "invite.error.requestNew": "新しい招待をリクエスト",
    "invite.error.requestNewDescription":
      "管理者に新しい招待を依頼してください。",
    "invite.error.message.otpExpired": "メールリンクの有効期限が切れました。",
    "invite.error.message.accessDenied": "アクセスが拒否されました。",
    "invite.error.message.invalidInvite": "招待リンクに問題があります。",
    "invite.error.description.default":
      "招待リンクをもう一度確認してください。",
    "invite.error.description.otpExpired":
      "招待リンクの有効期限が切れています。新しい招待を依頼してください。",
    "invite.error.description.accessDenied":
      "この招待リンクにアクセスする権限がありません。管理者にお問い合わせください。",
    "invite.error.description.invalidInvite":
      "招待リンクが無効か破損しています。",
    "invite.error.code": "エラーコード",
    "invite.error.type": "エラータイプ",
    "invite.error.detail": "詳細",
    "invite.error.goHome": "ホームに戻る",
    "invite.verify.title": "メール認証",
    "invite.verify.description": "メール認証が完了しました。",
    "invite.verify.verifying": "認証確認中...",
    "invite.verify.completedTitle": "メール認証完了",
    "invite.verify.processedTitle": "招待処理完了",
    "invite.verify.processedDescription":
      "既に処理済みの招待、または期限切れの招待です。",
    "invite.verify.sessionNotFound": "セッション情報が見つかりません。",
    "invite.verify.userNotFound": "ユーザー情報が見つかりません。",
    "invite.verify.checkError": "認証確認中にエラーが発生しました。",
    "invite.verify.passwordSetAndAccepted":
      "パスワードが設定され、招待が受諾されました。",
    "invite.verify.passwordSetupPrompt":
      "安全なパスワードを設定して招待を受諾してください。",
    "invite.verify.linkVerifiedPrompt":
      "招待リンクの確認が完了しました。パスワードを設定してください。",
    "invite.verify.infoTitle": "認証情報",
    "invite.verify.safePasswordNote": "安全なパスワードを設定してください。",
    "invite.verify.setPassword": "パスワードを設定",
    "invite.verify.userInfoTitle": "ユーザー情報",
    "invite.verify.newPasswordPlaceholder":
      "新しいパスワードを入力してください（8文字以上）",
    "invite.verify.error": "認証エラー",
    "invite.verify.errorDescription": "メール認証が必要です。",
    "invite.verify.acceptTitle": "招待受諾",
    "invite.verify.acceptDescription":
      "店舗招待を受諾してサービスをご利用ください。",
    "invite.verify.loginInfo": "ログイン情報",
    "invite.verify.defaultPassword": "デフォルトパスワード",
    "invite.verify.passwordChangeNote":
      "ログイン後、設定でパスワードを変更してください。",
    "invite.verify.acceptButton": "招待を受諾",
    "invite.verify.accepting": "受諾中...",
    "invite.verify.success":
      "招待が正常に受諾されました。ログインしてサービスをご利用ください。",
    "invite.verify.acceptError": "招待の受諾に失敗しました。",
    "dashboard.passwordChangeRequired": "パスワード変更が必要です",
    "dashboard.passwordChangeDescription":
      "セキュリティのため、デフォルトパスワードを変更してください。",
    "dashboard.passwordChangeButton": "パスワード変更",
    // settings.store (編集タブ)
    "settings.store.title": "店舗設定",
    "settings.store.basic": "基本情報",
    "settings.store.hours": "営業時間・休業日",
    "settings.store.workItems": "勤務項目",
    "settings.store.roles": "職務管理",
    "settings.store.targets": "カバレッジ",
    "settings.store.breakRules": "休憩ルール",
    "settings.store.policies": "運用ルール",
    "settings.store.notifications": "通知",
    "settings.store.saveSuccess": "保存しました",
    "settings.store.saveError": "保存に失敗しました",
    "settings.store.validationError": "検証エラー",
    "settings.store.open": "開始時間",
    "settings.store.close": "終了時間",
    "settings.store.holidays": "休業日",
    "settings.store.date": "日付",
    "settings.store.policiesDescription":
      "運営に必要な核心的な規則のみ設定します。複雑な規則は削除して使いやすくしました。",
    "settings.store.currencyUnitDesc":
      "人件費予算および給与計算に使用される通貨単位",
    "settings.store.publishCutoffDesc":
      "スケジュール公開前の最小時間（時間単位）",
    "settings.store.swapLeadTimeDesc":
      "シフト交換リクエストの最小リードタイム（時間単位）",
    "settings.store.minRestDesc": "連続勤務間の最小休息時間",
    "settings.store.maxHoursPerDayDesc": "1日の最大勤務時間（0 = 制限なし）",
    "settings.store.maxHoursPerWeekDesc": "週の最大勤務時間（0 = 制限なし）",
    "settings.store.maxHoursPerMonthDesc":
      "月の最大勤務時間（デフォルト: 160時間）",
    "settings.store.maxConsecutiveDaysDesc": "連続勤務最大日数（0 = 制限なし）",
    "settings.store.weeklyBudgetDesc":
      "週間人件費予算（{currency}単位、0 = 制限なし）",
    // workItems
    "workItems.title": "勤務項目",
    "workItems.name": "名称",
    "workItems.start": "開始(分)",
    "workItems.end": "終了(分)",
    "workItems.break": "無給休憩(分)",
    "workItems.maxHeadcount": "最大人数",
    "workItems.roleHint": "役割",
    "workItems.roles": "職務管理",
    "workItems.create": "勤務項目追加",
    "workItems.description": "店舗で必要な勤務項目を管理します",
    "workItems.namePlaceholder": "勤務項目名を入力してください",
    "workItems.timeSettings": "時間設定",
    "workItems.startTime": "開始時間",
    "workItems.endTime": "終了時間",
    "workItems.nextDay": "翌日",
    "workItems.nextDayLabel": "翌日まで",
    "workItems.roleRequirements": "職務要件",
    "workItems.availableJobs": "利用可能な職務",
    "workItems.selectedJobs": "選択された職務",
    "workItems.noRolesSelected": "選択された職務がありません",
    "workItems.list": "勤務項目一覧",
    "workItems.listDescription": "現在この店舗で使用中の勤務項目です",
    "workItems.noItems": "勤務項目が見つかりません",
    "workItems.editNotImplemented": "編集機能は今後実装予定です",
    "workItems.deleteNotImplemented": "削除機能は今後実装予定です",
    "workItems.edit": "編集",
    "workItems.delete": "削除",
    "workItems.editTitle": "勤務項目編集",
    "workItems.deleteConfirm": "この勤務項目を削除しますか？",
    "workItems.deleteConfirmDescription":
      "削除された勤務項目は復元できません。",
    "workItems.updateSuccess": "勤務項目が正常に更新されました",
    "workItems.deleteSuccess": "勤務項目が正常に削除されました",
    // 運営規則 (Policies)
    "policies.publish_cutoff_hours": "スケジュール発行締切時間",
    "policies.swap_lead_time_hours": "交代リクエスト先行時間",
    "policies.min_rest_hours_between_shifts": "勤務間最小休息時間",
    "policies.max_hours_per_day": "日最大勤務時間",
    "policies.max_hours_per_week": "週最大勤務時間",
    "policies.max_consecutive_days": "最大連続勤務日数",
    "policies.max_morning_staff": "午前最大勤務人数",
    "policies.max_morning_staff_desc":
      "午前時間帯の最大勤務人数 (0 = 制限なし)",
    "policies.min_morning_staff": "午前最小勤務人数",
    "policies.min_morning_staff_desc":
      "午前時間帯の最小勤務人数 (0 = 制限なし)",
    "policies.max_afternoon_staff": "午後最大勤務人数",
    "policies.max_afternoon_staff_desc":
      "午後時間帯の最大勤務人数 (0 = 制限なし)",
    "policies.min_afternoon_staff": "午後最小勤務人数",
    "policies.min_afternoon_staff_desc":
      "午後時間帯の最小勤務人数 (0 = 制限なし)",
    "policies.weekly_labor_budget_cents": "週間人件費予算（円）",
    "policies.schedule_unit": "スケジュール単位",
    "policies.schedule_unit_week": "週単位",
    "policies.schedule_unit_month": "月単位",
    "policies.currency_unit": "通貨単位",
    "policies.currency_krw": "韓国ウォン",
    "policies.currency_usd": "米ドル",
    "policies.currency_jpy": "日本円",
    "policies.currency_eur": "ユーロ",
    "policies.currency_cad": "カナダドル",
    "policies.currency_aud": "オーストラリアドル",
    "policies.shift_boundary_time": "午前/午後区分時間",
    "policies.shift_boundary_time_desc":
      "スケジュールグリッドで午前と午後を区別する基準時間です。この時間を基準に午前/午後の人員数が計算されます。",
    "policies.group.basicSettings": "基本設定",
    "policies.group.basicSettingsDesc": "スケジュール単位、通貨単位、時間区分などの基本設定",
    "policies.group.schedulePublishing": "スケジュール発行",
    "policies.group.schedulePublishingDesc": "スケジュール発行に関する設定",
    "policies.group.workHoursLimits": "勤務時間制限",
    "policies.group.workHoursLimitsDesc": "週間、月間勤務時間および連続勤務日数の制限",
    "policies.group.staffManagement": "人員管理",
    "policies.group.staffManagementDesc": "午前/午後時間帯別の最小/最大勤務人員設定",
    "policies.group.unusedSettings": "未使用項目",
    "policies.group.unusedSettingsDesc": "現在使用していない設定項目",
    "workItems.nameRequired": "勤務項目名が必要です",
    "workItems.endAfterStart": "終了時間は開始時間より遅くする必要があります",
    "workItems.breakTooLong": "休憩時間は勤務時間より長くすることはできません",
    "workItems.calculatedMaxHeadcount": "計算された最大人数",
    "workItems.autoCalculated": "職務別合計で自動計算",
    "workItems.maxHeadcountLimit":
      "職務ごとの最小人数合計は99を超えることはできません",
    "workItems.endTimeLimit": "終了時間は24:00以内に設定してください",
    "workItems.updateFailed": "勤務項目の更新に失敗しました",
    "workItems.createFailed": "勤務項目の作成に失敗しました",
    "workItems.timeInfo":
      "午前10時〜午前12時のように設定すると、自動的に翌日まで計算されます",
    // targets
    "targets.title": "カバレッジ目標",
    "targets.weekday": "曜日",
    "targets.start": "開始(分)",
    "targets.end": "終了(分)",
    "targets.min": "最小",
    "targets.max": "最大",
    "targets.roleHint": "役割",
    "targets.create": "目標追加",
    // rules / break
    "rules.title": "運用ルール",
    "rules.break.threshold": "しきい値(時間)",
    "rules.break.breakMin": "休憩(分)",
    "rules.break.paid": "有給",
    "rules.compliance.maxPerMonth": "月最大勤務(時間)",
    // user preferred weekdays
    "user.preferredWeekdays": "出勤不可曜日",
    "user.preferredWeekdays.description": "出勤不可曜日を選択してください",
    "user.weekdays.sunday": "日曜日",
    "user.weekdays.monday": "月曜日",
    "user.weekdays.tuesday": "火曜日",
    "user.weekdays.wednesday": "水曜日",
    "user.weekdays.thursday": "木曜日",
    "user.weekdays.friday": "金曜日",
    "user.weekdays.saturday": "土曜日",
    // schedule errors
    "schedule.errors.endAfterStart": "終了は開始より後である必要があります",
    "schedule.errors.breakTooLong": "休憩が勤務時間を超えています",
    "schedule.errors.insufficientRoleCoverage": "役割人員不足",
    "schedule.errors.roleValidationError": "役割検証エラー",
    // schedule warnings
    "schedule.warning.exceedDesiredHours": "希望週勤務時間超過警告",
    "schedule.warning.exceedDesiredHoursMessage":
      "さんの今週の総勤務時間が希望週勤務時間を超過します。",
    "schedule.warning.currentHours": "現在の勤務時間",
    "schedule.warning.newHours": "追加される勤務時間",
    "schedule.warning.totalHours": "総勤務時間",
    "schedule.warning.desiredHours": "希望週勤務時間",
    "schedule.warning.exceedBy": "超過時間",
    "schedule.warning.difficultDay": "出勤不可曜日警告",
    "schedule.warning.difficultDayMessage":
      "このスケジュールは、このユーザーの出勤不可曜日です。",
    "schedule.warning.difficultDayName": "出勤不可曜日",
    "schedule.warning.maxStaff": "最大勤務人数超過警告",
    "schedule.warning.maxStaffMessage":
      "このスケジュールを追加すると、該当時間帯の最大勤務人数を超過します。",
    "schedule.warning.maxMorningStaff": "午前最大人数",
    "schedule.warning.maxAfternoonStaff": "午後最大人数",
    "schedule.warning.currentMorningStaff": "現在の午前人数",
    "schedule.warning.currentAfternoonStaff": "現在の午後人数",
    "schedule.warning.unavailable": "出勤不可警告",
    "schedule.warning.unavailableMessage":
      "さんはこの日付に出勤不可としてマークされています。",
    "schedule.warning.crossStoreConflict": "他店舗勤務の競合",
    "schedule.warning.crossStoreConflictMessage":
      "他店舗の勤務時間と重複しているため、この店舗には登録できません。",
    "schedule.warning.transferDesiredHours":
      "スケジュール移転 - 希望週勤務時間超過警告",
    "schedule.warning.transferDesiredHoursMessage":
      "このスケジュールを移転すると、さんの今週の総勤務時間が希望週勤務時間を超過します。",
    "schedule.warning.transferDifficultDay":
      "スケジュール移転 - 出勤不可曜日警告",
    "schedule.warning.transferDifficultDayMessage":
      "このスケジュールは、対象ユーザーの出勤不可曜日です。",
    // schedule management
    "schedule.title": "スケジュール管理",
    "schedule.noStoreSelected": "店舗を選択してください",
    "schedule.export": "エクスポート",
    "schedule.currentWeek": "今週",
    "schedule.weekOfMonth": "{month}月 第{week}週",
    "schedule.weekRangeWithMeta": "{weekMeta}（{range}）",
    "schedule.weekView": "週表示",
    "schedule.monthView": "月表示",
    "schedule.weekGrid": "週グリッド",
    "schedule.noData": "スケジュールデータがありません",
    "schedule.noDataDescription":
      "まだスケジュールが作成されていません。最初のスケジュールを作成してください。",
    "schedule.createFirst": "スケジュール作成",
    "schedule.monthViewComingSoon": "月表示準備中",
    "schedule.monthViewDescription": "月間カレンダー表示は近日提供予定です。",
    // availability calendar
    "availability.title": "休日希望カレンダー",
    "availability.markUnavailable": "休日希望日を登録",
    "availability.markAvailable": "休日希望日を解除",
    "availability.removeUnavailable": "休日希望日を解除",
    "availability.selectAction": "アクション選択",
    "availability.selectUser": "ユーザー選択",
    "availability.currentUser": "現在のユーザー",
    "availability.user": "ユーザー",
    "availability.selectUserToRemove": "解除するユーザー選択",
    "availability.confirmRemove": "解除",
    "availability.reason": "理由（任意）",
    "availability.reasonPlaceholder": "休日希望の理由を入力してください",
    "availability.confirmMarkUnavailable":
      "この日付を休日希望日として登録しますか？",
    "availability.confirmMarkAvailable":
      "この日付の休日希望日を解除しますか？",
    "availability.alreadyAssigned": "この日付は既に割り当てられています",
    "availability.cannotModifyAfterPublish": "公開後は変更できません",
    "availability.notificationTitle": "休日希望状態変更",
    "availability.notificationMessage":
      "{userName}さんが{date}の休日希望状態を{action}しました。",
    // export
    "export.title": "スケジュールエクスポート",
    "export.format": "ファイル形式",
    "export.xlsx": "Excel (XLSX)",
    "export.csv": "CSV",
    "export.png": "PNG画像",
    "export.scope": "エクスポート範囲",
    "export.scopeAll": "全スケジュール",
    "export.scopeMe": "自分のスケジュールのみ",
    "export.includePrivateInfo": "個人情報を含める",
    "export.includePrivateInfoDescription":
      "メールアドレス、電話番号などを含めます",
    "export.download": "ダウンロード",
    "export.success": "ファイルが正常に生成されました",
    "export.error": "ファイル生成に失敗しました",
    "export.advanced": "詳細設定",
    "export.fileDownloaded": "ファイルがダウンロードされました",
    "export.scopeMeOnly": "管理者のみ全スケジュールのエクスポートが可能です",
    "export.period": "期間",
    "export.exporting": "エクスポート中...",
    "export.noDataToExport": "エクスポートするデータがありません",
    "export.storeIdRequired": "店舗IDが必要です",
    "export.defaultScheduleTitle": "スケジュール",
    "export.weeklySheetTitle": "週間スケジュール",
    "export.crossStoreLegend": "他店舗勤務の凡例",
    // week grid
    "schedule.user": "ユーザー",
    "schedule.clickToAssign": "クリックして割り当て",
    "schedule.noUsers": "ユーザーがいません",
    "schedule.selectAction": "アクションを選択してください",
    "schedule.currentSchedule": "現在のスケジュール",
    "schedule.selectWorkItem": "勤務項目選択",
    "schedule.selectWorkItemPlaceholder": "勤務項目を選択してください",
    "schedule.selectWorkItemFirst": "まず勤務項目を選択してください",
    "schedule.processing": "処理中...",
    "schedule.scheduleAdded": "スケジュールが追加されました",
    "schedule.scheduleAddError": "スケジュール追加中にエラーが発生しました",
    "schedule.unavailableWarning":
      "警告: この日付にユーザーが休日希望日として登録されています。",
    "schedule.scheduleDeleted": "スケジュールが削除されました",
    "schedule.scheduleDeleteError": "スケジュール削除中にエラーが発生しました",
    "schedule.noScheduleToDelete": "削除するスケジュールがありません",
    "schedule.userAlreadyHasAssignment": "既にその日付にスケジュールがあります",
    "schedule.addSchedule": "スケジュール追加/編集",
    "schedule.setAvailability": "出勤可能設定",
    "schedule.requestSwap": "シフト交換依頼",
    "schedule.deleteSchedule": "スケジュール削除",
    "schedule.transferSchedule": "他のユーザーに移転",
    "schedule.transferTo": "移転先ユーザーを選択",
    "schedule.transferToPlaceholder": "ユーザーを選択してください",
    "schedule.transferSuccess": "スケジュールが正常に移転されました",
    "schedule.transferError": "スケジュール移転中にエラーが発生しました",
    "schedule.multiDaySchedule": "複数日スケジュール登録",
    "schedule.selectDays": "日付選択",
    "schedule.weekdayPreferences.manage": "曜日希望設定",
    "schedule.weekdayPreferences.title": "曜日希望設定",
    "schedule.weekdayPreferences.description":
      "希望曜日と出勤不可曜日をまとめて設定します。",
    "schedule.weekdayPreferences.preferredDays": "出勤希望曜日",
    "schedule.weekdayPreferences.difficultDays": "出勤不可曜日",
    "schedule.weekdayPreferences.loadError":
      "曜日希望情報の読み込みに失敗しました",
    "schedule.weekdayPreferences.saveSuccess":
      "曜日希望情報を保存しました",
    "schedule.weekdayPreferences.saveError":
      "曜日希望情報の保存に失敗しました",
    "schedule.warnings": "警告",
    "schedule.register": "登録",
    "schedule.warning.multiDayMessage":
      "さんの複数日スケジュール登録時に以下の警告があります。",
    "schedule.crossStoreBadge": "他店",
    "schedule.crossStoreTooltipTitle": "他店舗勤務",
    "availability.unavailable": "休日希望日",
    "availability.currentMonth": "今月",
    "availability.selectMonth": "月を選択",
    "availability.chooseMonth": "選択する月を選んでください",
    "availability.date": "日付",
    "availability.today": "今日",
    "availability.pastDate": "過去の日付",
    "availability.pastDateDescription": "過去の日付は変更できません",
    "availability.success": "状態が正常に更新されました",
    "availability.multiDateMode": "他の日付も一緒に登録",
    "availability.selectAdditionalDates": "追加日付選択",
    "availability.selectedDates": "選択された日付",
    "availability.dateAlreadyUnavailable":
      "この日付は既に休日希望日として登録されています",
    "availability.hasTimeRestriction": "時間制限",
    "availability.timePeriod": "時間帯",
    "availability.startTime": "開始時間",
    "availability.endTime": "終了時間",
    "availability.detailsTitle": "日付詳細",
    "availability.noUnavailableDetails":
      "この日付に登録された休日希望情報はありません",
    "availability.multiDateRegisterResult":
      "{successCount}件の日付を登録しました{failText}",
    "availability.multiDateRegisterFailSuffix": " ({failCount}件失敗)",
    "availability.allDateRegisterFailed":
      "選択したすべての日付の登録に失敗しました",
    "schedule.copyWeek": "週コピー",
    "schedule.selectWeek": "週を選択",
    "schedule.chooseWeek": "選択する週を選んでください",
    "schedule.copyTargetWeek": "コピー適用対象週",
    "schedule.selectWeekToCopy": "コピーする週を選択",
    "schedule.copyPreviewTitle": "コピー元週プレビュー",
    "schedule.copyPreviewLoading": "コピー元週のスケジュールを読み込み中...",
    "schedule.copyPreviewEmpty": "コピー元週に登録済みスケジュールはありません",
    "schedule.copyPreviewLoadError": "コピー元週プレビューの読み込みに失敗しました",
    "schedule.copyPreviousWeek": "前週スケジュールコピー",
    "schedule.copyWarning": "現在の週のすべてのスケジュールが削除されます",
    "schedule.copySuccess": "スケジュールがコピーされました",
    "schedule.copyError": "スケジュールコピー中にエラーが発生しました",
    "schedule.copyConfirm": "スケジュールをコピーしますか？",
    "schedule.copyConfirmDescription":
      "現在の週のすべてのスケジュールが削除され、選択した週のスケジュールがコピーされます。",
    "schedule.noWorkItemsTitle": "勤務項目が必要です",
    "schedule.noWorkItemsDescription":
      "スケジュールを登録するには、まず勤務項目を設定する必要があります",
    "schedule.noWorkItemsMessage":
      "店舗設定で勤務項目を登録した後、スケジュールを登録できます。",
    "schedule.goToWorkItems": "勤務項目設定へ移動",
    // work item roles
    "workItemRoles.title": "勤務項目役割要件",
    "workItemRoles.description": "この勤務項目に必要な役割を設定します",
    "workItemRoles.addRole": "役割追加",
    "workItemRoles.removeRole": "役割削除",
    "workItemRoles.minCount": "最小人数",
    "workItemRoles.saveSuccess": "役割要件が正常に保存されました",
    "workItemRoles.saveFailed": "役割要件の保存に失敗しました",
    "workItemRoles.minCountMin": "最小人数は0以上である必要があります",
    "workItemRoles.minCountMax": "最小人数は99以下である必要があります",
    "workItemRoles.requirementsSet": "要件設定済み",
    "workItemRoles.currentCount": "現在: {current}名",
    "workItemRoles.requiredCount": "必要: {required}名",
    "workItemRoles.roleCoverage": "役割カバレッジ",
    "workItemRoles.insufficient": "役割人員不足",
    "workItemRoles.sufficient": "役割人員充足",
    // job roles
    "jobRoles.title": "職務管理",
    "jobRoles.description": "店舗で必要な職務を管理します",
    "jobRoles.create": "職務作成",
    "jobRoles.edit": "職務編集",
    "jobRoles.delete": "職務削除",
    "jobRoles.name": "職務名",
    "jobRoles.code": "コード",
    "jobRoles.roleDescription": "説明",
    "jobRoles.active": "有効",
    "jobRoles.inactive": "無効",
    "jobRoles.usageCount": "使用中: {count}名",
    "jobRoles.confirmDelete": "この職務を削除してもよろしいですか？",
    "jobRoles.deleteError": "使用中の職務は削除できません",
    "jobRoles.createSuccess": "職務が作成されました",
    "jobRoles.updateSuccess": "職務が更新されました",
    "jobRoles.deleteSuccess": "職務が削除されました",
    "jobRoles.nameRequired": "職務名は必須です",
    "jobRoles.nameDuplicate": "既に存在する職務名です",
    "jobRoles.nameMaxLength": "職務名は48文字以下である必要があります",
    "jobRoles.codeMaxLength": "コードは32文字以下である必要があります",
    "jobRoles.descriptionMaxLength": "説明は160文字以下である必要があります",
    "jobRoles.namePlaceholder": "職務名を入力してください",
    "jobRoles.descriptionPlaceholder": "職務の説明を入力してください",
    "jobRoles.list": "職務一覧",
    "jobRoles.listDescription": "現在この店舗で使用中の職務です",
    "jobRoles.createdAt": "作成日",
    "jobRoles.noRoles": "職務が見つかりません",
    invite: {
      title: "パートタイマー招待",
      create: "招待作成",
      list: "招待リスト",
      email: "メールアドレス",
      role: "役割",
      expiresIn: "有効期限",
      status: "ステータス",
      actions: "アクション",
      resend: "再送信",
      cancel: "キャンセル",
      accept: "承諾",
      pending: "保留中",
      accepted: "承諾済み",
      expired: "期限切れ",
      cancelled: "キャンセル済み",
      partTimer: "パートタイマー",
      subManager: "サブマネージャー",
      days: "日",
      createSuccess: "招待が正常に作成されました",
      createError: "招待の作成に失敗しました",
      resendSuccess: "招待が正常に再送信されました",
      resendError: "招待の再送信に失敗しました",
      cancelSuccess: "招待が正常にキャンセルされました",
      cancelError: "招待のキャンセルに失敗しました",
      acceptSuccess: "招待が正常に承諾されました",
      acceptError: "招待の承諾に失敗しました",
      invalidToken: "無効または期限切れの招待です",
      noPermission: "招待管理の権限がありません",
      onlyPendingResend: "保留中の招待のみ再送信できます",
      onlyPendingCancel: "保留中の招待のみキャンセルできます",
      emailRequired: "メールアドレスが必要です",
      invalidEmail: "有効なメールアドレスを入力してください",
      roleRequired: "役割が必要です",
      expiresInRequired: "有効期限が必要です",
      expiresInRange: "有効期限は1日から30日の間である必要があります",
      tokenRequired: "招待トークンが必要です",
      subject: "店舗招待",
      body: {
        welcome: "こんにちは！",
        invited: "店舗に招待されました。",
        storeName: "店舗名",
        role: "役割",
        expiresAt: "有効期限",
        cta: "招待を承諾",
        footer: "このリンクは7日後に期限切れになります。",
      },
    },
  },
  // 초대 시스템
  invite: {
    title: "파트타이머 초대",
    create: "초대 생성",
    list: "초대 목록",
    email: "이메일 주소",
    role: "역할",
    expiresIn: "만료 기간",
    status: "상태",
    actions: "작업",
    resend: "재발송",
    cancel: "취소",
    accept: "수락",
    pending: "대기 중",
    accepted: "수락됨",
    expired: "만료됨",
    cancelled: "취소됨",
    partTimer: "파트타이머",
    subManager: "서브 매니저",
    days: "일",
    createSuccess: "초대가 성공적으로 생성되었습니다",
    createError: "초대 생성에 실패했습니다",
    resendSuccess: "초대가 성공적으로 재발송되었습니다",
    resendError: "초대 재발송에 실패했습니다",
    cancelSuccess: "초대가 성공적으로 취소되었습니다",
    cancelError: "초대 취소에 실패했습니다",
    acceptSuccess: "초대가 성공적으로 수락되었습니다",
    acceptError: "초대 수락에 실패했습니다",
    invalidToken: "유효하지 않거나 만료된 초대입니다",
    noPermission: "초대 관리 권한이 없습니다",
    onlyPendingResend: "대기 중인 초대만 재발송할 수 있습니다",
    onlyPendingCancel: "대기 중인 초대만 취소할 수 있습니다",
    emailRequired: "이메일 주소를 입력해주세요",
    invalidEmail: "유효한 이메일 주소를 입력해주세요",
    roleRequired: "역할을 선택해주세요",
    expiresInRequired: "만료 기간을 입력해주세요",
    expiresInRange: "만료 기간은 1일에서 30일 사이여야 합니다",
    tokenRequired: "초대 토큰이 필요합니다",
    subject: "매장 초대",
    body: {
      welcome: "안녕하세요!",
      invited: "매장에 초대되었습니다.",
      storeName: "매장명",
      role: "역할",
      expiresAt: "만료일",
      cta: "초대 수락하기",
      footer: "이 링크는 7일 후 만료됩니다.",
    },
  },
} as const;

// 언어 검증
export function isValidLocale(locale: string): locale is Locale {
  return locales.includes(locale as Locale);
}

// 언어 쿠키 설정
export function setLocaleCookie(locale: Locale) {
  if (typeof document !== "undefined") {
    document.cookie = `locale=${locale}; path=/; max-age=31536000; SameSite=lax`;
  }
}

// 쿠키에서 언어 가져오기
export function getLocaleFromCookie(): Locale | null {
  if (typeof document === "undefined") return null;

  const cookies = document.cookie.split(";");
  const localeCookie = cookies.find((cookie) =>
    cookie.trim().startsWith("locale=")
  );

  if (localeCookie) {
    const locale = localeCookie.split("=")[1];
    if (isValidLocale(locale)) {
      return locale as Locale;
    }
  }

  return null;
}

// 브라우저 언어 감지
export function detectLocale(): Locale {
  if (typeof window !== "undefined") {
    const browserLocale = navigator.language.split("-")[0];
    if (isValidLocale(browserLocale)) {
      return browserLocale as Locale;
    }
  }
  return defaultLocale;
}

// 번역 함수 (플랫 구조용, 매개변수 치환 지원)
export function t(
  key: string,
  locale: Locale = defaultLocale,
  params?: Record<string, string | number>
): string {
  const translationData = translations[locale];
  let translation =
    translationData?.[key as keyof typeof translationData] || key;

  // 매개변수 치환
  if (params) {
    Object.entries(params).forEach(([param, value]) => {
      translation = translation.replace(
        new RegExp(`{${param}}`, "g"),
        String(value)
      );
    });
  }

  return translation;
}

// 현재 언어 상태를 관리하는 간단한 상태
let currentLocale: Locale = defaultLocale;

export function getCurrentLocale(): Locale {
  return currentLocale;
}

export function setCurrentLocale(locale: Locale): void {
  currentLocale = locale;
}

// 레거시 호환성을 위한 exports
export const SUPPORTED_LOCALES = locales;
export type SupportedLocale = Locale;
export const DEFAULT_LOCALE = defaultLocale;
export const LOCALE_NAMES = localeNames;
